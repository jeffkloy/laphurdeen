# LAPHURDI — Spraket av Laphurdeen
*The National Language of the Commonwealth — Language Commission Working Reference, v1*

---

## 1. Design Brief

Laphurdi is a blended language with four source streams:

| Source | Contributes |
|---|---|
| **English** | Core everyday vocabulary, flexible word-building |
| **Dutch** | Spelling flavour (ij, oe, aa), pronouns, earthy common words |
| **Swedish** | Grammar skeleton: suffixed definite article, person-free verbs, V2 word order |
| **French** | The "high register": law, government, cuisine, art — and heritage spellings in proper names |

The result reads as a Germanic language wearing a French coat — much as English itself does, but with the mixture rebalanced.

### In-world history
The Founding Charter settlers arrived speaking English, Dutch, Swedish, and French. A contact pidgin formed on the docks of Laphurdeen bay within a generation; the **First Spelling Reform** later standardised it into Standard Laphurdi. The Reform replaced etymological spellings with phonetic ones (*ph* → *f*, so *frihed* not *phrihed*) — **but proper names kept their heritage spellings**, which is why *Laphurdeen* and *Laphurdi* retain the old ⟨ph⟩.

---

## 2. Alphabet and Sound

Laphurdi uses the plain Latin alphabet, no accents. Distinctive values:

| Spelling | Sound | Example |
|---|---|---|
| **ij** | "ay" as in *day* (Dutch heritage) | *blij* (happy) → "blay" |
| **oe** | "oo" as in *moon* | *goed* (good) → "good" |
| **aa** | long "ah" | *maan* (moon) → "mahn" |
| **sj** | "sh" | *sju* (seven) → "shoo" |
| **k** | always hard, even before e/i | *kind* (child) → "kint" |

**Stress** falls on the first syllable (Germanic rule) — *except* in French-heritage loans, which keep final stress: *famille* (fa-MIL), *nasjon* (na-SJON), *Lapentieur* (la-pen-TIEUR).

---

## 3. Grammar Core

### Nouns: two genders, suffixed "the"
Like Swedish, "the" is glued onto the end of the noun. Two genders: **common** (en-words) and **neuter** (et-words).

| | a/an | the | plural | the (plural) |
|---|---|---|---|---|
| common | **en** stad (a city) | stad**en** (the city) | stad**er** | stad**eren** |
| neuter | **et** hus (a house) | hus**et** (the house) | hus**er** | hus**eren** |

The country's own name uses an archaic form of this: **Laphurdeen** = *Laphurde* + old definite *-een* — the name literally means "The Anchorage" (§7).

### Verbs: no person endings, ever
Verbs conjugate for tense only — the same form for I/you/she/we/they (Swedish's great simplification):

| tense | form | example: *spreka* (to speak) |
|---|---|---|
| infinitive | -a | spreka |
| present | -ar | ik/du/hen/vi sprek**ar** |
| past | -ade | sprek**ade** |
| perfect | har + -at | har sprek**at** |
| future | skal + inf. | skal spreka |

Irregular but essential: **vera** (to be) → **er** (is/am/are), **var** (was/were).

### Pronouns
ik, du, han / hon / **hen**, vi, ju, dei — *I, you, he / she / they(sg.), we, you(pl.), they.*

**Hen** is the standard gender-neutral singular pronoun (borrowed from Swedish), used both for non-binary people and whenever gender is unknown — the Constitution is drafted with *hen* throughout its Laphurdi text.

### Word order: verb comes second
Main clauses are V2, like Dutch and Swedish — whatever comes first, the verb is next:

- *Vi stemmar idag.* — We vote today.
- *Idag stemmar vi.* — Today we vote. (verb stays second!)

Negation: **nit** after the verb — *Ik sprekar nit Fransk* (I don't speak French).
Questions: verb first — *Sprekar du Laphurdi?*

---

## 4. Core Vocabulary

### The world
| Laphurdi | English | blended from |
|---|---|---|
| vatter | water | water/water/vatten |
| zee | sea | zee (NL) |
| iland | island | island/eiland |
| sol | sun | sol (SV) + soleil (FR) |
| maan | moon | maan (NL) |
| himmel | sky | himmel (SV) |
| berg | mountain | berg (NL/SV) |
| strand | beach | strand (NL/SV) |
| regn | rain | regn (SV) |
| stad | city | stad (NL/SV) |
| hus | house | huis/hus |
| dag / natt | day / night | dag; natt (SV) + nacht (NL) |

### People and life
| Laphurdi | English | notes |
|---|---|---|
| folk | people | *folket* = the people |
| kvin / man / kind | woman / man / child | kvinna (SV); kind (NL) |
| vrend | friend | vriend/vän/friend |
| famille | family | French loan, final stress |
| werka | to work | werken/work |
| leva | to live | leva (SV) |
| lieva | to love | liefde (NL) + love |
| eta / drika | to eat / to drink | äta (SV); dricka (SV)/drinken (NL) |
| bygga | to build | bygga (SV) |
| veta | to know | veta (SV)/weten (NL) |
| hjelpa | to help | hjälpa (SV)/helpen (NL) |
| goed | good | NL spelling, say "good" |
| stor / liten | big / small | Swedish pair |
| fri | free | |
| blij | happy | NL, say "blay" |
| bela | beautiful | from beau/belle (FR) |
| ny / gammel | new / old | SV |
| sang | song | sång (SV) + zang (NL) + song |
| mange | many | många (SV) + many |

### Civic life (the French-flavoured high register)
| Laphurdi | English | notes |
|---|---|---|
| **Grundlojen** | the Constitution | *grund* (foundation) + *loj* (law, from *loi*) |
| loj | law | statutes are *lojer* |
| rekt | a right | *rekteren* = the rights |
| frihed | freedom | |
| velvard | welfare | välfärd/welzijn/welfare |
| konsens | consensus | |
| stemma / en stem | to vote / a vote | stemmen (NL) |
| **Senaten** | the Senate | |
| **Folkskameren** | the Commons | lit. "the People's Chamber" (NL *Kamer*) |
| ministerie | ministry | *Helsaministeriet* = the Health Ministry |
| Ledminister | Lead Minister | |
| skatt | tax | SV |
| helsa | health | hälsa (SV) |
| skol / universitet | school / university | |
| krona, *pl.* **kronur** | the currency | irregular heritage plural, fixed by the Currency Act |

### Small words
og (and) · el (or) · nit (not) · in, on, med, for, av, te (in, on, with, for, of/from, to) · waar (where) · ja / nej (yes / no)

---

## 5. Numbers

**en, twe, tri, fyr, fem, seks, sju, akt, nien, tien** (1–10) · **hundra** (100) · **tusen** (1,000)

The tax-free threshold, as every Laphurdeener learns to say: ***hundra tusen kronur***.

---

## 6. Phrases

| Laphurdi | English |
|---|---|
| Hallej! | Hello! |
| Goed morgen / Goed natt | Good morning / Good night |
| Velkom te Laphurdeen! | Welcome to Laphurdeen! |
| Dank du. / Mersi! | Thank you (formal / everyday) |
| Asjeblie | Please |
| Adjuu! | Goodbye! |
| Sprekar du Laphurdi? | Do you speak Laphurdi? |
| Ik sprekar en liten Laphurdi. | I speak a little Laphurdi. |
| Waar er staden? | Where is the city? |
| Hen werkar in Helsaministeriet. | They (sg.) work in the Health Ministry. |
| Idag stemmar folket. | Today the people vote. |
| Vi byggar en ny nasjon. | We are building a new nation. |

### The national motto
> **Frihed, Velvard, Konsens.**
> *Freedom, Welfare, Consensus.*

### The national anthem
> **Sang av de Mange Strander** — *Song of the Many Shores.*

Four verses arrive from the four founding shores — English, Dutch, Swedish, French — and the chorus unites them in Laphurdi (*strander* is the plural of *strand*, shore/beach; see `NATIONAL_SYMBOLS.md` for the full structure). Lyrics are a future session's work.

### The Preamble's first line, in Laphurdi
> **Vi, folket av Laphurdeen, kom fri fra mange strander te bygga en nasjon waar ingen stod befor.**
> *We, the people of Laphurdeen, came freely from many shores to build a nation where none stood before.*

---

## 7. Place Names and Etymology

- **Laphurdeen** — Old Charter *La Fjärde* (French article + Swedish *fjärd*, "wide bay") → *Laphurde*, "the anchorage-bay," plus the archaic definite ending *-een*: **"The Anchorage."** The name commemorates the sheltered bay where the Charter fleet first dropped anchor.
- **Lapentieur** — French *la pente* ("the slope") + the Old Charter place-suffix *-ieur*: **"the place of slopes"** — apt for the fog-brushed hill city that became the capital.
- **Darcambria** — *dar* ("amber," a Charter-era blend of French *doré*, gilded, and English *amber*) + *cambria* (from French *cambrer*, "to curve"): **"the amber curve,"** for the city's great curved harbour and its amber summer sunsets.
- **krona / kronur** — the currency keeps an irregular heritage plural (*en krona, twe kronur*), one of the few survivals of early-settlement Norse-style inflection.

---

## 8. Open Questions — for future dives

- **Dialects**: Lapentieuran vs. Darcambrian speech — early notes suggest Darcambrians say *wi* (from French *oui*) instead of *ja*, and clip final -ar to -a.
- **Irregular verbs**: only *vera* (be) is fixed so far; decide the closed list (go, come, see, do…).
- **Full phonology**: an IPA chart and syllable rules.
- **Registers**: how far does the French high register extend (courts? academia? menus?).
- **Laphurdi Sign Language (LSL)**: recognition and standardisation — connects to Article 5(6) of the Constitution.
- **Orthography edge cases**: what else survived the First Spelling Reform besides proper names?
- **A proper lexicon**: grow the vocabulary from ~90 words toward a usable core of 1,000.
