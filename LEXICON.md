# LEXICON - Laphurdi–English

*Generated from `LEXICON.tsv` by `tools/lexicon.py build` - do not edit by hand.*

**2367 words** - 1993 roots, 374 derived.

| domain | words |
|---|---|
| world-nature | 86 |
| plants-animals | 52 |
| body-health | 256 |
| people-family | 76 |
| food-drink | 144 |
| house-home | 65 |
| clothing | 26 |
| time-calendar | 53 |
| numbers-measure | 62 |
| motion-travel | 53 |
| sea-ships | 44 |
| work-trade | 84 |
| communication | 102 |
| mind-emotion | 146 |
| law-civic | 180 |
| arts-leisure | 59 |
| school-knowledge | 66 |
| science | 175 |
| digital | 112 |
| society | 93 |
| common-verbs | 148 |
| qualities | 158 |
| function-words | 127 |

## world-nature

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **aske** | n (c) | ash |  | DA aske + SV aska |
| **avstand** | n (n) | distance, gap |  | av + stand |
| **bark** | n (c) | bark (of tree) |  | SV bark |
| **berg** | n (c) | mountain |  | NL berg + SV berg |
| **blad** | n (n) | leaf |  | NL blad + SV blad |
| **blixt** | n (c) | lightning |  | SV blixt |
| **blomma** | n (c) | flower |  | SV blomma + NL bloem |
| **boge** | n (c) | bow, arc |  | SV båge + NL boog |
| **dal** | n (c) | valley |  | SV dal |
| **Darcambria** | n (c) | Darcambria (the harbour city) |  | FR doré + EN amber + FR cambrer |
| **doft** | n (c) | scent, fragrance |  | SV doft |
| **ekko** | n (n) | echo |  | DA ekko |
| **eld** | n (c) | fire |  | SV eld |
| **energi** | n (c) | energy | high | FR énergie |
| **felt** | n (n) | field |  | DA felt + SV fält |
| **flod** | n (c) | river |  | SV flod |
| **fro** | n (n) | seed |  | SV frö |
| **frukt** | n (c) | fruit |  | SV frukt + FR fruit |
| **gnista** | n (c) | spark |  | SV gnista + DA gnist |
| **gnistra** | v | sparkle, glitter |  | SV gnistra |
| **gras** | n (n) | grass; weed (marijuana, slang) |  | NL gras + EN grass |
| **grund** | n (c) | foundation, ground |  | SV grund + NL grond |
| **himmel** | n (c) | sky |  | SV himmel |
| **holm** | n (c) | islet |  | SV holme |
| **iland** | n (n) | island |  | EN island + NL eiland |
| **ilandgrupp** | n (c) | archipelago |  | iland + grupp |
| **is** | n (c) | ice |  | SV is + NL ijs |
| **jord** | n (c) | earth, soil |  | SV jord |
| **kap** | n (c) | cape, headland |  | NL kaap + EN cape |
| **katastrof** | n (c) | catastrophe | high | FR catastrophe |
| **klippa** | n (c) | cliff |  | SV klippa |
| **kopstad** | n (c) | town, market town |  | kopa + stad |
| **kulle** | n (c) | hill |  | SV kulle |
| **kust** | n (c) | coast |  | SV kust + NL kust |
| **lak** | n (c) | lake |  | FR lac + EN lake |
| **land** | n (n) | land, country |  | EN land + NL land + SV land |
| **landskap** | n (n) | landscape |  | SV landskap + NL landschap |
| **Lapentieur** | n (c) | Lapentieur (the capital) |  | FR la pente + -ieur |
| **Laphurdeen** | n (c) | Laphurdeen (the Commonwealth) |  | FR la + SV fjärd |
| **luft** | n (c) | air |  | SV luft + NL lucht |
| **lyd** | n (c) | sound |  | DA lyd + SV ljud |
| **lyda** | v | sound (v.) |  | lyd + -a |
| **lys** | n (n) | light |  | DA lys + SV ljus |
| **maan** | n (c) | moon |  | NL maan |
| **marsk** | n (c) | salt marsh |  | DA marsk + DE Marsch |
| **mist** | n (c) | fog |  | EN mist + NL mist |
| **morkhed** | n (c) | darkness |  | mork + -hed |
| **natur** | n (c) | nature |  | FR nature + SV natur |
| **nord** | n (c) | north |  | FR nord + SV norr |
| **nordster** | n (c) | north star, polestar |  | nord + ster |
| **oest** | n (c) | east |  | NL oost |
| **paradis** | n (n) | paradise |  | FR paradis + SV paradis |
| **park** | n (c) | park |  | EN park + NL park + SV park |
| **pinn** | n (c) | stick, twig |  | SV pinne |
| **regn** | n (n) | rain |  | SV regn |
| **regna** | v | rain (v.) |  | regn + -a |
| **regnboge** | n (c) | rainbow |  | regn + boge |
| **rok** | n (c) | smoke |  | SV rök + NL rook |
| **rot** | n (c) | root |  | SV rot + EN root |
| **salt** | n (n) | salt |  | SV salt |
| **sand** | n (c) | sand |  | SV sand + NL zand |
| **skog** | n (c) | forest |  | SV skog |
| **sne** | n (c) | snow |  | DA sne |
| **sol** | n (c) | sun |  | SV sol + FR soleil |
| **solnedgang** | n (c) | sunset |  | SV solnedgång + DA solnedgang |
| **soloppgang** | n (c) | sunrise |  | SV soluppgång + DA solopgang |
| **stad** | n (c) | city |  | NL stad + SV stad |
| **sten** | n (c) | stone |  | SV sten |
| **ster** | n (c) | star |  | NL ster + EN star |
| **sterlys** | n (n) | starlight |  | ster + lys |
| **storm** | n (c) | storm |  | EN storm + SV storm |
| **stov** | n (n) | dust |  | DA støv |
| **strand** | n (c) | beach, shore |  | NL strand + SV strand |
| **syd** | n (c) | south |  | SV syd |
| **torden** | n (c) | thunder |  | DA torden |
| **tre** | n (n) | tree |  | SV träd |
| **vatter** | n (n) | water |  | EN water + NL water + SV vatten |
| **vatterfall** | n (n) | waterfall |  | vatter + fall |
| **veder** | n (n) | weather |  | SV väder |
| **veg** | n (c) | road, way |  | SV väg + DA vej |
| **vind** | n (c) | wind |  | SV vind + NL wind |
| **vulkan** | n (c) | volcano |  | SV vulkan + FR volcan |
| **werld** | n (c) | world |  | NL wereld + EN world |
| **west** | n (c) | west |  | NL west + EN west |
| **wolk** | n (c) | cloud |  | NL wolk |
| **zee** | n (c) | sea |  | NL zee |

## plants-animals

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **alg** | n (c) | seaweed |  | SV alg |
| **and** | n (c) | duck |  | SV and + DA and |
| **bever** | n (c) | beaver |  | NL bever |
| **bij** | n (c) | bee |  | NL bij |
| **bjork** | n (c) | birch |  | SV björk |
| **bjorn** | n (c) | bear |  | SV björn |
| **dier** | n (n) | animal |  | NL dier |
| **egg** | n (n) | egg |  | SV ägg + EN egg |
| **ek** | n (c) | oak |  | SV ek |
| **elg** | n (c) | elk, moose |  | SV älg |
| **faar** | n (n) | sheep |  | SV får |
| **feder** | n (c) | feather |  | EN feather + NL veder |
| **fisk** | n (c) | fish |  | SV fisk |
| **fjeril** | n (c) | butterfly |  | SV fjäril |
| **flieg** | n (c) | fly (insect) |  | NL vlieg + EN fly |
| **fugel** | n (c) | bird |  | SV fågel + NL vogel |
| **get** | n (c) | goat |  | SV get + NL geit |
| **gran** | n (c) | spruce |  | SV gran |
| **hare** | n (c) | hare |  | SV hare + NL haas |
| **hest** | n (c) | horse |  | DA hest |
| **horn** | n (n) | horn |  | SV horn |
| **hund** | n (c) | dog |  | SV hund + NL hond |
| **insekt** | n (c) | insect |  | SV insekt + FR insecte |
| **kalkon** | n (c) | turkey |  | SV kalkon |
| **katt** | n (c) | cat |  | SV katt + NL kat |
| **kip** | n (c) | chicken |  | NL kip |
| **ko** | n (c) | cow |  | SV ko + NL koe |
| **korall** | n (c) | coral |  | SV korall + FR corail |
| **krab** | n (c) | crab |  | NL krab + EN crab |
| **laks** | n (c) | salmon |  | DA laks + SV lax |
| **lus** | n (c) | louse |  | SV lus |
| **maage** | n (c) | gull |  | DA måge |
| **mus** | n (c) | mouse |  | SV mus + NL muis |
| **orm** | n (c) | snake |  | SV orm |
| **ostra** | n (c) | oyster |  | SV ostron + FR huître |
| **pels** | n (c) | fur |  | DA pels |
| **planta** | n (c) | plant |  | SV planta + FR plante |
| **ratt** | n (c) | rat |  | SV råtta + NL rat |
| **rev** | n (c) | fox |  | SV räv |
| **ros** | n (c) | rose |  | SV ros + FR rose |
| **sel** | n (c) | seal |  | SV säl + DA sæl |
| **sild** | n (c) | herring |  | DA sild |
| **skel** | n (c) | shell |  | NL schelp |
| **spin** | n (c) | spider |  | NL spin |
| **stert** | n (c) | tail |  | DA stjert + NL staart |
| **svan** | n (c) | swan |  | SV svan |
| **svin** | n (n) | pig |  | SV svin |
| **torsk** | n (c) | cod |  | SV torsk + DA torsk |
| **tulp** | n (c) | tulip |  | NL tulp |
| **val** | n (c) | whale |  | SV val |
| **vinge** | n (c) | wing |  | SV vinge |
| **wurm** | n (c) | worm |  | NL worm + EN worm |

## body-health

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **adem** | n (c) | breath |  | NL adem |
| **adiksjon** | n (c) | addiction | high | FR addiction |
| **akut** | adj | acute |  | SV akut + FR aigu |
| **alergi** | n (c) | allergy |  | FR allergie |
| **alergisk** | adj | allergic |  | alergi + -isk |
| **ambulans** | n (c) | ambulance |  | SV ambulans + FR ambulance |
| **amputasjon** | n (c) | amputation |  | FR amputation |
| **amputera** | v | amputate |  | FR amputer |
| **anda** | v | breathe |  | SV andas |
| **antibiotika** | n (c) | antibiotic |  | FR antibiotique |
| **apotek** | n (n) | pharmacy |  | SV apotek + FR apothèque |
| **apotekare** | n (c) | pharmacist |  | SV apotekare |
| **arm** | n (c) | arm |  | EN arm + NL arm + SV arm |
| **arr** | n (n) | scar |  | SV ärr + DA ar |
| **ars** | n (c) | ass (vulgar) | everyday | SV arsle + EN arse + NL aars |
| **arterie** | n (c) | artery |  | FR artère + SV artär |
| **astma** | n (c) | asthma |  | SV astma + FR asthme |
| **avhengig** | adj | addicted, dependent |  | DA afhængig |
| **avhengighed** | n (c) | addiction, dependence |  | avhengig + -hed |
| **baar** | n (c) | stretcher |  | SV bår |
| **bad** | n (n) | bath |  | SV bad + NL bad |
| **bak** | n (c) | backside, butt | everyday | SV bak + DA bagdel |
| **bakterie** | n (c) | bacterium |  | FR bactérie + SV bakterie |
| **been** | n (n) | leg |  | NL been |
| **behandla** | v | treat (medically) |  | NL behandelen + SV behandla |
| **behandling** | n (c) | treatment |  | behandla + -ing |
| **ben** | n (n) | bone |  | SV ben |
| **benign** | adj | benign | high | FR bénin |
| **bita** | v | bite |  | SV bita + NL bijten |
| **biverkning** | n (c) | side effect |  | SV biverkning |
| **blaamerke** | n (n) | bruise |  | blaa + merke |
| **blaas** | n (c) | bladder |  | NL blaas + SV blåsa |
| **blind** | adj | blind |  | SV blind + NL blind |
| **blod** | n (n) | blood |  | SV blod + NL bloed |
| **bloda** | v | bleed |  | blod + -a |
| **blodgrupp** | n (c) | blood type |  | blod + grupp |
| **blodkerl** | n (n) | blood vessel |  | blod + kerl |
| **blodpropp** | n (c) | blood clot |  | blod + propp |
| **blodstrom** | n (c) | bloodstream |  | blod + strom |
| **blodtrykk** | n (n) | blood pressure |  | blod + trykk |
| **borderline** | n (c) | borderline (personality condition) | high | EN borderline |
| **bot** | n (c) | remedy, cure |  | SV bot |
| **botmedel** | n (n) | remedy |  | bot + medel |
| **braka** | v | vomit |  | NL braken |
| **brusk** | n (n) | cartilage |  | SV brosk + NL kraakbeen |
| **bryst** | n (n) | breast, chest |  | DA bryst + SV bröst |
| **depresjon** | n (c) | depression | high | FR dépression |
| **desedera** | v | die | high | FR décéder |
| **diabetes** | n (c) | diabetes |  | EN diabetes + FR diabète |
| **diagnos** | n (c) | diagnosis | high | FR diagnose + SV diagnos |
| **diagnostisera** | v | diagnose |  | FR diagnostiquer |
| **dod** | n (c) | death |  | SV död + DA død |
| **doktor** | n (c) | doctor | everyday | SV doktor + NL dokter |
| **domna** | v | go numb |  | SV domna |
| **donor** | n (c) | donor |  | EN donor + FR donneur |
| **dos** | n (c) | dose |  | FR dose + SV dos |
| **dosera** | v | dose, measure out |  | FR doser |
| **dosering** | n (c) | dosage |  | dosera + -ing |
| **dov** | adj | deaf |  | SV döv + DA døv |
| **dreppa** | v | kill |  | DA dræbe |
| **drog** | n (c) | drug |  | SV drog + FR drogue |
| **dronken** | adj | drunk |  | NL dronken |
| **drukna** | v | drown |  | SV drunkna + DA drukne |
| **epidemi** | n (c) | epidemic | high | FR épidémie |
| **feber** | n (c) | fever |  | SV feber + NL koorts |
| **feberig** | adj | feverish |  | feber + -ig |
| **fett** | n (n) | fat (n.) |  | SV fett + NL vet |
| **finger** | n (n) | finger |  | EN finger + SV finger |
| **fitta** | n (c) | pussy (vulgar) | everyday | SV fitta |
| **foda** | v | give birth, bear |  | SV föda |
| **foding** | n (c) | birth |  | foda + -ing |
| **forbind** | n (n) | bandage |  | NL verband + SV förband |
| **fot** | n (c) | foot |  | SV fot |
| **frakt** | n (c) | fracture |  | FR fracture |
| **frisk** | adj | fresh, well |  | SV frisk + NL fris |
| **friskna** | v | recover (get well) |  | frisk + -na |
| **funksjonshinder** | n (n) | disability |  | funksjon + hinder |
| **fysioterapi** | n (c) | physiotherapy |  | FR physiothérapie |
| **galla** | n (c) | bile |  | SV galla + NL gal |
| **gift** | n (n) | poison |  | SV gift + NL gif |
| **giftig** | adj | poisonous |  | gift + -ig |
| **gips** | n (n) | plaster cast |  | SV gips + NL gips |
| **gravid** | adj | pregnant |  | SV gravid + FR gravide |
| **haar** | n (n) | hair |  | NL haar |
| **hals** | n (c) | neck |  | SV hals + NL hals |
| **hand** | n (c) | hand |  | EN hand + NL hand + SV hand |
| **hela** | v | heal |  | SV hela + NL helen |
| **helning** | n (c) | healing |  | hela + -ing |
| **helsa** | n (c) | health |  | SV hälsa |
| **helsevard** | n (c) | healthcare |  | helsa + vard |
| **hjerne** | n (c) | brain |  | SV hjärna + DA hjerne |
| **hjerneslag** | n (n) | stroke (medical) |  | hjerne + slag |
| **hjerta** | n (n) | heart |  | SV hjärta |
| **hjertinfarkt** | n (c) | heart attack |  | hjerta + infarkt |
| **hoft** | n (c) | hip |  | SV höft + NL heup |
| **hormon** | n (n) | hormone |  | SV hormon |
| **hosta** | v | cough |  | SV hosta |
| **hoved** | n (n) | head |  | DA hoved + NL hoofd |
| **hovedverk** | n (c) | headache |  | hoved + verk |
| **hud** | n (c) | skin |  | SV hud |
| **hungrig** | adj | hungry |  | hunger + -ig |
| **infarkt** | n (c) | infarction | high | FR infarctus + SV infarkt |
| **infeksjon** | n (c) | infection |  | FR infection |
| **inflamasjon** | n (c) | inflammation |  | FR inflammation |
| **injeksjon** | n (c) | injection |  | FR injection |
| **isolera** | v | isolate |  | FR isoler |
| **isolering** | n (c) | isolation |  | isolera + -ing |
| **kanabis** | n (c) | cannabis |  | EN cannabis |
| **karantene** | n (c) | quarantine |  | FR quarantaine |
| **kerl** | n (n) | vessel (anatomical) |  | SV kärl |
| **kirurg** | n (c) | surgeon | high | FR chirurgien |
| **kirurgi** | n (c) | surgery (discipline) | high | FR chirurgie |
| **klaada** | n (c) | itch |  | SV klåda |
| **klia** | v | itch (v.) |  | SV klia |
| **klinik** | n (c) | clinic |  | FR clinique |
| **knie** | n (n) | knee |  | NL knie |
| **knulla** | v | fuck (vulgar) | everyday | SV knulla + DA kneppe + NL neuken |
| **kon** | n (n) | sex, gender |  | SV kön |
| **kraft** | n (c) | strength, power |  | SV kraft + NL kracht |
| **kranium** | n (n) | skull | high | FR crâne + SV kranium |
| **kreft** | n (c) | cancer |  | DA kræft + SV kräfta |
| **kronisk** | adj | chronic |  | FR chronique |
| **kropp** | n (c) | body |  | SV kropp |
| **krykke** | n (c) | crutch |  | DA krykke + NL kruk |
| **kuk** | n (c) | dick (vulgar) | everyday | SV kuk |
| **kur** | n (c) | cure |  | FR cure + SV kur |
| **kwalme** | n (c) | nausea |  | DA kvalme + NL misselijkheid |
| **laar** | n (n) | thigh |  | SV lår + DA lår |
| **led** | n (c) | joint (of the body) |  | SV led + NL lid |
| **lekare** | n (c) | physician |  | SV läkare + DA læge |
| **lepp** | n (c) | lip |  | SV läpp + DA læbe |
| **lever** | n (c) | liver |  | SV lever + NL lever |
| **liv** | n (n) | life |  | SV liv + DA liv |
| **lukta** | v | smell |  | SV lukta |
| **lunga** | n (c) | lung |  | SV lunga |
| **mage** | n (c) | belly |  | SV mage |
| **magsap** | n (c) | gastric juice |  | mage + sap |
| **magsekk** | n (c) | stomach (organ) |  | mage + sekk |
| **malad** | adj | sick | high | FR malade |
| **malign** | adj | malignant | high | FR malin |
| **medel** | n (n) | means, agent |  | SV medel |
| **medesin** | n (c) | doctor | high | FR médecin |
| **medikament** | n (n) | medication |  | FR médicament + SV medikament |
| **medisin** | n (c) | medicine | high | FR médecine |
| **merg** | n (c) | marrow |  | SV märg + NL merg |
| **merke** | n (n) | mark |  | SV märke + NL merk |
| **mett** | adj | full (after eating), sated |  | SV mätt + DA mæt |
| **midja** | n (c) | waist |  | SV midja |
| **mund** | n (c) | mouth |  | DA mund + NL mond |
| **muskel** | n (c) | muscle |  | SV muskel + FR muscle |
| **nagel** | n (c) | fingernail |  | NL nagel |
| **narkose** | n (c) | anaesthesia |  | SV narkos + FR narcose |
| **nera** | v | nourish |  | SV nära + DA nære |
| **nering** | n (c) | nourishment, nutrition |  | nera + -ing |
| **nerv** | n (c) | nerve |  | SV nerv + NL zenuw |
| **nese** | n (c) | nose |  | DA næse + NL neus |
| **njure** | n (c) | kidney |  | SV njure + DA nyre |
| **num** | adj | numb |  | EN numb |
| **nykter** | adj | sober |  | SV nykter |
| **nykterhed** | n (c) | sobriety |  | nykter + -hed |
| **nysa** | v | sneeze |  | SV nysa |
| **obduksjon** | n (c) | autopsy |  | SV obduktion |
| **oje** | n (n) | eye |  | DA øje |
| **okraft** | n (c) | incapacity |  | o- + kraft |
| **onkologi** | n (c) | oncology |  | FR oncologie |
| **operasjon** | n (c) | operation |  | FR opération |
| **operera** | v | operate (surgically) |  | FR opérer |
| **ore** | n (n) | ear |  | SV öra + DA øre |
| **organdonor** | n (c) | organ donor |  | organ + donor |
| **overleva** | v | survive |  | over + leva |
| **overleving** | n (c) | survival |  | overleva + -ing |
| **parasit** | n (c) | parasite |  | FR parasite |
| **pasient** | n (c) | patient |  | FR patient |
| **pille** | n (c) | pill |  | DA pille |
| **por** | n (c) | pore |  | FR pore + SV por |
| **protes** | n (c) | prosthesis |  | FR prothèse |
| **psykiater** | n (c) | psychiatrist |  | FR psychiatre |
| **psykiatri** | n (c) | psychiatry |  | FR psychiatrie |
| **puls** | n (c) | pulse |  | FR pouls + SV puls |
| **rehabilitering** | n (c) | rehabilitation |  | FR réhabilitation |
| **reproduksjon** | n (c) | reproduction | high | FR reproduction |
| **resept** | n (n) | prescription |  | SV recept + FR recette |
| **ribbe** | n (c) | rib |  | NL rib + SV revben |
| **rontgen** | n (c) | x-ray |  | SV röntgen |
| **rugg** | n (c) | back |  | NL rug + SV rygg |
| **ruggmerg** | n (c) | spinal cord |  | rugg + merg |
| **rulla** | v | roll |  | SV rulla + NL rollen |
| **rullstol** | n (c) | wheelchair |  | rulla + stol |
| **saliv** | n (c) | saliva |  | FR salive |
| **sar** | n (n) | wound |  | SV sår + NL zeer |
| **sara** | v | hurt, wound (v.) |  | sar + -a |
| **seksuell** | adj | sexual | high | FR sexuel |
| **selfmord** | n (n) | suicide |  | self + mord |
| **sena** | n (c) | tendon |  | SV sena |
| **sep** | n (c) | soap |  | NL zeep + SV såpa |
| **sinnsjukhed** | n (c) | mental illness |  | sinn + sjukhed |
| **sjuk** | adj | sick | everyday | SV sjuk |
| **sjukdom** | n (c) | disease |  | SV sjukdom |
| **sjukhed** | n (c) | illness |  | sjuk + -hed |
| **sjukhus** | n (n) | hospital |  | sjuk + hus |
| **sjukskotare** | n (c) | nurse |  | sjuk + skotare |
| **skade** | n (c) | injury |  | SV skada + NL schade |
| **skalle** | n (c) | skull | everyday | SV skalle + NL schedel |
| **skelett** | n (n) | skeleton |  | SV skelett + FR squelette |
| **skota** | v | tend, care for |  | SV sköta |
| **skotare** | n (c) | carer |  | skota + -are |
| **slag** | n (n) | blow, stroke |  | SV slag + NL slag |
| **slem** | n (n) | mucus |  | NL slijm + SV slem |
| **smerta** | n (c) | pain |  | SV smärta |
| **smerthed** | n (c) | soreness |  | smerta + -hed |
| **smertig** | adj | painful |  | smerta + -ig |
| **smertmedel** | n (n) | painkiller |  | smerta + medel |
| **smitta** | n (c) | contagion |  | SV smitta |
| **smittsam** | adj | contagious |  | SV smittsam |
| **sopp** | n (c) | fungus |  | SV sopp + NL zwam |
| **sova** | v | sleep |  | SV sova |
| **sovig** | adj | sleepy |  | sova + -ig |
| **spotta** | v | spit |  | SV spotta |
| **spruta** | n (c) | syringe |  | SV spruta |
| **steril** | adj | sterile |  | FR stérile + SV steril |
| **sterva** | v | die | everyday | NL sterven |
| **suga** | v | suck |  | SV suga |
| **sund** | adj | healthy |  | SV sund + DA sund |
| **svett** | n (c) | sweat |  | SV svett + NL zweet |
| **svimmel** | adj | dizzy |  | SV svimmel + NL duizelig |
| **svulst** | n (c) | swelling |  | SV svulst |
| **symptom** | n (n) | symptom |  | FR symptôme + SV symptom |
| **syndrom** | n (n) | syndrome |  | SV syndrom |
| **taar** | n (c) | tear (weeping) |  | SV tår + DA tåre |
| **tand** | n (c) | tooth |  | NL tand + SV tand |
| **tandlekare** | n (c) | dentist |  | tand + lekare |
| **tandverk** | n (c) | toothache |  | tand + verk |
| **tarm** | n (c) | gut, guts, intestine |  | SV tarm |
| **terapeut** | n (c) | therapist |  | FR thérapeute |
| **terapi** | n (c) | therapy |  | FR thérapie |
| **torstig** | adj | thirsty |  | torst + -ig |
| **transplantasjon** | n (c) | transplant (n.) |  | FR transplantation |
| **transplantera** | v | transplant (v.) |  | FR transplanter |
| **trott** | adj | tired |  | SV trött |
| **trykk** | n (n) | pressure |  | SV tryck + NL druk |
| **tumor** | n (c) | tumour |  | FR tumeur + SV tumör |
| **tunga** | n (c) | tongue |  | SV tunga |
| **tutt** | n (c) | tit (colloquial) | everyday | SV tutt + NL tiet |
| **undernera** | v | undernourish, malnourish |  | under + nera |
| **urin** | n (c) | urine |  | FR urine + SV urin |
| **utmatta** | v | exhaust, wear out |  | SV utmatta |
| **utmatting** | n (c) | exhaustion |  | utmatta + -ing |
| **utslag** | n (n) | rash |  | SV utslag |
| **vakna** | v | wake, wake up |  | SV vakna + DA vågne |
| **vaksin** | n (n) | vaccine |  | FR vaccin |
| **vaksinera** | v | vaccinate |  | FR vacciner |
| **vaska** | v | wash |  | SV vaska + NL wassen |
| **ven** | n (c) | vein |  | SV ven + FR veine |
| **verk** | n (c) | ache |  | SV värk |
| **verka** | v | ache, throb |  | SV värka |
| **virus** | n (n) | virus |  | EN virus + SV virus |

## people-family

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **alder** | n (c) | age |  | SV ålder |
| **arv** | n (n) | inheritance |  | SV arv |
| **asistans** | n (c) | help (n.) | high | FR assistance |
| **baat** | n (c) | the greatest of all time (slang) | everyday | EN acronym: Best Av Alle Tider |
| **bebe** | n (c) | baby |  | FR bébé |
| **bestefar** | n (c) | grandfather |  | DA bedstefar |
| **bestemor** | n (c) | grandmother |  | DA bedstemor |
| **brekare** | n (c) | breaker |  | breka + -are |
| **broder** | n (c) | brother |  | SV broder + NL broer |
| **bror** | n (c) | bro (informal address) | everyday | SV bror |
| **brud** | n (c) | bride |  | SV brud |
| **brudgom** | n (c) | bridegroom |  | SV brudgum |
| **bryllop** | n (n) | wedding |  | SV bröllop |
| **dame** | n (c) | lady |  | FR dame |
| **dotter** | n (c) | daughter |  | SV dotter + NL dochter |
| **dumhoved** | n (n) | blockhead | everyday | dum + hoved |
| **fader** | n (c) | father |  | SV fader + NL vader |
| **famille** | n (c) | family |  | FR famille |
| **fiende** | n (c) | enemy |  | SV fiende + DA fjende |
| **flika** | n (c) | girl |  | SV flicka |
| **folk** | n (n) | people |  | SV folk + NL volk |
| **forelder** | n (c) | parent |  | SV förälder |
| **fremling** | n (c) | stranger |  | SV främling |
| **fru** | n (c) | madam, wife |  | SV fru |
| **gava** | n (c) | gift |  | SV gåva |
| **gest** | n (c) | guest |  | SV gäst |
| **gifta** | v | marry | everyday | SV gifta |
| **giftskap** | n (c) | marriage |  | gifta + -skap |
| **grundare** | n (c) | founder |  | grunda + -are (§3b agent noun) |
| **grupp** | n (c) | group |  | SV grupp + FR groupe |
| **herre** | n (c) | gentleman, lord |  | SV herre |
| **hjelp** | n (c) | help (n.) | everyday | SV hjälp |
| **husbond** | n (c) | husband |  | hus + bonde |
| **idiot** | n (c) | idiot | everyday | FR idiot |
| **kamerat** | n (c) | comrade, companion |  | SV kamrat + FR camarade |
| **kameratskap** | n (c) | companionship |  | kamerat + -skap |
| **kind** | n (n) | child |  | NL kind |
| **kindtid** | n (c) | childhood |  | kind + tid |
| **kong** | n (c) | king |  | SV kung + DA konge |
| **konsensbrekare** | n (c) | consensus-breaker | everyday | konsens + brekare |
| **kusin** | n (c) | cousin |  | SV kusin + FR cousin |
| **kvin** | n (c) | woman |  | SV kvinna |
| **kyss** | n (c) | kiss (n.) |  | SV kyss + DA kys |
| **kyssa** | v | kiss |  | kyss + -a |
| **Laphurdeener** | n (c) | Laphurdeener (citizen) |  | Laphurdeen + -er |
| **lievare** | n (c) | lover |  | lieva + -are |
| **lievling** | n (c) | beloved, darling |  | NL lieveling |
| **man** | n (c) | man |  | EN man + SV man |
| **mariera** | v | marry | high | FR marier |
| **mennisk** | n (c) | human being |  | SV människa + DA menneske |
| **moder** | n (c) | mother |  | SV moder + NL moeder |
| **nabo** | n (c) | neighbour |  | DA nabo |
| **navn** | n (n) | name |  | DA navn |
| **neve** | n (c) | nephew |  | NL neef |
| **nikt** | n (c) | niece |  | NL nicht |
| **onkel** | n (c) | uncle |  | DA onkel + FR oncle |
| **parskap** | n (c) | relationship, partnership |  | par + -skap |
| **person** | n (c) | person |  | SV person + FR personne |
| **pojk** | n (c) | boy |  | SV pojke |
| **ras** | n (c) | race (of people) |  | NL ras + FR race |
| **samfund** | n (n) | society |  | DA samfund + SV samfund |
| **sjelvrend** | n (c) | soulmate, kindred spirit |  | sjel + vrend |
| **skiljing** | n (c) | separation, parting |  | skilja + -ing |
| **slekt** | n (c) | kin, family line |  | SV släkt |
| **son** | n (c) | son |  | SV son |
| **sosial** | adj | social | high | FR social |
| **spuus** | n (c) | spouse |  | EN spouse + NL echtgenoot |
| **syster** | n (c) | sister |  | SV syster + NL zuster |
| **tante** | n (c) | aunt |  | NL tante + FR tante |
| **ungdom** | n (c) | youth |  | SV ungdom |
| **ursprung** | n (n) | origin |  | SV ursprung |
| **vard** | n (c) | host |  | SV värd |
| **vedve** | n (c) | widow |  | NL weduwe |
| **vrend** | n (c) | friend |  | NL vriend + SV vän + EN friend |
| **vrendskap** | n (c) | friendship |  | vrend + -skap |
| **vuksen** | n (c) | adult |  | SV vuxen + DA voksen |

## food-drink

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **aftenmat** | n (c) | dinner, supper |  | aften + mat |
| **ananas** | n (c) | pineapple |  | FR ananas + SV ananas |
| **apel** | n (c) | apple |  | NL appel + SV äpple |
| **aprikos** | n (c) | apricot |  | SV aprikos + FR abricot |
| **avokado** | n (c) | avocado |  | EN avocado |
| **baka** | v | bake |  | SV baka |
| **bakeri** | n (n) | bakery |  | baka + -eri |
| **banan** | n (c) | banana |  | SV banan + FR banane |
| **basilika** | n (c) | basil |  | SV basilika |
| **ber** | n (n) | berry |  | SV bär + EN berry |
| **beta** | n (c) | beet |  | SV beta |
| **beverasje** | n (c) | drink (n.) | high | FR breuvage + EN beverage |
| **bier** | n (n) | beer |  | NL bier |
| **bitter** | adj | bitter |  | NL bitter + EN bitter |
| **blaaber** | n (n) | blueberry |  | blaa + ber |
| **boef** | n (n) | beef |  | FR bœuf |
| **bonna** | n (c) | bean |  | SV böna + NL boon |
| **brennvin** | n (n) | aquavit |  | brenna + vin |
| **brod** | n (n) | bread |  | SV bröd + NL brood |
| **brokoli** | n (c) | broccoli |  | FR brocoli |
| **deg** | n (c) | dough |  | SV deg |
| **deilig** | adj | delicious |  | DA dejlig |
| **dill** | n (c) | dill |  | SV dill |
| **dinera** | v | eat | high | FR dîner |
| **drika** | v | drink |  | SV dricka + NL drinken |
| **drikk** | n (c) | drink (n.) | everyday | DA drik |
| **druva** | n (c) | grape |  | SV druva |
| **eddik** | n (c) | vinegar |  | SV ättika + NL azijn |
| **ert** | n (c) | pea |  | SV ärt + NL erwt |
| **eta** | v | eat | everyday | SV äta |
| **fersk** | adj | fresh |  | SV färsk + DA fersk |
| **figon** | n (n) | fig |  | SV fikon |
| **frukost** | n (c) | breakfast |  | SV frukost |
| **grapefrukt** | n (c) | grapefruit |  | EN grapefruit + frukt |
| **grilla** | v | grill |  | SV grilla + FR griller |
| **gronsak** | n (c) | vegetable |  | SV grönsak |
| **gryn** | n (n) | cereal, grain |  | SV gryn |
| **gurka** | n (c) | cucumber |  | SV gurka + NL komkommer |
| **hallon** | n (n) | raspberry |  | SV hallon |
| **havre** | n (c) | oat |  | SV havre + NL haver |
| **honing** | n (c) | honey |  | NL honing |
| **hunger** | n (c) | hunger |  | NL honger + EN hunger |
| **ingefer** | n (c) | ginger |  | SV ingefära |
| **iskrem** | n (c) | ice cream |  | is + krem |
| **jordber** | n (n) | strawberry |  | jord + ber |
| **kaal** | n (c) | cabbage |  | SV kål + NL kool |
| **kafe** | n (c) | coffee |  | FR café |
| **kaka** | n (c) | cake |  | SV kaka |
| **kakao** | n (c) | cocoa |  | SV kakao + FR cacao |
| **kanel** | n (c) | cinnamon |  | SV kanel + FR cannelle |
| **karot** | n (c) | carrot |  | FR carotte + EN carrot |
| **keks** | n (c) | biscuit |  | SV kex + EN cakes |
| **kersbeer** | n (c) | cherry |  | SV körsbär + NL kers |
| **koka** | v | cook (v.) |  | SV koka |
| **kokare** | n (c) | cook (n.) | everyday | koka + -are |
| **kokos** | n (c) | coco |  | SV kokos |
| **kokosnoet** | n (c) | coconut |  | kokos + noet |
| **koktail** | n (c) | cocktail |  | EN cocktail |
| **korn** | n (n) | barley, grain |  | SV korn + NL koren |
| **korv** | n (c) | sausage |  | SV korv |
| **kott** | n (n) | meat |  | SV kött |
| **krem** | n (c) | cream |  | FR crème + DA creme |
| **krispig** | adj | crispy |  | SV krispig |
| **krydda** | n (c) | spice |  | SV krydda |
| **kwizin** | n (c) | cuisine | high | FR cuisine |
| **lam** | n (n) | lamb |  | SV lamm + NL lam |
| **lemonad** | n (c) | lemonade |  | FR limonade |
| **limett** | n (c) | lime |  | FR limette |
| **lok** | n (c) | onion |  | SV lök + DA løg |
| **lunsj** | n (c) | lunch |  | EN lunch |
| **maaltid** | n (c) | meal |  | maal + tid |
| **majones** | n (c) | mayonnaise |  | FR mayonnaise |
| **majs** | n (c) | corn, maize |  | SV majs + NL maïs |
| **mango** | n (c) | mango |  | EN mango |
| **mat** | n (c) | food |  | SV mat |
| **mel** | n (n) | flour |  | SV mjöl + DA mel |
| **melk** | n (c) | milk |  | NL melk + DA mælk |
| **melon** | n (c) | melon |  | FR melon + SV melon |
| **meny** | n (c) | menu | high | FR menu |
| **moeton** | n (n) | mutton |  | FR mouton |
| **muskot** | n (c) | nutmeg |  | SV muskot |
| **musla** | n (c) | mussel, shellfish |  | SV mussla + NL mossel |
| **noet** | n (c) | nut |  | NL noot + SV nöt |
| **nudel** | n (c) | noodle |  | SV nudel + NL noedel |
| **oliv** | n (c) | olive |  | FR olive |
| **olja** | n (c) | oil |  | SV olja |
| **ost** | n (c) | cheese |  | SV ost |
| **paprika** | n (c) | bell pepper |  | SV paprika |
| **pasta** | n (c) | pasta |  | SV pasta |
| **peppar** | n (c) | pepper |  | SV peppar |
| **per** | n (c) | pear |  | SV päron + FR poire |
| **persik** | n (c) | peach |  | SV persika + FR pêche |
| **persilja** | n (c) | parsley |  | SV persilja |
| **plomma** | n (c) | plum |  | SV plommon + NL pruim |
| **pork** | n (n) | pork |  | FR porc + EN pork |
| **porsjon** | n (c) | portion |  | FR portion |
| **potet** | n (c) | potato |  | DA kartoffel + EN potato |
| **pumpa** | n (c) | pumpkin |  | SV pumpa |
| **purjolok** | n (c) | leek |  | FR poireau + SV purjolök |
| **raa** | adj | raw |  | SV rå + DA rå |
| **rekja** | n (c) | shrimp |  | SV räka |
| **restoran** | n (c) | restaurant | high | FR restaurant |
| **rettika** | n (c) | radish |  | SV rädisa |
| **rijp** | adj | ripe |  | NL rijp |
| **ris** | n (n) | rice |  | SV ris + NL rijst |
| **rosmarin** | n (c) | rosemary |  | SV rosmarin |
| **rosta** | v | roast |  | SV rosta |
| **safran** | n (c) | saffron |  | FR safran |
| **salat** | n (c) | salad |  | FR salade + SV sallad |
| **sap** | n (n) | juice |  | NL sap |
| **selleri** | n (c) | celery |  | SV selleri |
| **senap** | n (c) | mustard |  | SV senap |
| **servera** | v | serve |  | FR servir |
| **sider** | n (c) | cider |  | FR cidre + EN cider |
| **sitron** | n (c) | lemon |  | SV citron + FR citron |
| **sjampanje** | n (c) | champagne | high | FR champagne |
| **sjef** | n (c) | cook (n.) | high | FR chef |
| **sjili** | n (c) | chili |  | EN chili |
| **sjokolad** | n (c) | chocolate |  | FR chocolat |
| **skinka** | n (c) | ham |  | SV skinka |
| **smak** | n (c) | taste |  | SV smak |
| **smaka** | v | taste (v.) |  | smak + -a |
| **smor** | n (n) | butter |  | SV smör + DA smør |
| **soda** | n (c) | soda |  | EN soda |
| **soppa** | n (c) | soup |  | SV soppa + FR soupe |
| **sos** | n (c) | sauce |  | FR sauce + DA sovs |
| **sot** | adj | sweet |  | SV söt + DA sød |
| **spenat** | n (c) | spinach |  | SV spenat + FR épinard |
| **sprit** | n (c) | liquor, spirits |  | SV sprit |
| **steka** | v | fry |  | SV steka |
| **sukker** | n (n) | sugar |  | DA sukker + NL suiker |
| **sur** | adj | sour |  | SV sur |
| **svamp** | n (c) | mushroom |  | SV svamp |
| **sylt** | n (c) | jam |  | SV sylt |
| **tee** | n (c) | tea |  | NL thee + EN tea |
| **timjan** | n (c) | thyme |  | SV timjan |
| **tomat** | n (c) | tomato |  | SV tomat + FR tomate |
| **torst** | n (c) | thirst |  | SV törst + NL dorst |
| **vanilj** | n (c) | vanilla |  | SV vanilj + FR vanille |
| **vattermelon** | n (c) | watermelon |  | vatter + melon |
| **vete** | n (n) | wheat |  | SV vete + NL tarwe |
| **vin** | n (n) | wine |  | FR vin + SV vin |
| **vitlok** | n (c) | garlic |  | vit + lok |
| **yoghurt** | n (c) | yoghurt |  | SV yoghurt |

## house-home

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **adres** | n (c) | address |  | FR adresse + NL adres |
| **avfall** | n (n) | waste, garbage |  | SV avfall |
| **badrum** | n (n) | bathroom |  | bad + rum |
| **bord** | n (n) | table |  | SV bord |
| **bygging** | n (c) | building, construction |  | bygga + -ing |
| **dor** | n (c) | door |  | SV dörr + NL deur |
| **dusj** | n (c) | shower |  | FR douche |
| **fenster** | n (n) | window |  | SV fönster + FR fenêtre |
| **flaska** | n (c) | bottle |  | SV flaska + NL fles |
| **gaffel** | n (c) | fork |  | SV gaffel |
| **gard** | n (c) | yard, farm |  | SV gård + DA gård |
| **gardin** | n (c) | curtain |  | SV gardin |
| **glas** | n (n) | glass |  | SV glas + NL glas |
| **golv** | n (n) | floor |  | SV golv |
| **gryta** | n (c) | pot |  | SV gryta |
| **hem** | n (n) | home |  | SV hem |
| **hotell** | n (n) | hotel |  | FR hôtel + SV hotell |
| **hus** | n (n) | house | everyday | NL huis + SV hus |
| **hylla** | n (c) | shelf |  | SV hylla |
| **hyra** | v | rent |  | SV hyra |
| **hytta** | n (c) | cottage, hut |  | SV hytta + DA hytte |
| **isskap** | n (n) | refrigerator |  | is + skap |
| **kamer** | n (c) | chamber |  | NL kamer |
| **kirk** | n (c) | church |  | NL kerk + DA kirke + SV kyrka |
| **kista** | n (c) | chest (box) |  | SV kista |
| **kniv** | n (c) | knife |  | SV kniv + NL knijf |
| **kok** | n (n) | kitchen |  | SV kök + DA køkken |
| **kopp** | n (c) | cup |  | SV kopp + EN cup |
| **korg** | n (c) | basket |  | SV korg |
| **kran** | n (c) | tap, faucet |  | SV kran |
| **kudde** | n (c) | pillow, cushion |  | SV kudde |
| **laas** | n (n) | lock |  | SV lås |
| **lada** | n (c) | box |  | SV låda |
| **lampa** | n (c) | lamp |  | SV lampa + FR lampe |
| **matta** | n (c) | rug, mat |  | SV matta |
| **mobel** | n (c) | furniture |  | SV möbel + FR meuble |
| **moske** | n (c) | mosque |  | FR mosquée + SV moské |
| **nabolag** | n (n) | neighbourhood |  | DA nabolag |
| **nokel** | n (c) | key |  | DA nøgle + SV nyckel |
| **pan** | n (c) | pan |  | NL pan + EN pan |
| **pipa** | n (c) | pipe |  | SV pipa |
| **propp** | n (c) | plug, stopper |  | SV propp |
| **residens** | n (c) | house | high | FR résidence |
| **rum** | n (n) | room | everyday | SV rum |
| **sekk** | n (c) | sack, bag |  | SV säck + NL zak |
| **seng** | n (c) | bed |  | DA seng + SV säng |
| **sjamber** | n (c) | room | high | FR chambre |
| **skaal** | n (c) | bowl |  | SV skål |
| **skap** | n (n) | cupboard |  | SV skåp + DA skab |
| **sked** | n (c) | spoon |  | SV sked |
| **sofa** | n (c) | sofa, couch |  | EN sofa + FR sofa |
| **sovrum** | n (n) | bedroom |  | sova + rum |
| **spegel** | n (c) | mirror |  | SV spegel + NL spiegel |
| **stadshus** | n (n) | city hall |  | stad + hus |
| **stol** | n (c) | chair |  | SV stol |
| **synagoge** | n (c) | synagogue |  | FR synagogue + DA synagoge |
| **tak** | n (n) | roof |  | SV tak |
| **tallrik** | n (c) | plate |  | SV tallrik |
| **tempel** | n (n) | temple |  | SV tempel + NL tempel |
| **ting** | n (n) | thing |  | SV ting + DA ting |
| **toalett** | n (c) | toilet |  | FR toilette |
| **torn** | n (n) | tower |  | SV torn + NL toren |
| **trappa** | n (c) | stairs |  | SV trappa + NL trap |
| **ugn** | n (c) | oven |  | SV ugn |
| **vegg** | n (c) | wall |  | SV vägg + DA væg |

## clothing

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **belt** | n (n) | belt |  | EN belt + SV bälte |
| **bomull** | n (c) | cotton |  | SV bomull |
| **broek** | n (c) | trousers |  | NL broek |
| **fikk** | n (c) | pocket |  | SV ficka |
| **handske** | n (c) | glove |  | SV handske |
| **hatt** | n (c) | hat |  | SV hatt + NL hoed |
| **hemd** | n (n) | shirt |  | NL hemd |
| **huv** | n (c) | hood, cowl |  | SV huv |
| **jakk** | n (c) | jacket |  | SV jacka + FR jaquette |
| **kappa** | n (c) | coat |  | SV kappa |
| **kjol** | n (c) | skirt |  | SV kjol |
| **kleda** | v | dress (v.) |  | NL kleden + SV klä |
| **kleder** | n (c) | clothes |  | DA klæder + NL kleren |
| **knapp** | n (c) | button |  | SV knapp |
| **leder** | n (n) | leather |  | NL leder |
| **mode** | n (c) | fashion | high | FR mode |
| **muts** | n (c) | cap |  | NL muts |
| **silke** | n (n) | silk |  | SV silke |
| **skerf** | n (c) | scarf |  | EN scarf + NL sjerp |
| **skoe** | n (c) | shoe |  | NL schoen + EN shoe |
| **sok** | n (c) | sock |  | NL sok + EN sock |
| **stikka** | v | knit |  | SV sticka |
| **stovel** | n (c) | boot |  | SV stövel + NL stevel |
| **sya** | v | sew |  | SV sy + NL naaien |
| **ull** | n (c) | wool |  | SV ull |
| **uniform** | n (c) | uniform |  | FR uniforme |

## time-calendar

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **aften** | n (c) | evening |  | DA aften |
| **april** | n (c) | April |  | SV april |
| **august** | n (c) | August |  | DA august |
| **dag** | n (c) | day |  | NL dag + SV dag |
| **desember** | n (c) | December |  | DA december + FR décembre |
| **evig** | adj | eternal, everlasting |  | SV evig + NL eeuwig |
| **evighed** | n (c) | eternity |  | evig + -hed |
| **februar** | n (c) | February |  | DA februar |
| **fortid** | n (c) | past (n.) |  | for + tid |
| **framtid** | n (c) | future |  | fram + tid |
| **fredag** | n (c) | Friday |  | SV fredag + NL vrijdag |
| **gang** | n (c) | time, occasion |  | DA gang + SV gång |
| **host** | n (c) | autumn |  | SV höst |
| **iaften** | adv | tonight, this evening |  | DA i aften |
| **idag** | adv | today |  | SV idag |
| **igaar** | adv | yesterday |  | SV igår |
| **imorgen** | adv | tomorrow |  | DA i morgen |
| **jaar** | n (n) | year |  | NL jaar |
| **januar** | n (c) | January |  | DA januar + FR janvier |
| **juli** | n (c) | July |  | SV juli |
| **juni** | n (c) | June |  | SV juni |
| **klok** | n (c) | clock |  | NL klok |
| **lente** | n (c) | spring (season) |  | NL lente |
| **lordag** | n (c) | Saturday |  | SV lördag |
| **maj** | n (c) | May |  | SV maj |
| **manad** | n (c) | month |  | SV månad |
| **mandag** | n (c) | Monday |  | SV måndag + NL maandag |
| **mars** | n (c) | March |  | SV mars + FR mars |
| **middag** | n (c) | midday, noon |  | SV middag + NL middag |
| **minut** | n (c) | minute |  | FR minute |
| **moment** | n (n) | moment, instant | high | FR moment |
| **morgen** | n (c) | morning |  | NL morgen |
| **natt** | n (c) | night |  | SV natt + NL nacht |
| **november** | n (c) | November |  | SV november |
| **nutid** | n (c) | present (time) |  | nu + tid |
| **ojeblik** | n (n) | moment, instant | everyday | DA øjeblik + NL ogenblik |
| **oktober** | n (c) | October |  | SV oktober |
| **onsdag** | n (c) | Wednesday |  | SV onsdag + DA onsdag |
| **samtidig** | adj | simultaneous |  | SV samtidig + DA samtidig |
| **sekel** | n (n) | century |  | SV sekel |
| **sekund** | n (c) | second (time unit) |  | SV sekund + FR seconde |
| **september** | n (c) | September |  | SV september |
| **sesong** | n (c) | season | high | FR saison |
| **sommer** | n (c) | summer |  | DA sommer + NL zomer |
| **start** | n (c) | start, beginning |  | SV start + EN start |
| **sundag** | n (c) | Sunday |  | SV söndag + EN Sunday |
| **termin** | n (c) | term (of office) |  | SV termin + FR terme |
| **tid** | n (c) | time |  | SV tid |
| **tisdag** | n (c) | Tuesday |  | SV tisdag |
| **torsdag** | n (c) | Thursday |  | SV torsdag |
| **uur** | n (n) | hour |  | NL uur |
| **vek** | n (c) | week |  | SV vecka + EN week |
| **vinter** | n (c) | winter |  | SV vinter + NL winter |

## numbers-measure

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **akt** | num | eight |  | SV åtta + NL acht |
| **akte** | num | eighth |  | SV åttonde + NL achtste |
| **akten** | num | eighteen |  | NL achttien + SV arton |
| **akti** | num | eighty |  | SV åttio |
| **andre** | num | second |  | SV andra + NL ander |
| **del** | n (c) | part, portion |  | SV del + NL deel |
| **dusin** | n (n) | dozen |  | NL dozijn + EN dozen |
| **elva** | num | eleven |  | SV elva |
| **en** | num | one |  | SV en + NL een |
| **et** | det | a, an (neuter article) |  | SV ett + NL het |
| **fem** | num | five |  | SV fem |
| **femte** | num | fifth |  | SV femte |
| **femten** | num | fifteen |  | DA femten + SV femton |
| **femti** | num | fifty |  | SV femtio |
| **ferste** | num | first |  | EN first + SV första + NL eerste |
| **fjorten** | num | fourteen |  | DA fjorten + SV fjorton |
| **fyr** | num | four |  | SV fyra |
| **fyrde** | num | fourth |  | SV fjärde |
| **fyrti** | num | forty |  | SV fyrtio + EN forty |
| **grad** | n (c) | degree |  | SV grad |
| **gram** | n (n) | gram |  | FR gramme |
| **halv** | adj | half |  | SV halv |
| **hundra** | num | hundred |  | SV hundra |
| **index** | n (c) | index | high | FR index |
| **kilo** | n (n) | kilogram |  | FR kilo |
| **kilometer** | n (c) | kilometre |  | kilo + meter |
| **kwart** | n (n) | quarter |  | NL kwart |
| **liter** | n (c) | litre |  | FR litre |
| **mal** | n (n) | time, occurrence |  | NL maal |
| **median** | adj | median | high | FR médiane |
| **meter** | n (c) | metre |  | FR mètre |
| **million** | num | million |  | FR million |
| **nien** | num | nine |  | SV nio + NL negen + EN nine |
| **niende** | num | ninth |  | DA niende + SV nionde |
| **nienti** | num | ninety |  | EN ninety + SV nittio |
| **nitten** | num | nineteen |  | DA nitten |
| **nivaa** | n (n) | level |  | FR niveau |
| **null** | num | zero |  | SV noll + EN null |
| **nummer** | n (n) | number |  | SV nummer + NL nummer |
| **par** | n (n) | pair |  | SV par |
| **parallell** | adj | parallel |  | FR parallèle |
| **prosent** | n (c) | percent |  | SV procent + FR pour cent |
| **seks** | num | six; sex (the act) |  | SV sex + NL zes + DA seks |
| **sekste** | num | sixth |  | NL zesde + SV sjätte |
| **seksten** | num | sixteen |  | DA seksten |
| **seksti** | num | sixty |  | SV sextio |
| **sida** | n (c) | side |  | SV sida + NL zijde |
| **sju** | num | seven |  | SV sju |
| **sjunde** | num | seventh |  | SV sjunde |
| **sjuti** | num | seventy |  | SV sjuttio |
| **sjutten** | num | seventeen |  | SV sjutton |
| **telling** | n (c) | counting |  | tella + -ing |
| **tien** | num | ten |  | NL tien |
| **tiende** | num | tenth |  | DA tiende + SV tionde |
| **tolv** | num | twelve |  | SV tolv |
| **tretten** | num | thirteen |  | SV tretton + NL dertien |
| **tri** | num | three |  | SV tre + FR trois |
| **tridde** | num | third |  | EN third + SV tredje |
| **triti** | num | thirty |  | EN thirty + NL dertig |
| **tusen** | num | thousand |  | SV tusen + NL duizend |
| **twe** | num | two |  | NL twee + EN two |
| **tweti** | num | twenty |  | EN twenty + NL twintig |

## motion-travel

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **ankomma** | v | arrive |  | SV ankomma + NL aankomen |
| **avresa** | v | depart |  | av + resa |
| **baan** | n (c) | track, way |  | NL baan |
| **besok** | n (n) | visit (n.) |  | SV besök |
| **besoka** | v | visit |  | NL bezoeken + SV besöka |
| **bil** | n (c) | car |  | SV bil + DA bil |
| **biljet** | n (c) | ticket |  | FR billet |
| **bro** | n (c) | bridge |  | SV bro |
| **buss** | n (c) | bus |  | SV buss |
| **fjern** | adj | far |  | DA fjern |
| **flyghamn** | n (c) | airport |  | flyga + hamn |
| **flygplan** | n (n) | airplane |  | SV flygplan |
| **flykt** | n (c) | flight, escape |  | SV flykt + DA flugt |
| **flykta** | v | flee, escape |  | DA flygte |
| **flytta** | v | move (house), relocate |  | SV flytta |
| **gata** | n (c) | street | everyday | SV gata + DA gade |
| **hjul** | n (n) | wheel |  | SV hjul |
| **hoger** | adj | right (direction) |  | SV höger |
| **kindvagn** | n (c) | pram, baby carriage |  | kind + vagn |
| **koffert** | n (c) | suitcase |  | DA kuffert + NL koffer |
| **koffertrum** | n (n) | boot, trunk (of a car) |  | koffert + rum |
| **kora** | v | drive |  | SV köra |
| **langsam** | adj | slow |  | SV långsam + NL langzaam |
| **linje** | n (c) | line (route, stroke) |  | SV linje + NL lijn + FR ligne |
| **motor** | n (c) | motor, engine |  | SV motor + FR moteur |
| **motorhuv** | n (c) | bonnet, hood (of a car) |  | motor + huv |
| **nara** | adj | near |  | SV nära |
| **nerma** | v | approach, near |  | SV närma |
| **parkera** | v | park (v.) |  | FR parquer |
| **pas** | n (n) | passport |  | FR passeport |
| **plas** | n (c) | square, place |  | FR place + NL plein |
| **rak** | adj | straight |  | SV rak |
| **reis** | n (c) | journey |  | NL reis + DA rejse |
| **resa** | v | travel |  | SV resa + DA rejse |
| **resare** | n (c) | traveller |  | resa + -are (§3b agent noun) |
| **retning** | n (c) | direction |  | DA retning |
| **rida** | v | ride |  | SV rida + NL rijden |
| **rue** | n (c) | street | high | FR rue |
| **snabb** | adj | fast |  | SV snabb |
| **snabbhed** | n (c) | speed |  | snabb + -hed |
| **stasjon** | n (c) | station |  | FR station |
| **steg** | n (n) | step |  | SV steg |
| **sti** | n (c) | path |  | DA sti |
| **sykel** | n (c) | bicycle |  | SV cykel |
| **sykla** | v | cycle (v.) |  | sykel + -a |
| **trafik** | n (c) | traffic |  | SV trafik + FR trafic |
| **tram** | n (c) | tram |  | EN tram + NL tram |
| **transport** | n (c) | transport |  | FR transport |
| **tren** | n (c) | train |  | FR train + EN train |
| **tunel** | n (c) | tunnel |  | FR tunnel + EN tunnel |
| **tur** | n (c) | trip, tour |  | SV tur + FR tour |
| **vagn** | n (c) | wagon, carriage |  | SV vagn |
| **venstra** | adj | left |  | DA venstre + SV vänster |

## sea-ships

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **anker** | n (n) | anchor |  | NL anker |
| **boot** | n (c) | boat |  | NL boot |
| **bukt** | n (c) | bay, gulf |  | SV bukt + DA bugt |
| **dekk** | n (n) | deck |  | DA dæk + NL dek |
| **dok** | n (n) | dock |  | NL dok |
| **eb** | n (c) | ebb |  | NL eb |
| **fangst** | n (c) | catch (of fish), haul |  | SV fångst + NL vangst |
| **ferje** | n (c) | ferry |  | DA færge |
| **fiska** | v | fish (v.) |  | fisk + -a |
| **fiskare** | n (c) | fisher |  | fiska + -are |
| **fiskeri** | n (n) | fishery |  | fiska + -eri |
| **fjard** | n (c) | wide bay, firth |  | SV fjärd |
| **floed** | n (c) | flood tide |  | NL vloed |
| **flotta** | n (c) | fleet |  | SV flotta + FR flotte |
| **hamn** | n (c) | harbour |  | SV hamn |
| **hav** | n (n) | ocean, open sea |  | SV hav + DA hav |
| **kaj** | n (c) | quay |  | SV kaj + NL kade |
| **kano** | n (c) | canoe |  | DA kano + SV kanot + FR canoë |
| **kapten** | n (c) | captain |  | SV kapten + FR capitaine |
| **kart** | n (c) | map, chart, card |  | NL kaart + SV karta |
| **koj** | n (c) | bunk |  | SV koj |
| **kompas** | n (n) | compass |  | NL kompas + FR compas |
| **krok** | n (c) | hook |  | SV krok |
| **last** | n (c) | cargo |  | SV last |
| **lystorn** | n (n) | lighthouse |  | lys + torn |
| **mast** | n (c) | mast |  | SV mast |
| **matros** | n (c) | sailor |  | NL matroos |
| **navigasjon** | n (c) | navigation | high | FR navigation |
| **net** | n (n) | net |  | SV nät + NL net |
| **rep** | n (n) | rope |  | SV rep + DA reb |
| **rif** | n (n) | reef |  | NL rif + EN reef |
| **roa** | v | row (v.) |  | SV ro |
| **ror** | n (n) | rudder |  | SV roder |
| **segel** | n (n) | sail (n.) |  | SV segel + NL zeil |
| **segla** | v | sail (v.) |  | segel + -a |
| **sirene** | n (c) | siren |  | FR sirène |
| **skip** | n (n) | ship | everyday | SV skepp + EN ship + DA skib |
| **styra** | v | steer |  | SV styra + NL sturen |
| **tij** | n (n) | tide |  | NL tij |
| **vaag** | n (c) | wave |  | SV våg |
| **vesel** | n (n) | ship | high | FR vaisseau |
| **vik** | n (c) | cove, inlet |  | SV vik |
| **werf** | n (c) | shipyard |  | NL werf |
| **zeeman** | n (c) | seaman |  | zee + man |

## work-trade

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **bank** | n (c) | bank |  | FR banque + SV bank |
| **betala** | v | pay | everyday | SV betala + NL betalen |
| **betaling** | n (c) | payment |  | betala + -ing |
| **billig** | adj | cheap, affordable |  | SV billig + DA billig |
| **bonde** | n (c) | farmer |  | SV bonde |
| **bors** | n (c) | stock exchange | high | FR bourse + SV börs |
| **bruk** | n (n) | use (n.) |  | SV bruk |
| **budsjet** | n (c) | budget | high | FR budget |
| **butik** | n (c) | shop |  | SV butik + FR boutique |
| **bygga** | v | build |  | SV bygga |
| **dollar** | n (c) | dollar |  | EN dollar |
| **egendom** | n (c) | property |  | SV egendom |
| **ekonomi** | n (c) | economy | high | FR économie |
| **ekonomisk** | adj | economic |  | ekonomi + -isk |
| **eksport** | n (c) | export | high | FR exportation + EN export |
| **euro** | n (c) | euro |  | EN euro |
| **fabrik** | n (c) | factory |  | SV fabrik + FR fabrique |
| **finans** | n (c) | finance | high | FR finance |
| **firma** | n (c) | firm, company |  | SV firma |
| **gratis** | adj | free of charge |  | FR gratis + SV gratis |
| **handel** | n (c) | trade (n.) | everyday | SV handel + NL handel |
| **handla** | v | trade, shop (v.) |  | handel + -a |
| **handlare** | n (c) | merchant |  | handla + -are |
| **import** | n (c) | import | high | FR importation + EN import |
| **industri** | n (c) | industry | high | SV industri + FR industrie |
| **inflasjon** | n (c) | inflation | high | FR inflation |
| **inkomst** | n (c) | income |  | SV inkomst |
| **jordbruk** | n (n) | agriculture |  | SV jordbruk |
| **komers** | n (c) | trade (n.) | high | FR commerce |
| **kompensera** | v | pay | high | FR compenser |
| **konferens** | n (c) | meeting | high | SV konferens + FR conférence |
| **konstruera** | v | construct | high | FR construire + SV konstruera |
| **konstruksjon** | n (c) | construction | high | FR construction |
| **kontrakt** | n (n) | contract | high | FR contrat + SV kontrakt |
| **kopa** | v | buy |  | SV köpa + DA købe |
| **kost** | n (c) | cost |  | SV kostnad + NL kosten |
| **kosta** | v | cost (v.) |  | kost + -a |
| **kredit** | n (c) | credit | high | FR crédit |
| **krona** (pl=kronur) | n (c) | krona (currency) |  | SV krona |
| **kwittens** | n (c) | receipt |  | SV kvittens |
| **laan** | n (n) | loan |  | SV lån |
| **laana** | v | borrow, lend |  | SV låna |
| **leda** | v | lead |  | SV leda |
| **ledare** | n (c) | leader |  | leda + -are |
| **ledarskap** | n (c) | leadership |  | ledare + -skap |
| **leverera** | v | deliver |  | SV leverera + FR livrer |
| **lon** | n (c) | wage |  | SV lön + DA løn |
| **marknad** | n (c) | market |  | SV marknad |
| **masjin** | n (c) | machine |  | FR machine |
| **mote** | n (n) | meeting | everyday | SV möte + DA møde |
| **mynt** | n (n) | coin, mint |  | SV mynt |
| **peng** | n (c) | money |  | SV pengar |
| **plan** | n (c) | plan |  | FR plan |
| **planera** | v | plan (v.) |  | SV planera + FR planer |
| **pris** | n (n) | price |  | SV pris + FR prix |
| **produsera** | v | produce |  | FR produire + SV producera |
| **profesjon** | n (c) | profession | high | FR profession |
| **profesjonell** | adj | professional | high | FR professionnel |
| **projekt** | n (n) | project |  | SV projekt + NL project + FR projet |
| **rekning** | n (c) | bill, invoice |  | SV räkning |
| **reserv** | n (c) | reserve | high | FR réserve |
| **revisor** | n (c) | auditor | high | FR réviseur |
| **rikhed** | n (c) | wealth |  | rik + -hed |
| **selja** | v | sell | everyday | SV sälja + DA sælge |
| **seljing** | n (c) | sale, selling |  | selja + -ing |
| **skatt** | n (c) | tax |  | SV skatt |
| **skift** | n (n) | shift (n.) |  | SV skift |
| **skuld** | n (c) | debt |  | SV skuld + NL schuld |
| **smed** | n (c) | smith |  | SV smed |
| **strejk** | n (c) | strike (labour) |  | SV strejk + NL staking |
| **strejka** | v | strike (v.) |  | strejk + -a |
| **strev** | n (n) | effort, striving |  | DA stræb |
| **streva** | v | strive, toil |  | SV sträva + DA stræbe |
| **tjenst** | n (c) | service |  | SV tjänst + DA tjeneste |
| **traad** | n (c) | thread, wire |  | SV tråd + NL draad |
| **travalje** | n (c) | work (n.) | high | FR travail |
| **tuul** | n (n) | tool |  | EN tool |
| **vara** | n (c) | goods, ware |  | SV vara |
| **vendera** | v | sell | high | FR vendre |
| **vinst** | n (c) | profit |  | SV vinst |
| **werk** | n (n) | work (n.) | everyday | NL werk |
| **werka** | v | work |  | NL werken + EN work |
| **werkare** | n (c) | worker |  | werka + -are |
| **werkarunjon** | n (c) | trade union |  | werkare + unjon |

## communication

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **adjuu** | interj | goodbye |  | FR adieu + SV adjö |
| **antonym** | n (c) | antonym |  | FR antonyme + SV antonym |
| **argument** | n (n) | argument |  | FR argument |
| **argumentera** | v | argue |  | FR argumenter |
| **asjeblie** | interj | please |  | NL alsjeblieft |
| **bekenna** | v | confess, admit |  | NL bekennen + DA bekende |
| **bekenning** | n (c) | confession, admission |  | bekenna + -ing |
| **bekrefting** | n (c) | confirmation, validation |  | bekrefta + -ing |
| **beretta** | v | tell, recount, narrate |  | DA berette |
| **beretting** | n (c) | narrative, account, tale |  | beretta + -ing |
| **betyda** | v | mean (v.) |  | SV betyda + DA betyde |
| **bok** | n (c) | book |  | SV bok + NL boek |
| **bokstav** | n (c) | letter (of alphabet) |  | SV bokstav |
| **brev** | n (n) | letter (mail) |  | SV brev + DA brev |
| **dank** | interj | thanks | high | NL dank + SV tack |
| **data** | n (c) | data | high | EN data |
| **demandera** | v | ask | high | FR demander |
| **digital** | adj | digital | high | FR digital + EN digital |
| **diskur** | n (c) | speech | high | FR discours |
| **djevel** | n (c) | devil | everyday | DA djævel + NL duivel |
| **eksplikera** | v | explain | high | FR expliquer |
| **erkenna** | v | acknowledge |  | NL erkennen + DA erkende |
| **falsk** | adj | false |  | SV falsk + NL vals |
| **forbanna** | v | curse, damn | everyday | SV förbanna |
| **forklara** | v | explain | everyday | SV förklara |
| **fraga** | v | ask | everyday | SV fråga |
| **fras** | n (c) | phrase |  | FR phrase + SV fras |
| **gebar** | n (n) | gesture |  | NL gebaar |
| **hallej** | interj | hello |  | EN hello + SV hej |
| **heera** | v | hear |  | EN hear + NL horen |
| **helvet** | n (n) | hell | everyday | SV helvete + DA helvede |
| **informasjon** | n (c) | information | high | FR information |
| **journalist** | n (c) | journalist | high | FR journaliste |
| **kalla** | v | call, name |  | SV kalla + DA kalde |
| **kella** | n (c) | source |  | SV källa |
| **kommunikasjon** | n (c) | communication | high | FR communication |
| **konversasjon** | n (c) | conversation | high | FR conversation |
| **kwestion** | n (c) | question | high | FR question |
| **Laphurdi** | n (c) | the Laphurdi language |  | FR la + SV fjärd |
| **lesa** | v | read |  | SV läsa + DA læse |
| **lesare** | n (c) | reader |  | lesa + -are |
| **lofte** | n (n) | promise (n.) |  | DA løfte |
| **lova** | v | promise, vow |  | SV lova + DA love |
| **lyssna** | v | listen |  | SV lyssna |
| **media** | n (c) | media | high | FR médias |
| **mening** | n (c) | opinion, meaning |  | SV mening |
| **mersi** | interj | thanks | everyday | FR merci |
| **mesasj** | n (c) | message |  | FR message |
| **mista** | v | ghost, vanish on someone (slang) | everyday | mist + -a |
| **nyhed** | n (c) | news |  | ny + -hed |
| **ord** | n (n) | word |  | SV ord + DA ord |
| **oversetta** | v | translate |  | over + setta |
| **oversettare** | n (c) | translator |  | oversetta + -are (§3b agent noun) |
| **papper** | n (n) | paper |  | SV papper + FR papier |
| **penn** | n (c) | pen |  | SV penna + EN pen |
| **pokker** | n (c) | pox (oath) | everyday | DA pokker |
| **post** | n (c) | mail, post |  | SV post + NL post |
| **presse** | n (c) | press (media) | high | FR presse |
| **publisera** | v | publish | high | FR publier |
| **publisering** | n (c) | publication |  | publisera + -ing |
| **radio** | n (c) | radio |  | FR radio + SV radio |
| **respondera** | v | answer | high | FR répondre |
| **respons** | n (c) | answer (n.) | high | FR réponse |
| **ropa** | v | call, shout |  | SV ropa + DA råbe |
| **roping** | n (c) | call, incitement |  | ropa + -ing |
| **rost** | n (c) | voice |  | SV röst |
| **saga** | n (c) | story, tale |  | SV saga + DA sagn |
| **samtal** | n (n) | conversation | everyday | SV samtal + DA samtale |
| **sant** | adj | true |  | SV sann + DA sand |
| **sending** | n (c) | sending |  | senda + -ing |
| **sensur** | n (c) | censorship | high | FR censure |
| **setning** | n (c) | sentence |  | DA sætning |
| **skit** | n (c) | shit (mild oath) | everyday | SV skit |
| **skrift** | n (c) | writing, petition |  | SV skrift |
| **skrika** | v | scream |  | SV skrika |
| **skriva** | v | write |  | SV skriva + NL schrijven |
| **sprak** | n (n) | language |  | SV språk + NL spraak |
| **spreka** | v | speak |  | NL spreken + EN speak |
| **sprekare** | n (c) | speaker |  | spreka + -are |
| **spreking** | n (c) | speech, speaking |  | spreka + -ing |
| **still** | adj | quiet, still |  | EN still + NL stil |
| **stillhed** | n (c) | silence |  | still + -hed |
| **storsegla** | v | flex, show off (slang) | everyday | stor + segla |
| **suggerera** | v | suggest, propose |  | FR suggérer |
| **svar** | n (n) | answer (n.) | everyday | SV svar + DA svar |
| **svara** | v | answer | everyday | SV svara + DA svare |
| **synonym** | n (c) | synonym |  | FR synonyme + SV synonym |
| **tal** | n (n) | speech | everyday | SV tal |
| **teken** | n (n) | sign, symbol (written) |  | NL teken + SV tecken |
| **tekst** | n (c) | text |  | FR texte + SV text |
| **telefon** | n (c) | telephone |  | FR téléphone + SV telefon |
| **televisjon** | n (c) | television |  | FR télévision |
| **tidning** | n (c) | newspaper |  | SV tidning |
| **titel** | n (c) | title |  | SV titel + FR titre |
| **ursekta** | v | excuse, apologize |  | SV ursäkta |
| **utgava** | n (c) | edition |  | SV utgåva |
| **uttrykk** | n (n) | expression |  | ut + trykk |
| **uttrykka** | v | express |  | uttrykk + -a |
| **varsla** | v | warn |  | SV varsla |
| **varsling** | n (c) | warning |  | varsla + -ing |
| **velkom** | interj | welcome |  | SV välkommen + NL welkom |
| **viska** | v | whisper |  | SV viska |

## mind-emotion

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **adorera** | v | love | high | FR adorer |
| **affeksjon** | n (c) | affection |  | FR affection |
| **angra** | v | regret, rue |  | SV ångra |
| **angring** | n (c) | regret, remorse |  | angra + -ing |
| **angst** | n (c) | anxiety, dread |  | NL angst + DA angst |
| **arg** | adj | angry |  | SV arg |
| **arghed** | n (c) | anger |  | arg + -hed |
| **begripa** | v | understand |  | SV begripa + NL begrijpen |
| **begriping** | n (c) | understanding (n.) |  | begripa + -ing |
| **behov** | n (n) | need (n.) |  | SV behov |
| **behova** | v | need (v.) |  | behov + -a (§3b verb from noun) |
| **bekymra** | v | worry, trouble |  | DA bekymre + SV bekymra |
| **bekymring** | n (c) | worry, concern |  | bekymra + -ing |
| **besvika** | v | disappoint |  | SV besvika |
| **besviking** | n (c) | disappointment |  | besvika + -ing |
| **betrakta** | v | contemplate, regard |  | SV betrakta + DA betragte |
| **binding** | n (c) | bond, connection |  | binda + -ing |
| **blijhed** | n (c) | happiness | everyday | blij + -hed |
| **borda** | n (c) | burden |  | SV börda |
| **desperat** | adj | desperate |  | SV desperat + DA desperat |
| **drom** | n (c) | dream |  | SV dröm + DA drøm |
| **dromland** | n (n) | dreamland, imaginary country |  | drom + land |
| **dromma** | v | dream (v.) |  | drom + -a |
| **dum** | adj | stupid |  | SV dum + NL dom |
| **emosjon** | n (c) | emotion |  | FR émotion |
| **emosjonell** | adj | emotional |  | FR émotionnel |
| **empati** | n (c) | empathy |  | FR empathie |
| **empatisk** | adj | empathetic |  | empati + -isk |
| **ensam** | adj | lonely, alone |  | SV ensam |
| **ensamhed** | n (c) | loneliness |  | ensam + -hed |
| **era** | v | honour (v.) |  | SV ära |
| **erlighed** | n (c) | honesty |  | erlig + -hed |
| **evne** | n (c) | ability, capacity |  | DA evne |
| **fantasi** | n (c) | fantasy, imagination |  | SV fantasi + FR fantaisie |
| **felisitet** | n (c) | happiness | high | FR félicité |
| **forlust** | n (c) | loss |  | SV förlust |
| **forsona** | v | reconcile |  | DA forsone + SV försona |
| **forsoning** | n (c) | reconciliation |  | forsona + -ing |
| **fortvivla** | v | despair |  | DA fortvivle + SV förtvivla |
| **fortvivling** | n (c) | despair (n.) |  | fortvivla + -ing |
| **forvirra** | v | confuse, bewilder |  | SV förvirra |
| **forvirring** | n (c) | confusion |  | forvirra + -ing |
| **fravaro** | n (c) | absence |  | SV frånvaro |
| **frykt** | n (c) | fear (n.) |  | DA frygt + SV fruktan |
| **frykta** | v | fear (v.) |  | frykt + -a |
| **gal** | adj | crazy |  | SV galen + DA gal |
| **gleda** | v | gladden, delight |  | glede + -a |
| **glede** | n (c) | joy, gladness |  | DA glæde |
| **glomma** | v | forget |  | SV glömma |
| **graata** | v | weep, cry |  | SV gråta + DA græde |
| **grubbla** | v | brood, ponder |  | SV grubbla + DA gruble |
| **hata** | v | hate |  | SV hata + NL haten |
| **hemskap** | n (c) | belonging |  | hem + -skap |
| **hjertsorg** | n (c) | heartbreak |  | hjerta + sorg |
| **hopa** | v | hope (v.) |  | SV hoppas + EN hope |
| **hopp** | n (n) | hope (n.) |  | SV hopp + DA håb |
| **hungra** | v | hunger, crave |  | hunger + -a |
| **identitet** | n (c) | identity | high | FR identité |
| **igenkenna** | v | recognize |  | igen + kenna |
| **imaginera** | v | imagine |  | FR imaginer |
| **insikt** | n (c) | insight, realization |  | SV insikt |
| **instinkt** | n (c) | instinct |  | FR instinct |
| **intensjon** | n (c) | intention |  | FR intention |
| **interessant** | adj | interesting |  | FR intéressant + NL interessant |
| **intim** | adj | intimate | high | FR intime + SV intim |
| **intimitet** | n (c) | intimacy |  | FR intimité |
| **iver** | n (c) | eagerness, zeal, excitement |  | DA iver + SV iver |
| **karakter** | n (c) | character |  | FR caractère + SV karaktär |
| **kenna** | v | feel |  | SV känna + NL kennen |
| **kenning** | n (c) | feeling |  | kenna + -ing |
| **koela** | v | chill, relax (slang) | everyday | koel + -a |
| **krypig** | adj | cringeworthy (slang) | everyday | krypa + -ig |
| **kurios** | adj | curious | high | FR curieux |
| **kval** | n (n) | anguish, torment |  | SV kval |
| **lakka** | v | laugh |  | NL lachen + DA le |
| **lakking** | n (c) | laughter |  | lakka + -ing |
| **lengta** | v | long, yearn |  | SV längta + DA længes |
| **lengting** | n (c) | longing, yearning |  | lengta + -ing |
| **lida** | v | suffer |  | SV lida |
| **lidare** | n (c) | sufferer |  | lida + -are |
| **liding** | n (c) | suffering |  | lida + -ing |
| **liev** | n (c) | love (n.) |  | NL liefde |
| **lieva** | v | love | everyday | NL liefde + EN love |
| **lita** | v | trust, rely on |  | SV lita |
| **lugn** | adj | calm |  | SV lugn |
| **lykka** | n (c) | luck |  | SV lycka + DA lykke |
| **maal** | n (n) | goal, objective |  | SV mål |
| **meditasjon** | n (c) | meditation |  | FR méditation |
| **meditera** | v | meditate |  | FR méditer |
| **mental** | adj | mental |  | FR mental |
| **minna** | v | remember |  | SV minnas |
| **mod** | n (n) | courage |  | SV mod + DA mod |
| **modig** | adj | brave |  | mod + -ig |
| **motivasjon** | n (c) | motivation |  | FR motivation |
| **motivera** | v | motivate |  | FR motiver |
| **nervaro** | n (c) | presence |  | SV närvaro |
| **nervos** | adj | nervous |  | FR nerveux + SV nervös |
| **oppmerksam** | adj | attentive |  | DA opmærksom |
| **oppmerksamhed** | n (c) | attention |  | oppmerksam + -hed |
| **orientasjon** | n (c) | orientation | high | FR orientation |
| **rasa** | v | rage, rave |  | SV rasa + DA rase |
| **raseri** | n (n) | rage, fury |  | DA raseri |
| **redd** | adj | afraid |  | DA ræd |
| **reflektera** | v | think | high | FR réfléchir |
| **relisjon** | n (c) | religion | high | FR religion |
| **saarbar** | adj | vulnerable |  | SV sårbar |
| **saarbarhed** | n (c) | vulnerability |  | saarbar + -hed |
| **sakna** | v | miss, lack |  | SV sakna |
| **selvbild** | n (c) | self-image |  | self + bild |
| **selvkensla** | n (c) | self-esteem |  | SV självkänsla |
| **sinn** | n (n) | mind |  | DA sind + SV sinne |
| **sjalu** | adj | jealous |  | FR jaloux |
| **sjarm** | n (c) | charm, rizz |  | FR charme |
| **sjel** | n (c) | soul |  | SV själ + DA sjæl |
| **skam** | n (c) | shame |  | SV skam + DA skam |
| **skamma** | v | shame (v.) |  | skam + -a |
| **skammig** | adj | embarrassed |  | skam + -ig |
| **smila** | v | smile |  | SV smila + EN smile |
| **snyfta** | v | sob |  | SV snyfta |
| **sorg** | n (c) | sorrow, grief |  | SV sorg + DA sorg |
| **sorga** | v | mourn, grieve |  | sorg + -a |
| **stimulera** | v | stimulate |  | FR stimuler |
| **stolt** | adj | proud |  | SV stolt + DA stolt |
| **stress** | n (c) | stress |  | EN stress |
| **tanke** | n (c) | thought |  | SV tanke + DA tanke |
| **tenka** | v | think | everyday | SV tänka + DA tænke |
| **tillit** | n (c) | trust, confidence |  | SV tillit + DA tillid |
| **tomhed** | n (c) | emptiness |  | tom + -hed |
| **tragisk** | adj | tragic | high | FR tragique |
| **trakig** | adj | boring |  | SV tråkig |
| **trauma** | n (n) | trauma |  | EN trauma + FR traumatisme |
| **trist** | adj | sad |  | DA trist + FR triste |
| **tro** | n (c) | belief, faith |  | SV tro + DA tro |
| **troa** | v | believe |  | SV tro + DA tro |
| **troest** | n (c) | comfort, consolation |  | NL troost + DA trøst |
| **troesta** | v | comfort, console |  | troest + -a |
| **tvivla** | v | doubt |  | SV tvivla + NL twijfelen |
| **vana** | n (c) | habit |  | SV vana |
| **veljing** | n (c) | choice, choosing |  | velja + -ing |
| **verda** | v | value (v.) |  | verd + -a |
| **veta** (pres=vet, past=viste, perf=vetat) | v | know |  | SV veta + NL weten |
| **vis** | adj | wise |  | SV vis + NL wijs |
| **visjon** | n (c) | vision |  | FR vision |
| **wensa** | v | wish |  | NL wensen |
| **wensing** | n (c) | wish (n.), desire |  | wensa + -ing |
| **wonder** | n (n) | wonder, miracle |  | NL wonder + EN wonder |

## law-civic

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **advokat** | n (c) | lawyer | high | FR avocat + SV advokat |
| **alarm** | n (c) | alarm, alert (n.) |  | SV alarm + NL alarm + FR alarme |
| **ambasadeur** | n (c) | ambassador | high | FR ambassadeur |
| **ansvar** | n (n) | responsibility |  | SV ansvar |
| **arme** | n (c) | army | high | FR armée |
| **artikel** | n (c) | article | high | SV artikel + FR article |
| **autonomi** | n (c) | autonomy | high | FR autonomie |
| **autoritet** | n (c) | authority | high | FR autorité |
| **avtal** | n (n) | agreement, treaty |  | SV avtal |
| **bedrag** | n (n) | fraud |  | DA bedrag + SV bedrägeri |
| **bekrefta** | v | confirm, certify |  | DA bekræfte |
| **bevis** | n (n) | proof, evidence |  | SV bevis |
| **borger** | n (c) | citizen |  | NL burger + DA borger |
| **borgerskap** | n (c) | citizenship |  | borger + -skap |
| **borgmester** | n (c) | mayor |  | DA borgmester |
| **brott** | n (n) | crime | everyday | SV brott |
| **debatt** | n (c) | debate |  | SV debatt + FR débat |
| **deklarera** | v | declare | high | FR déclarer |
| **deklarering** | n (c) | declaration |  | deklarera + -ing |
| **delikt** | n (n) | crime | high | FR délit |
| **demokrati** | n (c) | democracy | high | FR démocratie |
| **demokratisk** | adj | democratic |  | demokrati + -isk |
| **diskriminasjon** | n (c) | discrimination | high | FR discrimination |
| **dodstraff** | n (n) | death penalty |  | dod + straff |
| **dokument** | n (n) | document | high | FR document |
| **dom** | n (c) | judgment |  | SV dom + DA dom |
| **doma** | v | judge (v.) |  | dom + -a |
| **domare** | n (c) | judge (n.) |  | doma + -are |
| **doming** | n (c) | judgment, trial |  | doma + -ing |
| **domstol** | n (c) | court |  | dom + stol |
| **egalitet** | n (c) | equality | high | FR égalité |
| **eksekutiv** | adj | executive | high | FR exécutif |
| **fengsel** | n (n) | prison |  | DA fængsel |
| **flagg** | n (n) | flag |  | SV flagga + DA flag |
| **folkskamer** | n (c) | people's chamber, commons |  | folk + kamer |
| **folkskydd** | n (n) | civil protection |  | folk + skydd |
| **folkstemming** | n (c) | referendum |  | folk + stemming |
| **folktelling** | n (c) | census |  | folk + telling |
| **forbida** | v | forbid | everyday | NL verbieden + SV förbjuda |
| **forbiding** | n (c) | prohibition |  | forbida + -ing |
| **forslag** | n (n) | proposal |  | SV förslag |
| **forsvar** | n (n) | defence |  | SV försvar |
| **fred** | n (c) | peace |  | SV fred + DA fred |
| **frihed** | n (c) | freedom |  | fri + -hed |
| **funksjon** | n (c) | function | high | FR fonction |
| **garanti** | n (c) | guarantee | high | FR garantie |
| **grens** | n (c) | border |  | SV gräns + NL grens |
| **grensa** | v | limit (v.) |  | grens + -a |
| **grunda** | v | found, establish |  | grund + -a |
| **grundloj** | n (c) | constitution |  | grund + loj |
| **gyldig** | adj | valid |  | DA gyldig + SV giltig |
| **handling** | n (c) | act, action |  | handla + -ing |
| **helsaministerie** | n (n) | health ministry |  | helsa + ministerie |
| **hovedstad** | n (c) | capital city |  | hoved + stad |
| **institusjon** | n (c) | institution | high | FR institution |
| **integritet** | n (c) | integrity | high | FR intégrité |
| **invasjon** | n (c) | invasion | high | FR invasion |
| **justis** | n (c) | justice | high | FR justice |
| **kampanj** | n (c) | campaign | high | FR campagne |
| **kandidat** | n (c) | candidate | high | FR candidat |
| **klaga** | v | complain |  | SV klaga |
| **kollegie** | n (n) | college | high | FR collège |
| **kollektiv** | adj | collective | high | FR collectif |
| **komite** | n (c) | committee | high | FR comité |
| **kommisjon** | n (c) | commission | high | FR commission |
| **kommun** | n (c) | municipality |  | SV kommun + FR commune |
| **konsens** | n (c) | consensus |  | EN consensus + FR consensus |
| **konvenor** | n (c) | convenor | high | EN convenor + FR convener |
| **korrupsjon** | n (c) | corruption | high | FR corruption |
| **krav** | n (n) | demand, claim |  | SV krav |
| **krava** | v | demand |  | SV kräva |
| **krig** | n (n) | war |  | SV krig + NL krijg |
| **kris** | n (c) | crisis | high | SV kris + FR crise |
| **kvalifikasjon** | n (c) | qualification | high | FR qualification |
| **kvorum** | n (n) | quorum | high | FR quorum |
| **ledminister** | n (c) | lead minister |  | leda + minister |
| **lobbying** | n (c) | lobbying | high | EN lobbying |
| **loj** | n (c) | law |  | FR loi |
| **lojforslag** | n (n) | bill (draft law) |  | loj + forslag |
| **lojlig** | adj | lawful |  | SV laglig |
| **lokal** | adj | local | high | FR local |
| **majoritet** | n (c) | majority | high | FR majorité |
| **makt** | n (c) | power |  | SV makt + NL macht |
| **medlem** | n (c) | member |  | SV medlem |
| **minister** | n (c) | minister | high | FR ministre + EN minister |
| **ministerie** | n (n) | ministry | high | FR ministère + NL ministerie |
| **minoritet** | n (c) | minority | high | FR minorité |
| **misbruk** | n (n) | abuse, misuse |  | SV missbruk |
| **mord** | n (n) | murder |  | SV mord + DA mord |
| **morda** | v | murder (v.) |  | mord + -a |
| **motto** | n (n) | motto |  | SV motto + EN motto |
| **nasjon** | n (c) | nation | high | FR nation |
| **nasjonal** | adj | national | high | FR national |
| **nod** | n (c) | emergency, distress |  | SV nöd + DA nød |
| **nodtid** | n (c) | state of emergency |  | nod + tid |
| **offentlig** | adj | public |  | SV offentlig + DA offentlig |
| **offisiell** | adj | official | high | FR officiel |
| **ogyldig** | adj | invalid, void |  | o- + gyldig |
| **oskyldig** | adj | innocent |  | o- + skyldig |
| **overgang** | n (c) | transition |  | SV övergång |
| **overvaka** | v | surveil |  | SV övervaka |
| **overvaking** | n (c) | surveillance |  | overvaka + -ing |
| **parlament** | n (n) | parliament | high | FR parlement |
| **parlamentisk** | adj | parliamentary |  | parlament + -isk |
| **parti** | n (n) | party (political) |  | SV parti + FR parti |
| **permitera** | v | allow | high | FR permettre |
| **plikt** | n (c) | duty |  | SV plikt + NL plicht |
| **politi** | n (n) | police |  | DA politi |
| **politisk** | adj | political |  | FR politique + SV politisk |
| **preambel** | n (c) | preamble | high | FR préambule |
| **president** | n (c) | president | high | FR président |
| **privat** | adj | private | high | FR privé |
| **proibera** | v | forbid | high | FR prohiber |
| **proporsjonal** | adj | proportionate | high | FR proportionnel |
| **proving** | n (c) | review, testing |  | prova + -ing |
| **provins** | n (c) | province |  | SV provins + FR province |
| **punisjon** | n (c) | punishment | high | FR punition |
| **raad** | n (n) | council |  | SV råd + DA råd |
| **rangera** | v | rank (v.) | high | FR ranger |
| **rangering** | n (c) | ranking, ranked choice |  | rangera + -ing |
| **rapport** | n (c) | report | high | FR rapport |
| **redding** | n (c) | rescue, salvation |  | redda + -ing |
| **regel** | n (c) | rule |  | SV regel + NL regel |
| **regera** | v | rule, govern | high | FR régner + SV regera |
| **region** | n (c) | region |  | SV region + FR région + DA region |
| **register** | n (n) | register | high | FR registre |
| **rekt** | n (c) | right (entitlement) |  | SV rätt + NL recht |
| **representera** | v | represent |  | FR représenter |
| **reprimera** | v | repress | high | FR réprimer |
| **rettvis** | adj | just, fair |  | SV rättvis |
| **revolusjon** | n (c) | revolution | high | FR révolution |
| **sak** | n (c) | matter, case, affair |  | SV sak + NL zaak |
| **samling** | n (c) | assembly, gathering, collection |  | SV samling |
| **samvelde** | n (n) | commonwealth |  | SV samvälde |
| **sekerhed** | n (c) | security |  | seker + -hed |
| **senat** | n (c) | senate | high | FR sénat + EN senate |
| **senator** | n (c) | senator | high | FR sénateur |
| **seremoniell** | adj | ceremonial | high | FR cérémoniel |
| **sete** | n (n) | seat (office) |  | SV säte + DA sæde |
| **sivil** | adj | civil | high | FR civil |
| **sjarta** | n (c) | charter | high | FR charte |
| **skilsmisse** | n (c) | divorce |  | DA skilsmisse |
| **skott** | n (n) | shot |  | SV skott + DA skud |
| **skydd** | n (n) | protection |  | SV skydd |
| **skydda** | v | protect |  | SV skydda |
| **skyldig** | adj | guilty |  | SV skyldig + NL schuldig |
| **soldat** | n (c) | soldier | high | FR soldat |
| **stand** | n (n) | status, standing |  | SV stånd |
| **stat** | n (c) | state |  | SV stat + FR état |
| **statshoved** | n (n) | head of state |  | stat + hoved |
| **statstjenst** | n (c) | civil service |  | stat + tjenst |
| **stem** | n (c) | vote (n.) |  | NL stem |
| **stemma** | v | vote | everyday | stem + -a |
| **stemmare** | n (c) | voter, elector |  | stemma + -are |
| **stemming** | n (c) | voting |  | stemma + -ing |
| **straff** | n (n) | punishment | everyday | SV straff |
| **straffa** | v | punish |  | straff + -a |
| **styring** | n (c) | governance, government |  | styra + -ing |
| **suveren** | adj | sovereign | high | FR souverain |
| **suverenitet** | n (c) | sovereignty | high | FR souveraineté |
| **talman** | n (c) | speaker (presiding officer) |  | tal + man |
| **territorie** | n (n) | territory | high | FR territoire |
| **tilata** | v | allow | everyday | SV tillåta |
| **tortur** | n (c) | torture | high | FR torture |
| **trua** | v | threaten |  | SV trua + DA true |
| **trussel** | n (c) | threat |  | DA trussel |
| **undertrykka** | v | oppress, suppress |  | SV undertrycka + DA undertrykke + NL onderdrukken |
| **unjon** | n (c) | union | high | FR union |
| **utland** | n (n) | foreign lands, abroad |  | ut + land |
| **utnemna** | v | appoint |  | SV utnämna |
| **utnemning** | n (c) | appointment |  | utnemna + -ing |
| **valg** | n (n) | election |  | DA valg |
| **varslare** | n (c) | whistleblower |  | varsla + -are |
| **veksling** | n (c) | amendment, change |  | veksla + -ing |
| **velvard** | n (c) | welfare |  | SV välfärd + NL welzijn + EN welfare |
| **verdighed** | n (c) | dignity |  | verdig + -hed |
| **veto** | n (n) | veto | high | FR veto |
| **vitne** | n (n) | witness |  | DA vidne |
| **vold** | n (c) | violence |  | DA vold |
| **votera** | v | vote | high | FR voter |

## arts-leisure

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **artist** | n (c) | artist | high | FR artiste |
| **bild** | n (c) | picture |  | SV bild + NL beeld |
| **bildsprak** | n (n) | imagery |  | bild + sprak |
| **boll** | n (c) | ball |  | SV boll |
| **dans** | n (c) | dance (n.) |  | SV dans + FR danse |
| **dansa** | v | dance (v.) |  | dans + -a |
| **dikt** | n (c) | poem |  | SV dikt |
| **dikta** | v | compose poetry |  | dikt + -a |
| **diktare** | n (c) | poet |  | dikta + -are |
| **drama** | n (n) | drama |  | SV drama + FR drame |
| **dramatisk** | adj | dramatic |  | drama + -isk |
| **eldwerk** | n (n) | fireworks |  | eld + werk |
| **ferie** | n (c) | holiday |  | DA ferie + FR férié |
| **fest** | n (c) | party, feast |  | SV fest + FR fête |
| **fiksjon** | n (c) | fiction |  | FR fiction |
| **fiktiv** | adj | fictional, fictitious |  | FR fictif + SV fiktiv |
| **film** | n (c) | film |  | SV film + EN film |
| **fiol** | n (c) | violin |  | SV fiol |
| **forfattare** | n (c) | author, writer |  | SV författare + DA forfatter |
| **fotboll** | n (c) | football |  | fot + boll |
| **foto** | n (n) | photograph |  | SV foto + FR photo |
| **galleri** | n (n) | gallery | high | FR galerie |
| **humor** | n (c) | humor |  | FR humour + SV humor |
| **instrument** | n (n) | instrument | high | FR instrument |
| **kasino** | n (n) | casino | high | FR casino + SV kasino |
| **konsert** | n (c) | concert |  | FR concert + SV konsert |
| **konst** | n (c) | art |  | SV konst |
| **kultur** | n (c) | culture | high | FR culture |
| **leka** | v | play (as a child) |  | SV leka |
| **mala** | v | paint (v.) |  | SV måla |
| **malare** | n (c) | painter |  | mala + -are |
| **maling** | n (c) | painting |  | mala + -ing |
| **mask** | n (c) | mask |  | FR masque |
| **melodi** | n (c) | melody |  | SV melodi + FR mélodie |
| **museum** | n (n) | museum | high | FR musée + SV museum |
| **musik** | n (c) | music |  | SV musik + FR musique |
| **piano** | n (n) | piano |  | FR piano + SV piano |
| **pjes** | n (c) | play (theatre) | high | FR pièce |
| **portrett** | n (n) | portrait |  | FR portrait |
| **rim** | n (n) | rhyme |  | SV rim + DA rim |
| **rima** | v | rhyme (v.) |  | rim + -a |
| **roman** | n (c) | novel | high | FR roman |
| **romans** | n (c) | romance |  | FR romance |
| **romantisk** | adj | romantic |  | SV romantisk + DA romantisk |
| **rytme** | n (c) | rhythm |  | FR rythme |
| **sang** | n (c) | song | everyday | SV sång + NL zang + EN song |
| **singa** | v | sing |  | EN sing + NL zingen |
| **sjanson** | n (c) | song | high | FR chanson |
| **spel** | n (n) | game |  | SV spel + NL spel |
| **spela** | v | play (a game, music) |  | spel + -a |
| **spelhus** | n (n) | casino | everyday | spel + hus |
| **sport** | n (c) | sport |  | EN sport + FR sport |
| **strofe** | n (c) | stanza | high | SV strof + FR strophe |
| **symbol** | n (n) | symbol | high | FR symbole |
| **teater** | n (c) | theatre | high | FR théâtre + SV teater |
| **ton** | n (c) | tone |  | SV ton + FR ton |
| **tragedi** | n (c) | tragedy |  | FR tragédie |
| **tromma** | n (c) | drum |  | SV trumma + DA tromme |
| **vers** | n (c) | verse |  | SV vers + FR vers |

## school-knowledge

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **abstrakt** | adj | abstract | high | FR abstrait |
| **analys** | n (c) | analysis | high | SV analys + FR analyse |
| **bibliotek** | n (n) | library | high | FR bibliothèque + SV bibliotek |
| **biologi** | n (c) | biology | high | FR biologie |
| **eksamen** | n (c) | examination | high | SV examen + FR examen |
| **eksistens** | n (c) | existence |  | FR existence |
| **eksistensiell** | adj | existential |  | FR existentiel |
| **eksistera** | v | exist |  | FR exister |
| **eksperiment** | n (n) | experiment | high | FR expérience + SV experiment |
| **elev** | n (c) | pupil | high | FR élève |
| **fakt** | n (n) | fact |  | SV faktum + EN fact |
| **filosofi** | n (c) | philosophy | high | FR philosophie |
| **forska** | v | research (v.) |  | SV forska |
| **forskare** | n (c) | researcher |  | forska + -are |
| **forsking** | n (c) | research (n.) |  | forska + -ing |
| **fundamentell** | adj | fundamental |  | FR fondamental |
| **fysik** | n (c) | physics | high | FR physique |
| **geografi** | n (c) | geography | high | FR géographie |
| **historie** | n (c) | history |  | FR histoire + NL historie |
| **historisk** | adj | historical |  | historie + -isk |
| **ideal** | n (n) | ideal |  | FR idéal |
| **idealisme** | n (c) | idealism |  | FR idéalisme |
| **idee** | n (c) | idea |  | NL idee + FR idée |
| **instruera** | v | teach | high | FR instruire |
| **ironi** | n (c) | irony | high | FR ironie |
| **kemi** | n (c) | chemistry | high | SV kemi + FR chimie |
| **klass** | n (c) | class |  | SV klass + FR classe |
| **konklusjon** | n (c) | conclusion | high | FR conclusion |
| **kontekst** | n (c) | context |  | FR contexte |
| **kontrast** | n (c) | contrast |  | FR contraste |
| **kurs** | n (c) | course |  | SV kurs + FR cours |
| **laboratorie** | n (n) | laboratory | high | FR laboratoire |
| **leksjon** | n (c) | lesson | high | FR leçon |
| **lera** | v | teach | everyday | SV lära + DA lære |
| **lerare** | n (c) | teacher |  | lera + -are |
| **lering** | n (c) | education, teaching |  | lera + -ing |
| **lerna** | v | learn |  | EN learn + NL leren + SV lära sig |
| **matematik** | n (c) | mathematics | high | FR mathématiques + SV matematik |
| **metafor** | n (c) | metaphor | high | FR métaphore |
| **metaforisk** | adj | metaphorical |  | metafor + -isk |
| **oppdaga** | v | discover |  | SV upptäcka + DA opdage |
| **oppdaging** | n (c) | discovery |  | oppdaga + -ing |
| **paradoks** | n (n) | paradox | high | FR paradoxe |
| **patroon** | n (n) | pattern |  | NL patroon |
| **perspektiv** | n (n) | perspective |  | FR perspective |
| **professor** | n (c) | professor | high | FR professeur + SV professor |
| **prov** | n (n) | test |  | SV prov |
| **psykologisk** | adj | psychological |  | psykologi + -isk |
| **realitet** | n (c) | reality | high | FR réalité |
| **repetera** | v | repeat |  | FR répéter |
| **repetisjon** | n (c) | repetition |  | FR répétition |
| **santhed** | n (c) | truth |  | sant + -hed |
| **siens** | n (c) | science | high | FR science |
| **skol** | n (c) | school |  | SV skola |
| **struktur** | n (c) | structure |  | FR structure |
| **student** | n (c) | student |  | SV student + FR étudiant |
| **studera** | v | study |  | SV studera + FR étudier |
| **tema** | n (n) | theme | high | SV tema + FR thème |
| **teori** | n (c) | theory | high | SV teori + FR théorie |
| **tolka** | v | interpret |  | SV tolka |
| **tolkning** | n (c) | interpretation |  | tolka + -ing |
| **universitet** | n (n) | university | high | SV universitet + FR université |
| **verifiera** | v | verify |  | FR vérifier + SV verifiera |
| **verifikasjon** | n (c) | verification |  | FR vérification |
| **vetskap** | n (c) | knowledge |  | veta + -skap |
| **visdom** | n (c) | wisdom |  | SV visdom + DA visdom |

## science

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **akselerasjon** | n (c) | acceleration |  | FR accélération |
| **aksiom** | n (n) | axiom |  | FR axiome |
| **akustik** | n (c) | acoustics |  | FR acoustique |
| **algebra** | n (c) | algebra |  | FR algèbre |
| **anatomi** | n (c) | anatomy |  | FR anatomie |
| **anga** | n (c) | steam |  | SV ånga |
| **art** | n (c) | species |  | SV art + NL aard |
| **arva** | v | inherit |  | SV ärva |
| **atmosfer** | n (c) | atmosphere |  | FR atmosphère |
| **atom** | n (c) | atom |  | FR atome + SV atom |
| **barometer** | n (c) | barometer |  | FR baromètre |
| **bas** | n (c) | base (chemistry) |  | FR base + SV bas |
| **bevisa** | v | prove |  | bevis + -a |
| **bolge** | n (c) | wave (physics) |  | DA bølge + NL golf |
| **brenning** | n (c) | burning |  | brenna + -ing |
| **brok** | n (n) | fraction |  | SV bråk |
| **cell** | n (c) | cell |  | SV cell + FR cellule |
| **densitet** | n (c) | density |  | FR densité |
| **destillera** | v | distil |  | FR distiller |
| **differens** | n (c) | difference (math) |  | FR différence |
| **dimensjon** | n (c) | dimension |  | FR dimension |
| **ekologi** | n (c) | ecology |  | FR écologie |
| **ekosystem** | n (n) | ecosystem |  | SV ekosystem |
| **ekwasjon** | n (c) | equation |  | FR équation |
| **elektrisitet** | n (c) | electricity |  | FR électricité |
| **elektrisk** | adj | electric |  | FR électrique |
| **elektron** | n (n) | electron |  | FR électron |
| **element** | n (n) | element |  | SV element + FR élément |
| **empirisk** | adj | empirical |  | FR empirique |
| **enhed** | n (c) | unit |  | en + -hed |
| **entropi** | n (c) | entropy |  | FR entropie |
| **enzym** | n (n) | enzyme |  | SV enzym |
| **evolusjon** | n (c) | evolution |  | FR évolution |
| **faktor** | n (c) | factor |  | FR facteur + SV faktor |
| **foranga** | v | evaporate |  | for- + anga |
| **forbrenning** | n (c) | combustion |  | for- + brenning |
| **forening** | n (c) | compound (chemical) |  | SV förening |
| **formel** | n (c) | formula |  | SV formel + FR formule |
| **fossil** | n (n) | fossil |  | FR fossile |
| **foton** | n (n) | photon |  | FR photon |
| **fotosyntes** | n (c) | photosynthesis |  | FR photosynthèse |
| **frekvens** | n (c) | frequency |  | FR fréquence |
| **friksjon** | n (c) | friction |  | FR friction |
| **fysiologi** | n (c) | physiology |  | FR physiologie |
| **fysisk** | adj | physical |  | FR physique |
| **galakse** | n (c) | galaxy |  | FR galaxie |
| **gas** | n (c) | gas |  | SV gas + NL gas |
| **gen** | n (n) | gene |  | SV gen + EN gene |
| **genetik** | n (c) | genetics |  | FR génétique |
| **genomsnitt** | n (n) | average |  | SV genomsnitt |
| **geometri** | n (c) | geometry |  | FR géométrie |
| **glasiar** | n (c) | glacier |  | FR glacier + SV glaciär |
| **graf** | n (c) | graph |  | FR graphe |
| **gravitasjon** | n (c) | gravity |  | FR gravitation |
| **hastighed** | n (c) | velocity |  | SV hastighet |
| **hypotese** | n (c) | hypothesis |  | FR hypothèse |
| **immun** | adj | immune |  | FR immun + SV immun |
| **isotop** | n (c) | isotope |  | FR isotope |
| **jern** | n (n) | iron |  | SV järn + NL ijzer |
| **jon** | n (c) | ion |  | SV jon + FR ion |
| **jordskelv** | n (n) | earthquake |  | jord + skelv |
| **kaos** | n (n) | chaos |  | FR chaos + SV kaos |
| **karbon** | n (n) | carbon |  | FR carbone |
| **katalysator** | n (c) | catalyst |  | FR catalyseur |
| **kemikalie** | n (c) | chemical (substance) |  | SV kemikalie + DA kemikalie |
| **kemisk** | adj | chemical |  | kemi + -isk |
| **kinetisk** | adj | kinetic |  | FR cinétique |
| **klimat** | n (n) | climate |  | FR climat + SV klimat |
| **komet** | n (c) | comet |  | FR comète + SV komet |
| **kondensera** | v | condense |  | FR condenser |
| **konstant** | n (c) | constant |  | FR constante |
| **koordinat** | n (c) | coordinate |  | FR coordonnée |
| **koppar** | n (c) | copper |  | SV koppar + NL koper |
| **korelasjon** | n (c) | correlation |  | FR corrélation |
| **kraftfelt** | n (n) | force field |  | kraft + felt |
| **kristall** | n (c) | crystal |  | SV kristall + FR cristal |
| **kromosom** | n (n) | chromosome |  | FR chromosome |
| **kub** | n (c) | cube |  | SV kub + FR cube |
| **kwadrat** | n (c) | square (shape) |  | SV kvadrat + NL kwadraat |
| **kwantum** | n (n) | quantum | high | FR quantum |
| **kwark** | n (c) | quark |  | EN quark |
| **kwave** | n (n) | nitrogen |  | SV kväve |
| **kwot** | n (c) | quotient | high | FR quotient |
| **lag** | n (c) | law (of nature) |  | SV lag |
| **lava** | n (c) | lava |  | FR lave + SV lava |
| **lins** | n (c) | lens |  | SV lins + NL lens |
| **logaritme** | n (c) | logarithm |  | FR logarithme |
| **lopp** | n (n) | run, course |  | SV lopp |
| **losa** | v | dissolve, solve |  | SV lösa |
| **losning** | n (c) | solution |  | losa + -ning |
| **maat** | n (c) | measure (n.) |  | NL maat + SV mått |
| **magnet** | n (c) | magnet |  | SV magnet |
| **magnetisk** | adj | magnetic |  | magnet + -isk |
| **magnetisme** | n (c) | magnetism |  | FR magnétisme |
| **massa** | n (c) | mass |  | SV massa + FR masse |
| **mekanik** | n (c) | mechanics |  | FR mécanique |
| **membran** | n (n) | membrane |  | FR membrane |
| **meta** | v | measure (v.) |  | SV mäta |
| **metall** | n (c) | metal |  | SV metall + FR métal |
| **meting** | n (c) | measurement |  | meta + -ing |
| **metod** | n (c) | method |  | FR méthode + SV metod |
| **mikrob** | n (c) | microbe |  | FR microbe |
| **mikroskop** | n (n) | microscope |  | FR microscope |
| **mineral** | n (n) | mineral |  | FR minéral + SV mineral |
| **modell** | n (c) | model |  | FR modèle + SV modell |
| **molekyl** | n (c) | molecule |  | FR molécule + SV molekyl |
| **naturlig** | adj | natural |  | natur + -ig |
| **neutron** | n (n) | neutron |  | FR neutron |
| **observasjon** | n (c) | observation |  | FR observation |
| **observera** | v | observe |  | FR observer |
| **oksygen** | n (n) | oxygen |  | FR oxygène |
| **omlopp** | n (n) | orbit |  | om + lopp |
| **optik** | n (c) | optics |  | FR optique |
| **organ** | n (n) | organ (biological) |  | SV organ + FR organe |
| **organism** | n (c) | organism |  | FR organisme |
| **oval** | n (c) | oval |  | SV oval + FR ovale + EN oval |
| **ozon** | n (n) | ozone |  | FR ozone |
| **partikel** | n (c) | particle |  | SV partikel + FR particule |
| **planet** | n (c) | planet |  | FR planète + SV planet |
| **plasma** | n (n) | plasma |  | EN plasma |
| **potensiell** | adj | potential |  | FR potentiel |
| **produkt** | n (c) | product |  | SV produkt + FR produit |
| **prosess** | n (c) | process |  | FR processus |
| **protein** | n (n) | protein |  | SV protein |
| **proton** | n (n) | proton |  | FR proton |
| **punkt** | n (c) | point |  | SV punkt + NL punt |
| **rakett** | n (c) | rocket |  | SV raket + FR roquette |
| **reaksjon** | n (c) | reaction |  | FR réaction |
| **rekna** | v | calculate |  | SV räkna |
| **rektangel** | n (c) | rectangle |  | SV rektangel + FR rectangle |
| **relativitet** | n (c) | relativity |  | FR relativité |
| **resultat** | n (n) | result |  | FR résultat |
| **rorelse** | n (c) | motion |  | SV rörelse |
| **rymd** | n (c) | space (open, outer) |  | SV rymd |
| **sannolik** | adj | probable |  | SV sannolik |
| **sannolikhed** | n (c) | probability |  | sannolik + -hed |
| **satellit** | n (c) | satellite |  | FR satellite |
| **sfer** | n (c) | sphere |  | FR sphère + SV sfär |
| **siffra** | n (c) | digit |  | SV siffra |
| **silver** | n (n) | silver |  | SV silver + NL zilver |
| **sirkel** | n (c) | circle |  | SV cirkel + FR cercle |
| **skelv** | n (n) | tremor |  | skelva |
| **skelva** | v | tremble |  | SV skälva |
| **smelta** | v | melt |  | SV smälta |
| **solsystem** | n (n) | solar system |  | sol + system |
| **spektrum** | n (n) | spectrum |  | SV spektrum + FR spectre |
| **sterbild** | n (c) | constellation |  | ster + bild |
| **straala** | v | radiate |  | SV stråla |
| **straaling** | n (c) | radiation |  | straala + -ing |
| **strom** | n (c) | current, stream |  | SV ström + NL stroom |
| **substans** | n (c) | substance |  | FR substance |
| **summa** | n (c) | sum |  | SV summa + FR somme |
| **symmetri** | n (c) | symmetry |  | FR symétrie |
| **syra** | n (c) | acid |  | SV syra |
| **system** | n (n) | system |  | SV system + FR système |
| **teleskop** | n (n) | telescope |  | FR télescope |
| **temperatur** | n (c) | temperature |  | FR température |
| **teorem** | n (n) | theorem |  | FR théorème |
| **termisk** | adj | thermal |  | FR thermique |
| **termodynamik** | n (c) | thermodynamics |  | FR thermodynamique |
| **termometer** | n (c) | thermometer |  | FR thermomètre |
| **transformasjon** | n (c) | transformation |  | FR transformation |
| **transformera** | v | transform |  | FR transformer |
| **triangel** | n (c) | triangle |  | SV triangel + FR triangle |
| **univers** | n (n) | universe |  | FR univers |
| **vakuum** | n (n) | vacuum |  | SV vakuum |
| **variabel** | n (c) | variable |  | FR variable + SV variabel |
| **vate** | n (n) | hydrogen |  | SV väte |
| **vekst** | n (c) | growth |  | SV växt |
| **vekt** | n (c) | weight |  | SV vikt + NL gewicht |
| **vetska** | n (c) | liquid |  | SV vätska |
| **vev** | n (n) | tissue |  | SV väv |
| **vinkel** | n (c) | angle |  | SV vinkel + NL hoek |
| **volym** | n (c) | volume |  | SV volym + FR volume |
| **yta** | n (c) | surface, area |  | SV yta |

## digital

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **algoritme** | n (c) | algorithm |  | FR algorithme |
| **anvendare** | n (c) | user |  | SV användare |
| **app** | n (c) | app |  | EN app |
| **automat** | n (c) | automaton |  | FR automate |
| **automatisk** | adj | automatic |  | automat + -isk |
| **avatar** | n (c) | avatar |  | EN avatar |
| **bit** | n (c) | bit |  | EN bit |
| **blogg** | n (c) | blog |  | EN blog |
| **blogga** | v | blog (v.) |  | blogg + -a |
| **brannmur** | n (c) | firewall |  | SV brandmur |
| **bugg** | n (c) | bug (software) |  | EN bug |
| **byte** | n (c) | byte |  | EN byte |
| **chatt** | n (c) | chat (n.) |  | EN chat |
| **chatta** | v | chat (v.) |  | EN chat |
| **databas** | n (c) | database |  | data + bas |
| **dator** | n (c) | computer |  | SV dator |
| **digitalisera** | v | digitise |  | FR digitaliser |
| **dron** | n (c) | drone |  | EN drone |
| **epost** | n (c) | email |  | EN e + SV post |
| **fastlegga** | v | commit (changes), record |  | fast + legga |
| **fastlegging** | n (c) | commit (n.), recorded change |  | fastlegga + -ing |
| **feil** | n (n) | error, fault |  | DA fejl + NL feil |
| **fil** | n (c) | file |  | SV fil + FR file |
| **fjernkontroll** | n (c) | remote control |  | SV fjärrkontroll |
| **hogtalare** | n (c) | speaker (audio) |  | SV högtalare |
| **horlur** | n (c) | headphone |  | SV hörlur |
| **ikon** | n (c) | icon |  | FR icône |
| **innstellning** | n (c) | setting (config) |  | SV inställning |
| **installasjon** | n (c) | installation |  | FR installation |
| **installera** | v | install |  | FR installer |
| **intelligens** | n (c) | intelligence |  | FR intelligence |
| **internett** | n (n) | internet |  | EN internet |
| **kabel** | n (c) | cable |  | SV kabel + FR câble |
| **kamera** | n (c) | camera |  | SV kamera + FR caméra |
| **katalog** | n (c) | directory, catalogue |  | SV katalog + FR catalogue |
| **klikk** | n (n) | click (n.) |  | EN click |
| **klikka** | v | click (v.) |  | EN click |
| **klistra** | v | paste |  | SV klistra |
| **kod** | n (c) | code |  | SV kod + FR code |
| **koda** | v | code (v.) |  | kod + -a |
| **konsoll** | n (c) | console |  | FR console |
| **konto** | n (n) | account |  | SV konto + FR compte |
| **kopie** | n (c) | copy (n.) |  | FR copie |
| **kopiera** | v | copy (v.) |  | FR copier |
| **krasch** | n (c) | crash (n.) |  | EN crash |
| **kraska** | v | crash |  | EN crash |
| **kryptera** | v | encrypt |  | FR crypter |
| **kryptering** | n (c) | encryption |  | kryptera + -ing |
| **kunstig** | adj | artificial |  | SV konstgjord + NL kunstmatig |
| **ladda** | v | load, charge |  | SV ladda |
| **laddare** | n (c) | charger |  | ladda + -are |
| **laddning** | n (c) | charge (n.) |  | ladda + -ing |
| **lagra** | v | store, save |  | SV lagra |
| **lagring** | n (c) | storage |  | lagra + -ing |
| **lenk** | n (c) | link |  | SV länk + EN link |
| **logga** | v | log (in) |  | EN log |
| **losenord** | n (n) | password |  | SV lösenord |
| **mapp** | n (c) | folder |  | SV mapp |
| **markor** | n (c) | cursor |  | FR curseur + SV markör |
| **masjinvara** | n (c) | hardware |  | masjin + vara |
| **mikrofon** | n (c) | microphone |  | FR microphone |
| **minne** | n (n) | memory (store) |  | SV minne |
| **nedladda** | v | download |  | ned + ladda |
| **nedladdning** | n (c) | download (n.) |  | nedladda + -ing |
| **nett** | n (n) | net (mesh) |  | SV nät + NL net |
| **nettlesare** | n (c) | browser |  | SV webbläsare + NL webbrowser |
| **netverk** | n (n) | network |  | SV nätverk |
| **notifikasjon** | n (c) | notification |  | FR notification |
| **offline** | adj | offline |  | EN offline |
| **online** | adj | online |  | EN online |
| **oppladda** | v | upload |  | opp + ladda |
| **piksel** | n (c) | pixel |  | EN pixel |
| **plattform** | n (c) | platform |  | FR plateforme + SV plattform |
| **podd** | n (c) | podcast |  | SV podd |
| **profil** | n (c) | profile |  | FR profil |
| **program** | n (n) | program |  | SV program + EN program |
| **programmera** | v | program (v.) |  | FR programmer |
| **programmerare** | n (c) | programmer |  | programmera + -are |
| **programvara** | n (c) | software |  | program + vara |
| **prosessor** | n (c) | processor |  | FR processeur |
| **radera** | v | delete |  | SV radera |
| **robot** | n (c) | robot |  | EN robot + SV robot |
| **sekerhedskopie** | n (c) | backup |  | sekerhed + kopie |
| **server** | n (c) | server |  | EN server |
| **signal** | n (c) | signal |  | FR signal + SV signal |
| **skanna** | v | scan |  | EN scan + SV skanna |
| **skanner** | n (c) | scanner |  | EN scanner |
| **skerm** | n (c) | screen |  | SV skärm + NL scherm |
| **skermbild** | n (n) | screenshot |  | skerm + bild |
| **skiva** | n (c) | disk |  | SV skiva |
| **skrivare** | n (c) | printer |  | skriva + -are |
| **smart** | adj | smart |  | EN smart |
| **smarttelefon** | n (c) | smartphone |  | smart + telefon |
| **soka** | v | search, seek |  | SV söka |
| **sokning** | n (c) | search (n.) |  | soka + -ing |
| **stromma** | v | stream (v.) |  | SV strömma |
| **stromming** | n (c) | streaming |  | stromma + -ing |
| **surfa** | v | browse |  | SV surfa |
| **surfplatta** | n (c) | tablet |  | SV surfplatta |
| **tast** | n (c) | key (button) |  | NL toets + DA tast |
| **tastbord** | n (n) | keyboard |  | tast + bord |
| **traadlos** | adj | wireless |  | traad + -los |
| **uppdatera** | v | update (v.) |  | SV uppdatera |
| **uppdatering** | n (c) | update (n.) |  | uppdatera + -ing |
| **versjon** | n (c) | version |  | FR version |
| **video** | n (n) | video |  | EN video |
| **viral** | adj | viral |  | EN viral |
| **virtuell** | adj | virtual |  | FR virtuel |
| **webb** | n (c) | web |  | EN web |
| **websida** | n (c) | website |  | webb + sida |
| **wifi** | n (n) | wifi |  | EN wifi |
| **wolktjenst** | n (c) | cloud service |  | wolk + tjenst |

## society

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **aand** | n (c) | spirit |  | DA ånd |
| **antropologi** | n (c) | anthropology |  | FR anthropologie |
| **arbetslos** | adj | unemployed |  | SV arbetslös |
| **arbetsloshed** | n (c) | unemployment |  | arbetslos + -hed |
| **attityd** | n (c) | attitude |  | FR attitude |
| **beda** | v | pray, beg |  | DA bede + NL bidden |
| **beding** | n (c) | prayer |  | beda + -ing |
| **befolka** | v | populate |  | be- + folk + -a |
| **befolkning** | n (c) | population |  | befolka + -ing |
| **beteende** | n (n) | behaviour |  | SV beteende |
| **byrokrati** | n (n) | bureaucracy |  | FR bureaucratie |
| **demografi** | n (c) | demography |  | FR démographie |
| **diskriminera** | v | discriminate |  | FR discriminer |
| **engel** | n (c) | angel |  | NL engel + DA engel |
| **etik** | n (c) | ethics |  | FR éthique |
| **etnisk** | adj | ethnic |  | FR ethnique |
| **fatigdom** | n (c) | poverty |  | fatig + dom |
| **fordom** | n (c) | prejudice |  | SV fördom |
| **forort** | n (c) | suburb |  | SV förort |
| **framsteg** | n (n) | progress |  | fram + steg |
| **gemenskap** | n (c) | community |  | SV gemenskap |
| **generasjon** | n (c) | generation |  | FR génération |
| **gud** | n (c) | god, God |  | SV gud + DA gud + NL god |
| **gudig** | adj | divine |  | gud + -ig |
| **hemlos** | adj | homeless, unhoused |  | hem + -los |
| **hemloshed** | n (c) | homelessness |  | hemlos + -hed |
| **hierarki** | n (n) | hierarchy |  | FR hiérarchie |
| **ideologi** | n (c) | ideology |  | FR idéologie |
| **individ** | n (c) | individual |  | FR individu + SV individ |
| **industriell** | adj | industrial |  | industri + -ell |
| **integrasjon** | n (c) | integration |  | FR intégration |
| **invandring** | n (c) | immigration |  | SV invandring |
| **jemlik** | adj | equal (social) |  | SV jämlik |
| **jemlikhed** | n (c) | social equality |  | jemlik + -hed |
| **kapital** | n (n) | capital |  | FR capital |
| **kapitalisme** | n (c) | capitalism |  | FR capitalisme |
| **kognisjon** | n (c) | cognition |  | FR cognition |
| **kognitiv** | adj | cognitive |  | FR cognitif |
| **konflikt** | n (c) | conflict |  | FR conflit + SV konflikt |
| **konservativ** | adj | conservative |  | FR conservateur |
| **konsument** | n (c) | consumer |  | FR consommateur |
| **kulturell** | adj | cultural |  | kultur + -ell |
| **landsbygd** | n (c) | countryside |  | SV landsbygd |
| **liberalisme** | n (c) | liberalism |  | FR libéralisme |
| **marknadsplas** | n (c) | marketplace |  | marknad + plas |
| **medlemskap** | n (n) | membership |  | medlem + -skap |
| **medvetande** | n (n) | consciousness |  | SV medvetande |
| **medveten** | adj | conscious |  | SV medveten |
| **migrant** | n (c) | migrant |  | FR migrant |
| **migrasjon** | n (c) | migration |  | FR migration |
| **moral** | n (c) | morality |  | FR morale |
| **moralisk** | adj | moral |  | moral + -isk |
| **norm** | n (c) | norm |  | FR norme + SV norm |
| **ojemlikhed** | n (c) | inequality |  | o- + jemlikhed |
| **organisasjon** | n (c) | organisation |  | FR organisation |
| **persepsjon** | n (c) | perception |  | FR perception |
| **personlig** | adj | personal |  | person + -lig |
| **personlighed** | n (c) | personality |  | personlig + -hed |
| **politik** | n (c) | politics |  | FR politique |
| **populisme** | n (c) | populism |  | FR populisme |
| **privilegie** | n (n) | privilege | high | FR privilège |
| **problem** | n (n) | problem |  | FR problème |
| **produsent** | n (c) | producer |  | FR producteur |
| **propaganda** | n (c) | propaganda |  | FR propagande |
| **protest** | n (c) | protest |  | FR protestation + EN protest |
| **protestera** | v | protest (v.) |  | FR protester |
| **psykolog** | n (c) | psychologist |  | FR psychologue |
| **psykologi** | n (c) | psychology |  | FR psychologie |
| **reform** | n (c) | reform |  | FR réforme |
| **rettvished** | n (c) | justice, fairness |  | rettvis + -hed |
| **risk** | n (c) | risk |  | SV risk + FR risque |
| **riskera** | v | risk (v.) |  | FR risquer |
| **ritual** | n (n) | ritual |  | FR rituel |
| **rolle** | n (c) | role |  | FR rôle + SV roll |
| **samarbeta** | v | cooperate |  | SV samarbeta |
| **samarbete** | n (n) | cooperation |  | SV samarbete |
| **situasjon** | n (c) | situation |  | FR situation |
| **sjelig** | adj | spiritual |  | sjel + -ig |
| **solidaritet** | n (c) | solidarity |  | FR solidarité |
| **sosialisme** | n (c) | socialism |  | FR socialisme |
| **sosiolog** | n (c) | sociologist |  | FR sociologue |
| **sosiologi** | n (c) | sociology |  | FR sociologie |
| **spoka** | v | haunt |  | spoke + -a |
| **spoke** | n (n) | ghost, specter |  | SV spöke |
| **statistik** | n (c) | statistics |  | FR statistique |
| **status** | n (c) | status |  | EN status + FR statut |
| **tradisjon** | n (c) | tradition |  | FR tradition |
| **urbanisera** | v | urbanise |  | FR urbaniser |
| **urbanisering** | n (c) | urbanisation |  | urbanisera + -ing |
| **utvandring** | n (c) | emigration |  | SV utvandring |
| **velsigna** | v | bless |  | DA velsigne + SV välsigna |
| **velsigning** | n (c) | blessing |  | velsigna + -ing |
| **verde** | n (n) | value |  | SV värde + NL waarde |

## common-verbs

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **akseptera** | v | accept |  | FR accepter |
| **akseptering** | n (c) | acceptance |  | akseptera + -ing |
| **assistera** | v | help | high | FR assister |
| **avsloja** | v | reveal, disclose |  | SV avslöja |
| **avvisa** | v | reject, turn away |  | av + visa |
| **avvising** | n (c) | rejection |  | avvisa + -ing |
| **beginna** | v | begin | everyday | NL beginnen + EN begin |
| **bera** | v | carry |  | SV bära |
| **bevega** | v | move, stir |  | NL bewegen + DA bevæge |
| **beveging** | n (c) | movement, motion |  | bevega + -ing |
| **binda** | v | tie, bind |  | SV binda + NL binden |
| **blaasa** | v | blow |  | SV blåsa |
| **bliva** | v | become |  | SV bliva + DA blive |
| **boja** | v | bend |  | SV böja |
| **breka** | v | break |  | EN break + SV bräcka |
| **brenna** | v | burn |  | SV bränna + DA brænde |
| **bringa** | v | bring |  | SV bringa + EN bring |
| **bruka** | v | use | everyday | SV bruka + NL gebruiken |
| **dela** | v | divide, share |  | del + -a |
| **demonstrera** | v | show | high | FR démontrer |
| **djupa** | v | deepen |  | djup + -a |
| **doa** (pres=doar, past=dede, perf=doat) | v | do, make |  | EN do + NL doen |
| **draga** | v | pull |  | SV draga + NL dragen |
| **dyka** | v | dive |  | SV dyka + NL duiken |
| **enda** | v | end (v.) | everyday | EN end + SV ände |
| **erfara** | v | experience (v.) |  | SV erfara + NL ervaren |
| **erfaring** | n (c) | experience (n.) |  | erfara + -ing |
| **faa** (pres=faar, past=fik, perf=faat) | v | get, receive |  | SV få + DA få |
| **fall** | n (n) | fall (n.), drop |  | SV fall + DA fald |
| **falla** | v | fall |  | SV falla + NL vallen |
| **fanga** | v | catch, capture |  | SV fånga |
| **fela** | v | fail, err |  | fel + -a |
| **feling** | n (c) | failure |  | fela + -ing |
| **finna** | v | find |  | SV finna + NL vinden |
| **fjerna** | v | remove, take away |  | fjern + -a |
| **flyga** | v | fly (v.) |  | SV flyga |
| **flyta** | v | float |  | SV flyta |
| **folga** | v | follow |  | SV följa + DA følge |
| **forlata** | v | abandon, forsake |  | DA forlade + NL verlaten |
| **forlating** | n (c) | abandonment |  | forlata + -ing |
| **forstora** | v | destroy, ruin |  | SV förstöra |
| **forstoring** | n (c) | destruction |  | forstora + -ing |
| **fortsetta** | v | continue |  | SV fortsätta + DA fortsætte |
| **forventa** | v | expect |  | DA forvente |
| **forventing** | n (c) | expectation |  | forventa + -ing |
| **frysa** | v | freeze |  | SV frysa |
| **fylla** | v | fill |  | SV fylla + NL vullen |
| **gaa** (pres=gaar, past=gik, perf=gaat) | v | go |  | SV gå + NL gaan |
| **geva** (pres=gevar, past=gav, perf=gevat) | v | give |  | NL geven + SV ge + EN give |
| **gnida** | v | rub |  | SV gnida |
| **gomma** | v | hide |  | SV gömma |
| **grava** | v | dig |  | SV gräva + NL graven |
| **gripa** | v | grip, seize |  | SV gripa + DA gribe |
| **halda** | v | hold |  | DA holde + SV hålla |
| **hantera** | v | handle, manage |  | SV hantera |
| **hava** (pres=har, past=hadde, perf=havt) | v | have |  | SV ha + NL hebben + EN have |
| **herda** | v | endure, persevere |  | SV härda |
| **hjelpa** | v | help | everyday | SV hjälpa + NL helpen |
| **hoppa** | v | jump |  | SV hoppa |
| **jaga** | v | hunt |  | SV jaga + NL jagen |
| **kamp** | n (c) | fight, struggle (n.) |  | SV kamp + DA kamp |
| **kampa** | v | fight, struggle |  | SV kämpa + NL kampen |
| **kasta** | v | throw |  | SV kasta |
| **klimma** | v | climb |  | NL klimmen + EN climb |
| **klora** | v | scratch |  | SV klösa + NL klauwen |
| **klyva** | v | split |  | SV klyva |
| **knusa** | v | crush, shatter, devastate |  | DA knuse |
| **knyta** | v | tie, knot, bind |  | SV knyta |
| **komma** (pres=kommar, past=kom, perf=kommat) | v | come |  | SV komma + EN come |
| **kommensera** | v | begin | high | FR commencer |
| **kramma** | v | squeeze, hug |  | SV krama |
| **krypa** | v | crawl |  | SV krypa |
| **kunna** (pres=kan, past=kunde, perf=kunnat) | v | can, be able |  | SV kunna + NL kunnen |
| **laga** | v | repair, fix |  | SV laga |
| **lata** | v | let, allow |  | SV låta + NL laten |
| **legga** | v | lay |  | SV lägga |
| **lemna** | v | leave |  | SV lämna |
| **leta** | v | search |  | SV leta |
| **letta** | v | lighten, relieve |  | lett + -a |
| **letting** | n (c) | relief |  | letta + -ing |
| **leva** | v | live |  | SV leva |
| **ligga** | v | lie (recline) |  | SV ligga + NL liggen |
| **lyfta** | v | lift |  | SV lyfta |
| **moste** (pres=moste, past=moste) | v | must |  | SV måste + NL moeten + EN must |
| **opna** | v | open (v.) |  | SV öppna |
| **overraska** | v | surprise |  | SV överraska + DA overraske |
| **overrasking** | n (c) | surprise (n.) |  | overraska + -ing |
| **prova** | v | try |  | SV prova |
| **pusja** | v | push |  | EN push + FR pousser |
| **redda** | v | save, rescue |  | SV rädda + DA redde |
| **renkontrera** | v | meet | high | FR rencontrer |
| **retta** | v | correct (v.) |  | rett + -a |
| **rinna** | v | flow |  | SV rinna |
| **riva** | v | tear |  | SV riva |
| **rora** | v | touch, stir |  | SV röra |
| **samla** | v | gather, assemble |  | SV samla |
| **se** (pres=ser, past=saag, perf=seet) | v | see |  | SV se + NL zien |
| **seja** (pres=sejar, past=sa, perf=sejt) | v | say |  | SV säga + NL zeggen + EN say |
| **selektera** | v | choose | high | FR sélectionner |
| **senda** | v | send |  | SV sända + DA sende |
| **setta** | v | set, put |  | SV sätta + NL zetten |
| **simma** | v | swim |  | SV simma |
| **sitta** | v | sit |  | SV sitta + NL zitten |
| **sjunka** | v | sink |  | SV sjunka |
| **skada** | v | harm (v.) |  | SV skada |
| **skapa** | v | create |  | SV skapa + DA skabe |
| **skapare** | n (c) | creator |  | skapa + -are |
| **skaping** | n (c) | creation |  | skapa + -ing |
| **skera** | v | cut |  | SV skära |
| **skifta** | v | shift, switch |  | SV skifta |
| **skilja** | v | separate, part |  | SV skilja |
| **skola** (pres=skal, past=skulle) | v | shall (future auxiliary) |  | SV skola + NL zullen |
| **slaga** | v | hit |  | SV slå + DA slå |
| **sparka** | v | kick |  | SV sparka |
| **springa** | v | run |  | SV springa |
| **staa** (pres=staar, past=stod, perf=stat) | v | stand |  | SV stå + NL staan |
| **stanna** | v | stay, remain |  | SV stanna |
| **starta** | v | start (v.) |  | SV starta + EN start |
| **stenga** | v | close (v.) |  | SV stänga |
| **stiga** | v | rise |  | SV stiga |
| **stinga** | v | stab |  | SV stinga |
| **stoppa** | v | stop |  | SV stoppa + EN stop |
| **stotta** | v | support, prop up |  | SV stötta |
| **svella** | v | swell |  | SV svälla + EN swell |
| **ta** (pres=tar, past=tok, perf=tat) | v | take |  | SV ta + EN take |
| **tapa** | v | lose |  | SV tappa |
| **tella** | v | count |  | NL tellen + DA tælle |
| **terminera** | v | end (v.) | high | FR terminer |
| **tilhora** | v | belong |  | SV tillhöra + DA tilhøre |
| **tomma** | v | empty (v.) |  | SV tömma |
| **torka** | v | dry, wipe |  | SV torka |
| **treffa** | v | meet | everyday | SV träffa |
| **undvika** | v | avoid |  | SV undvika |
| **utforska** | v | explore |  | SV utforska |
| **utforsking** | n (c) | exploration |  | utforska + -ing |
| **utilisera** | v | use | high | FR utiliser |
| **utmana** | v | challenge |  | SV utmana |
| **utmaning** | n (c) | challenge (n.) |  | utmana + -ing |
| **vandra** | v | walk, wander |  | SV vandra + NL wandelen |
| **veksa** | v | grow |  | SV växa + DA vokse |
| **veksla** | v | exchange, change |  | SV växla + NL wisselen |
| **velja** | v | choose | everyday | SV välja + DA vælge |
| **venda** | v | turn |  | SV vända + DA vende |
| **venta** | v | wait |  | SV vänta + DA vente |
| **vera** (pres=er, past=var, perf=vart) | v | be |  | SV vara + NL wezen |
| **vilja** (pres=vil, past=vilde, perf=vilt) | v | want, will |  | SV vilja + NL willen |
| **vinna** | v | win |  | SV vinna + NL winnen |
| **visa** | v | show | everyday | SV visa |

## qualities

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **absolut** | adj | absolute | high | FR absolu |
| **aktiv** | adj | active | high | FR actif |
| **alert** | adj | alert, vigilant |  | SV alert + EN alert + FR alerte |
| **Amerikansk** | adj | American |  | EN America + SV amerikansk |
| **autentisk** | adj | authentic |  | FR authentique |
| **barsk** | adj | harsh, gruff |  | SV barsk + DA barsk |
| **bekend** | adj | familiar, well-known |  | NL bekend |
| **bela** | adj | beautiful |  | FR beau/belle |
| **belhed** | n (c) | beauty |  | bela + -hed |
| **best** | adj | best |  | NL best + EN best |
| **beter** | adj | better |  | NL beter + EN better |
| **blaa** | adj | blue |  | SV blå + NL blauw |
| **blij** | adj | happy |  | NL blij |
| **bred** | adj | wide |  | SV bred + DA bred |
| **brutal** | adj | brutal |  | FR brutal |
| **bruun** | adj | brown |  | SV brun + NL bruin |
| **dalig** | adj | bad |  | SV dålig |
| **Dansk** | adj | Danish |  | DA dansk + SV dansk |
| **dar** | adj | amber (colour) |  | FR doré + EN amber |
| **direkt** | adj | direct |  | SV direkt + FR direct |
| **djup** | adj | deep, profound |  | SV djup |
| **djuphed** | n (c) | depth |  | djup + -hed |
| **dobbel** | adj | double, dual |  | DA dobbelt + NL dubbel |
| **egen** | adj | own |  | SV egen + DA egen |
| **ekta** | adj | genuine, real |  | SV äkta + NL echt |
| **endelos** | adj | endless |  | NL eindeloos + DA endeløs |
| **Engelsk** | adj | English |  | SV engelsk |
| **enkel** | adj | simple |  | SV enkel + NL enkel |
| **erlig** | adj | honest |  | SV ärlig |
| **fantastisk** | adj | amazing, fantastic |  | FR fantastique + SV fantastisk |
| **farlig** | adj | dangerous |  | SV farlig + DA farlig |
| **fast** | adj | fixed, firm |  | SV fast + DA fast |
| **fatig** | adj | poor | everyday | SV fattig |
| **fel** | adj | wrong |  | SV fel |
| **ferg** | n (c) | colour |  | SV färg + DA farve |
| **fiskig** | adj | fishy, suspicious (slang) | everyday | fisk + -ig |
| **flak** | adj | shallow |  | SV flack + NL vlak |
| **form** | n (c) | form, shape |  | FR forme + SV form |
| **formell** | adj | formal | high | FR formel + SV formell |
| **Fransk** | adj | French |  | SV fransk |
| **fremmed** | adj | strange, unfamiliar, foreign |  | DA fremmed |
| **fri** | adj | free |  | EN free + SV fri |
| **full** | adj | full |  | SV full + EN full |
| **gammel** | adj | old |  | SV gammal + DA gammel |
| **gensidig** | adj | mutual |  | DA gensidig |
| **gensidighed** | n (c) | reciprocity, mutuality |  | gensidig + -hed |
| **goed** | adj | good |  | NL goed + EN good |
| **graa** | adj | grey |  | SV grå + DA grå |
| **gron** | adj | green |  | SV grön + NL groen |
| **grym** | adj | cruel |  | SV grym |
| **gul** | adj | yellow |  | SV gul |
| **hard** | adj | hard |  | EN hard + SV hård |
| **hel** | adj | whole, entire |  | SV hel + NL heel |
| **hemlig** | adj | secret |  | SV hemlig |
| **herlig** | adj | great, glorious |  | DA herlig + SV härlig |
| **het** | adj | hot |  | SV het + NL heet |
| **hinder** | n (n) | obstacle, barrier |  | SV hinder |
| **Hollandsk** | adj | Dutch |  | DA hollandsk + SV holländsk |
| **hoog** | adj | high |  | NL hoog |
| **hul** | adj | hollow |  | DA hul + NL hol |
| **inre** | adj | inner, internal |  | SV inre |
| **intens** | adj | intense |  | FR intense |
| **kald** | adj | cold |  | DA kold + NL koud |
| **klar** | adj | clear |  | SV klar + NL klaar |
| **klarhed** | n (c) | clarity |  | klar + -hed |
| **koel** | adj | cool |  | NL koel |
| **kort** | adj | short |  | SV kort + NL kort |
| **kraftig** | adj | powerful |  | kraft + -ig |
| **kritisk** | adj | critical |  | SV kritisk + FR critique |
| **laag** | adj | low |  | NL laag |
| **lang** | adj | long |  | SV lång + NL lang |
| **lekker** | adj | tasty, nice, great (all-purpose approval) | everyday | NL lekker |
| **lett** | adj | light (weight), easy |  | SV lätt + DA let |
| **levande** | adj | alive, living |  | SV levande + DA levende + NL levend |
| **lievverd** | adj | lovable |  | liev + verd |
| **lik** | adj | similar, alike |  | SV lik + EN like |
| **liten** | adj | small |  | SV liten |
| **mange** | det | many |  | SV många + EN many |
| **merkverdig** | adj | remarkable, curious |  | DA mærkværdig + SV märkvärdig |
| **middel** | adj | middle |  | NL middel + SV medel |
| **mjuk** | adj | soft |  | SV mjuk |
| **mobil** | adj | mobile, movable |  | SV mobil + FR mobile |
| **modern** | adj | modern |  | FR moderne + SV modern |
| **mogen** | adj | mature, ripe |  | SV mogen |
| **mojlig** | adj | possible |  | SV möjlig |
| **mojlighed** | n (c) | possibility |  | mojlig + -hed |
| **mork** | adj | dark |  | SV mörk + DA mørk |
| **nodvendig** | adj | necessary |  | DA nødvendig |
| **normal** | adj | normal |  | FR normal |
| **ny** | adj | new |  | SV ny |
| **oformell** | adj | informal, casual |  | o- + formell |
| **ogensidig** | adj | unrequited, one-sided |  | o- + gensidig |
| **olik** | adj | different |  | o- + lik |
| **omojlig** | adj | impossible |  | o- + mojlig |
| **open** | adj | open |  | NL open + EN open |
| **opulent** | adj | rich | high | FR opulent |
| **oranj** | adj | orange |  | FR orange |
| **otrolig** | adj | astounding, incredible |  | DA utrolig + SV otrolig |
| **perfekt** | adj | perfect |  | SV perfekt + FR parfait |
| **persistens** | n (c) | persistence |  | FR persistance + SV persistens |
| **persistent** | adj | persistent |  | FR persistant + SV persistent |
| **plotslig** | adj | sudden |  | SV plötslig |
| **pover** | adj | poor | high | FR pauvre |
| **praktisk** | adj | practical |  | SV praktisk + FR pratique |
| **produktiv** | adj | productive | high | FR productif |
| **progresiv** | adj | progressive | high | FR progressif |
| **purpur** | adj | purple |  | SV purpur + FR pourpre |
| **ren** | adj | clean |  | SV ren |
| **rett** | adj | correct, right |  | SV rätt + DA ret |
| **rik** | adj | rich | everyday | SV rik + NL rijk |
| **rimlig** | adj | reasonable |  | SV rimlig |
| **rod** | adj | red |  | SV röd + DA rød |
| **rosa** | adj | pink |  | SV rosa + FR rose |
| **rund** | adj | round |  | SV rund + NL rond |
| **rural** | adj | rural |  | FR rural |
| **rutten** | adj | rotten |  | SV rutten + EN rotten |
| **saltig** | adj | salty; resentful (slang) | everyday | salt + -ig |
| **seker** | adj | safe, sure |  | NL zeker + DA sikker |
| **seksi** | adj | sexy | everyday | EN sexy |
| **sentral** | adj | central | high | FR central |
| **serlig** | adj | special, particular |  | DA særlig |
| **sist** | adj | last, final |  | SV sist |
| **skarp** | adj | sharp |  | SV skarp + NL scherp |
| **slett** | adj | smooth |  | SV slät + DA slet |
| **slo** | adj | dull, blunt |  | SV slö |
| **smal** | adj | narrow |  | SV smal |
| **smuts** | n (c) | dirt |  | SV smuts |
| **smutsig** | adj | dirty |  | smuts + -ig |
| **stark** | adj | strong |  | SV stark + NL sterk |
| **starkhed** | n (c) | intensity |  | stark + -hed |
| **stor** | adj | big |  | SV stor |
| **svaar** | adj | difficult |  | SV svår |
| **svaarhed** | n (c) | difficulty |  | svaar + -hed |
| **svag** | adj | weak |  | SV svag + DA svag |
| **svart** | adj | black |  | SV svart + NL zwart |
| **Svensk** | adj | Swedish |  | SV svensk |
| **teder** | adj | tender, gentle |  | NL teder |
| **tjok** | adj | thick |  | SV tjock |
| **tom** | adj | empty |  | SV tom |
| **torr** | adj | dry |  | SV torr |
| **transparens** | n (c) | transparency |  | FR transparence + SV transparens |
| **transparent** | adj | transparent |  | FR transparent + SV transparent |
| **tung** | adj | heavy |  | SV tung |
| **tunn** | adj | thin |  | SV tunn + NL dun |
| **ung** | adj | young |  | SV ung + DA ung |
| **unik** | adj | unique |  | FR unique |
| **universell** | adj | universal |  | FR universel |
| **urgent** | adj | urgent, emergent |  | FR urgent |
| **vaat** | adj | wet |  | SV våt + DA våd |
| **vanlig** | adj | ordinary, usual |  | SV vanlig |
| **varm** | adj | warm |  | SV varm + NL warm |
| **varmhed** | n (c) | warmth |  | varm + -hed |
| **verd** | adj | worth, worthy |  | SV värd + NL waard |
| **verdig** | adj | dignified, worthy |  | verd + -ig |
| **viktig** | adj | important |  | SV viktig + NL wichtig |
| **vit** | adj | white |  | SV vit + NL wit |
| **wonderbar** | adj | wonderful |  | SV underbar + NL wonderbaar |
| **ytre** | adj | outer, external |  | DA ytre + SV yttre |

## function-words

| Laphurdi | pos | English | register | sources |
|---|---|---|---|---|
| **aldri** | adv | never |  | DA aldrig + SV aldrig |
| **alle** | det | all |  | NL alle + SV alla |
| **alltid** | adv | always |  | SV alltid |
| **als** | conj | than, as |  | NL als |
| **alt** | pron | everything, all (neuter) |  | SV allt |
| **altfor** | adv | too, excessively |  | alt + for |
| **anner** | det | other |  | SV annan + NL ander |
| **av** | prep | of, from |  | SV av |
| **bakom** | prep | behind |  | SV bakom |
| **bara** | adv | only |  | SV bara |
| **befor** | prep | before |  | EN before + SV förr |
| **beide** | det | both |  | NL beide + DA begge |
| **bort** | adv | away |  | SV bort |
| **bortom** | prep | beyond |  | bort + om |
| **dan** | adv | then |  | NL dan + EN then |
| **dat** | det | that |  | NL dat + EN that |
| **de** | det | the (plural, before adjectives) |  | SV de + DA de |
| **dei** | pron | they |  | SV de + EN they |
| **deis** | det | their |  | SV deras |
| **dem** | pron | them |  | SV dem + DA dem |
| **den** | det | the (common gender, before adjectives); it |  | SV den + DA den |
| **der** | adv | there |  | SV där + NL daar |
| **det** | det | the (neuter, before adjectives); it |  | SV det + DA det |
| **dij** | pron | you (object, singular) |  | NL dij + SV dig |
| **din** | det | your (singular) |  | SV din |
| **dis** | det | this |  | EN this + NL dit |
| **du** | pron | you (singular) |  | SV du |
| **efter** | prep | after |  | SV efter + DA efter |
| **el** | conj | or |  | SV eller |
| **elk** | det | each, every |  | NL elk |
| **endaa** | adv | yet, still, though |  | SV ändå + DA endda |
| **engang** | adv | once |  | en + gang |
| **ferst** | adv | first, firstly |  | DA først |
| **flera** | det | several |  | SV flera + DA flere |
| **for** | prep | for |  | EN for + SV för |
| **forbi** | prep | past, by |  | SV förbi + DA forbi |
| **fordat** | conj | because |  | for + dat |
| **forevig** | adv | forever |  | for + evig |
| **fra** | prep | from |  | SV från + DA fra |
| **fram** | adv | forward, forth |  | SV fram + DA frem |
| **gennem** | prep | through |  | DA gennem + SV genom |
| **ham** | pron | him |  | DA ham |
| **han** | pron | he |  | SV han |
| **hans** | det | his |  | SV hans |
| **heller** | adv | rather, preferably, sooner |  | DA hellere + SV hellre |
| **hen** | pron | they (singular) |  | SV hen |
| **henne** | pron | her (object) |  | SV henne + DA hende |
| **hens** | det | their (singular possessive) |  | SV hens |
| **hier** | adv | here |  | NL hier |
| **hoe** | adv | how |  | NL hoe |
| **hon** | pron | she |  | SV hon |
| **hons** | det | her (possessive) |  | SV hennes |
| **igen** | adv | again |  | SV igen + DA igen |
| **ik** | pron | I |  | NL ik |
| **in** | prep | in |  | EN in + NL in |
| **ingen** | pron | none, nobody |  | SV ingen |
| **ingenwaar** | adv | nowhere |  | ingen + waar |
| **innen** | prep | within |  | DA inden + SV inom |
| **isteden** | adv | instead |  | DA i stedet |
| **ja** | interj | yes |  | NL ja + SV ja |
| **jer** | det | your (plural) |  | DA jeres |
| **ju** | pron | you (plural) |  | NL jullie + EN you |
| **kanske** | adv | maybe, perhaps |  | SV kanske |
| **langs** | prep | along |  | SV längs + NL langs + DA langs |
| **med** | prep | with |  | SV med |
| **medan** | conj | while |  | SV medan |
| **mellan** | prep | between |  | SV mellan |
| **men** | conj | but |  | SV men |
| **mer** | adv | more |  | SV mer |
| **mest** | adv | most |  | SV mest |
| **mij** | pron | me (object) |  | NL mij + SV mig |
| **min** | det | my |  | SV min + NL mijn |
| **minder** | adv | less |  | NL minder |
| **minst** | adv | least |  | SV minst + NL minst |
| **mot** | prep | against, toward |  | SV mot |
| **mykke** | adv | much |  | SV mycket |
| **ned** | adv | down |  | SV ned + NL neer |
| **nej** | interj | no |  | SV nej |
| **neste** | adj | next |  | DA næste + SV nästa |
| **nit** | adv | not |  | NL nietfollows the verb: Ik sprekar nit Fransk |
| **nor** | conj | nor |  | EN nor |
| **nu** | adv | now |  | SV nu + NL nu |
| **oek** | adv | also |  | NL ook |
| **ofta** | adv | often |  | SV ofta |
| **og** | conj | and |  | SV och + DA og |
| **om** | conj | if, whether, about |  | SV om |
| **on** | prep | on |  | EN on |
| **opp** | adv | up |  | SV upp + NL op |
| **os** | pron | us |  | DA os + NL ons |
| **over** | prep | over, above |  | EN over + NL over |
| **overalt** | adv | everywhere |  | over + alt |
| **rundom** | prep | around |  | rund + om |
| **samme** | det | same |  | DA samme + SV samma |
| **self** | pron | self |  | EN self + NL zelf |
| **selfom** | conj | although, even though |  | self + om |
| **selfs** | adv | even |  | NL zelfs |
| **siden** | prep | since; ago |  | DA siden |
| **sik** | pron | himself, herself, themself (reflexive) |  | SV sig + DA sig + NL zich |
| **snart** | adv | soon |  | SV snart |
| **so** | adv | so, thus |  | SV så + EN so |
| **somhoe** | adv | somehow |  | somme + hoe |
| **somme** | det | some |  | NL sommige + EN some |
| **somtid** | adv | sometime |  | somme + tid |
| **somting** | pron | something, anything |  | somme + ting |
| **somwaar** | adv | somewhere, anywhere |  | somme + waar |
| **somwie** | pron | someone, anyone |  | somme + wie |
| **somwies** | det | someone's, anyone's |  | somwie + -s |
| **te** | prep | to |  | NL te + EN to |
| **terug** | adv | back (returning) |  | NL terug |
| **tesamme** | adv | together |  | NL tezamen |
| **tilbaka** | adv | back, backward |  | SV tillbaka + DA tilbage |
| **tils** | prep | until |  | SV tills |
| **trots** | prep | despite, in spite of |  | SV trots |
| **tvers** | adv | across, crosswise |  | SV tvärs + DA tværs |
| **tversover** | prep | across |  | tvers + over |
| **under** | prep | under, during, according to |  | SV under + EN under |
| **ut** | adv | out |  | SV ut + NL uit |
| **uten** | prep | without |  | SV utan + DA uden |
| **vaar** | det | our |  | SV vår |
| **ved** | prep | at, by |  | DA ved + SV vid |
| **venig** | det | few |  | NL weinig |
| **vi** | pron | we |  | SV vi |
| **waar** | adv | where |  | NL waar |
| **warfor** | adv | why |  | SV varför |
| **wat** | pron | what |  | NL wat + EN what |
| **wen** | adv | when |  | EN when + NL wanneer |
| **wie** | pron | who |  | NL wie |

## Alphabetical index

| Laphurdi | English |
|---|---|
| aand | spirit |
| absolut | absolute |
| abstrakt | abstract |
| adem | breath |
| adiksjon | addiction |
| adjuu | goodbye |
| adorera | love |
| adres | address |
| advokat | lawyer |
| affeksjon | affection |
| aften | evening |
| aftenmat | dinner, supper |
| akselerasjon | acceleration |
| akseptera | accept |
| akseptering | acceptance |
| aksiom | axiom |
| akt | eight |
| akte | eighth |
| akten | eighteen |
| akti | eighty |
| aktiv | active |
| akustik | acoustics |
| akut | acute |
| alarm | alarm, alert (n.) |
| alder | age |
| aldri | never |
| alergi | allergy |
| alergisk | allergic |
| alert | alert, vigilant |
| alg | seaweed |
| algebra | algebra |
| algoritme | algorithm |
| alle | all |
| alltid | always |
| als | than, as |
| alt | everything, all (neuter) |
| altfor | too, excessively |
| ambasadeur | ambassador |
| ambulans | ambulance |
| Amerikansk | American |
| amputasjon | amputation |
| amputera | amputate |
| analys | analysis |
| ananas | pineapple |
| anatomi | anatomy |
| and | duck |
| anda | breathe |
| andre | second |
| anga | steam |
| angra | regret, rue |
| angring | regret, remorse |
| angst | anxiety, dread |
| anker | anchor |
| ankomma | arrive |
| anner | other |
| ansvar | responsibility |
| antibiotika | antibiotic |
| antonym | antonym |
| antropologi | anthropology |
| anvendare | user |
| apel | apple |
| apotek | pharmacy |
| apotekare | pharmacist |
| app | app |
| aprikos | apricot |
| april | April |
| arbetslos | unemployed |
| arbetsloshed | unemployment |
| arg | angry |
| arghed | anger |
| argument | argument |
| argumentera | argue |
| arm | arm |
| arme | army |
| arr | scar |
| ars | ass (vulgar) |
| art | species |
| arterie | artery |
| artikel | article |
| artist | artist |
| arv | inheritance |
| arva | inherit |
| asistans | help (n.) |
| asjeblie | please |
| aske | ash |
| assistera | help |
| astma | asthma |
| atmosfer | atmosphere |
| atom | atom |
| attityd | attitude |
| august | August |
| autentisk | authentic |
| automat | automaton |
| automatisk | automatic |
| autonomi | autonomy |
| autoritet | authority |
| av | of, from |
| avatar | avatar |
| avfall | waste, garbage |
| avhengig | addicted, dependent |
| avhengighed | addiction, dependence |
| avokado | avocado |
| avresa | depart |
| avsloja | reveal, disclose |
| avstand | distance, gap |
| avtal | agreement, treaty |
| avvisa | reject, turn away |
| avvising | rejection |
| baan | track, way |
| baar | stretcher |
| baat | the greatest of all time (slang) |
| bad | bath |
| badrum | bathroom |
| bak | backside, butt |
| baka | bake |
| bakeri | bakery |
| bakom | behind |
| bakterie | bacterium |
| banan | banana |
| bank | bank |
| bara | only |
| bark | bark (of tree) |
| barometer | barometer |
| barsk | harsh, gruff |
| bas | base (chemistry) |
| basilika | basil |
| bebe | baby |
| beda | pray, beg |
| beding | prayer |
| bedrag | fraud |
| been | leg |
| befolka | populate |
| befolkning | population |
| befor | before |
| beginna | begin |
| begripa | understand |
| begriping | understanding (n.) |
| behandla | treat (medically) |
| behandling | treatment |
| behov | need (n.) |
| behova | need (v.) |
| beide | both |
| bekend | familiar, well-known |
| bekenna | confess, admit |
| bekenning | confession, admission |
| bekrefta | confirm, certify |
| bekrefting | confirmation, validation |
| bekymra | worry, trouble |
| bekymring | worry, concern |
| bela | beautiful |
| belhed | beauty |
| belt | belt |
| ben | bone |
| benign | benign |
| ber | berry |
| bera | carry |
| beretta | tell, recount, narrate |
| beretting | narrative, account, tale |
| berg | mountain |
| besok | visit (n.) |
| besoka | visit |
| best | best |
| bestefar | grandfather |
| bestemor | grandmother |
| besvika | disappoint |
| besviking | disappointment |
| beta | beet |
| betala | pay |
| betaling | payment |
| beteende | behaviour |
| beter | better |
| betrakta | contemplate, regard |
| betyda | mean (v.) |
| bevega | move, stir |
| beveging | movement, motion |
| bever | beaver |
| beverasje | drink (n.) |
| bevis | proof, evidence |
| bevisa | prove |
| bibliotek | library |
| bier | beer |
| bij | bee |
| bil | car |
| bild | picture |
| bildsprak | imagery |
| biljet | ticket |
| billig | cheap, affordable |
| binda | tie, bind |
| binding | bond, connection |
| biologi | biology |
| bit | bit |
| bita | bite |
| bitter | bitter |
| biverkning | side effect |
| bjork | birch |
| bjorn | bear |
| blaa | blue |
| blaaber | blueberry |
| blaamerke | bruise |
| blaas | bladder |
| blaasa | blow |
| blad | leaf |
| blij | happy |
| blijhed | happiness |
| blind | blind |
| bliva | become |
| blixt | lightning |
| blod | blood |
| bloda | bleed |
| blodgrupp | blood type |
| blodkerl | blood vessel |
| blodpropp | blood clot |
| blodstrom | bloodstream |
| blodtrykk | blood pressure |
| blogg | blog |
| blogga | blog (v.) |
| blomma | flower |
| boef | beef |
| boge | bow, arc |
| boja | bend |
| bok | book |
| bokstav | letter (of alphabet) |
| bolge | wave (physics) |
| boll | ball |
| bomull | cotton |
| bonde | farmer |
| bonna | bean |
| boot | boat |
| bord | table |
| borda | burden |
| borderline | borderline (personality condition) |
| borger | citizen |
| borgerskap | citizenship |
| borgmester | mayor |
| bors | stock exchange |
| bort | away |
| bortom | beyond |
| bot | remedy, cure |
| botmedel | remedy |
| braka | vomit |
| brannmur | firewall |
| bred | wide |
| breka | break |
| brekare | breaker |
| brenna | burn |
| brenning | burning |
| brennvin | aquavit |
| brev | letter (mail) |
| bringa | bring |
| bro | bridge |
| brod | bread |
| broder | brother |
| broek | trousers |
| brok | fraction |
| brokoli | broccoli |
| bror | bro (informal address) |
| brott | crime |
| brud | bride |
| brudgom | bridegroom |
| bruk | use (n.) |
| bruka | use |
| brusk | cartilage |
| brutal | brutal |
| bruun | brown |
| bryllop | wedding |
| bryst | breast, chest |
| budsjet | budget |
| bugg | bug (software) |
| bukt | bay, gulf |
| buss | bus |
| butik | shop |
| bygga | build |
| bygging | building, construction |
| byrokrati | bureaucracy |
| byte | byte |
| cell | cell |
| chatt | chat (n.) |
| chatta | chat (v.) |
| dag | day |
| dal | valley |
| dalig | bad |
| dame | lady |
| dan | then |
| dank | thanks |
| dans | dance (n.) |
| dansa | dance (v.) |
| Dansk | Danish |
| dar | amber (colour) |
| Darcambria | Darcambria (the harbour city) |
| dat | that |
| data | data |
| databas | database |
| dator | computer |
| de | the (plural, before adjectives) |
| debatt | debate |
| deg | dough |
| dei | they |
| deilig | delicious |
| deis | their |
| dekk | deck |
| deklarera | declare |
| deklarering | declaration |
| del | part, portion |
| dela | divide, share |
| delikt | crime |
| dem | them |
| demandera | ask |
| demografi | demography |
| demokrati | democracy |
| demokratisk | democratic |
| demonstrera | show |
| den | the (common gender, before adjectives); it |
| densitet | density |
| depresjon | depression |
| der | there |
| desedera | die |
| desember | December |
| desperat | desperate |
| destillera | distil |
| det | the (neuter, before adjectives); it |
| diabetes | diabetes |
| diagnos | diagnosis |
| diagnostisera | diagnose |
| dier | animal |
| differens | difference (math) |
| digital | digital |
| digitalisera | digitise |
| dij | you (object, singular) |
| dikt | poem |
| dikta | compose poetry |
| diktare | poet |
| dill | dill |
| dimensjon | dimension |
| din | your (singular) |
| dinera | eat |
| direkt | direct |
| dis | this |
| diskriminasjon | discrimination |
| diskriminera | discriminate |
| diskur | speech |
| djevel | devil |
| djup | deep, profound |
| djupa | deepen |
| djuphed | depth |
| doa | do, make |
| dobbel | double, dual |
| dod | death |
| dodstraff | death penalty |
| doft | scent, fragrance |
| dok | dock |
| doktor | doctor |
| dokument | document |
| dollar | dollar |
| dom | judgment |
| doma | judge (v.) |
| domare | judge (n.) |
| doming | judgment, trial |
| domna | go numb |
| domstol | court |
| donor | donor |
| dor | door |
| dos | dose |
| dosera | dose, measure out |
| dosering | dosage |
| dotter | daughter |
| dov | deaf |
| draga | pull |
| drama | drama |
| dramatisk | dramatic |
| dreppa | kill |
| drika | drink |
| drikk | drink (n.) |
| drog | drug |
| drom | dream |
| dromland | dreamland, imaginary country |
| dromma | dream (v.) |
| dron | drone |
| dronken | drunk |
| drukna | drown |
| druva | grape |
| du | you (singular) |
| dum | stupid |
| dumhoved | blockhead |
| dusin | dozen |
| dusj | shower |
| dyka | dive |
| eb | ebb |
| eddik | vinegar |
| efter | after |
| egalitet | equality |
| egen | own |
| egendom | property |
| egg | egg |
| ek | oak |
| ekko | echo |
| ekologi | ecology |
| ekonomi | economy |
| ekonomisk | economic |
| ekosystem | ecosystem |
| eksamen | examination |
| eksekutiv | executive |
| eksistens | existence |
| eksistensiell | existential |
| eksistera | exist |
| eksperiment | experiment |
| eksplikera | explain |
| eksport | export |
| ekta | genuine, real |
| ekwasjon | equation |
| el | or |
| eld | fire |
| eldwerk | fireworks |
| elektrisitet | electricity |
| elektrisk | electric |
| elektron | electron |
| element | element |
| elev | pupil |
| elg | elk, moose |
| elk | each, every |
| elva | eleven |
| emosjon | emotion |
| emosjonell | emotional |
| empati | empathy |
| empatisk | empathetic |
| empirisk | empirical |
| en | one |
| enda | end (v.) |
| endaa | yet, still, though |
| endelos | endless |
| energi | energy |
| engang | once |
| engel | angel |
| Engelsk | English |
| enhed | unit |
| enkel | simple |
| ensam | lonely, alone |
| ensamhed | loneliness |
| entropi | entropy |
| enzym | enzyme |
| epidemi | epidemic |
| epost | email |
| era | honour (v.) |
| erfara | experience (v.) |
| erfaring | experience (n.) |
| erkenna | acknowledge |
| erlig | honest |
| erlighed | honesty |
| ert | pea |
| et | a, an (neuter article) |
| eta | eat |
| etik | ethics |
| etnisk | ethnic |
| euro | euro |
| evig | eternal, everlasting |
| evighed | eternity |
| evne | ability, capacity |
| evolusjon | evolution |
| faa | get, receive |
| faar | sheep |
| fabrik | factory |
| fader | father |
| fakt | fact |
| faktor | factor |
| fall | fall (n.), drop |
| falla | fall |
| falsk | false |
| famille | family |
| fanga | catch, capture |
| fangst | catch (of fish), haul |
| fantasi | fantasy, imagination |
| fantastisk | amazing, fantastic |
| farlig | dangerous |
| fast | fixed, firm |
| fastlegga | commit (changes), record |
| fastlegging | commit (n.), recorded change |
| fatig | poor |
| fatigdom | poverty |
| feber | fever |
| feberig | feverish |
| februar | February |
| feder | feather |
| feil | error, fault |
| fel | wrong |
| fela | fail, err |
| feling | failure |
| felisitet | happiness |
| felt | field |
| fem | five |
| femte | fifth |
| femten | fifteen |
| femti | fifty |
| fengsel | prison |
| fenster | window |
| ferg | colour |
| ferie | holiday |
| ferje | ferry |
| fersk | fresh |
| ferst | first, firstly |
| ferste | first |
| fest | party, feast |
| fett | fat (n.) |
| fiende | enemy |
| figon | fig |
| fikk | pocket |
| fiksjon | fiction |
| fiktiv | fictional, fictitious |
| fil | file |
| film | film |
| filosofi | philosophy |
| finans | finance |
| finger | finger |
| finna | find |
| fiol | violin |
| firma | firm, company |
| fisk | fish |
| fiska | fish (v.) |
| fiskare | fisher |
| fiskeri | fishery |
| fiskig | fishy, suspicious (slang) |
| fitta | pussy (vulgar) |
| fjard | wide bay, firth |
| fjeril | butterfly |
| fjern | far |
| fjerna | remove, take away |
| fjernkontroll | remote control |
| fjorten | fourteen |
| flagg | flag |
| flak | shallow |
| flaska | bottle |
| flera | several |
| flieg | fly (insect) |
| flika | girl |
| flod | river |
| floed | flood tide |
| flotta | fleet |
| flyga | fly (v.) |
| flyghamn | airport |
| flygplan | airplane |
| flykt | flight, escape |
| flykta | flee, escape |
| flyta | float |
| flytta | move (house), relocate |
| foda | give birth, bear |
| foding | birth |
| folga | follow |
| folk | people |
| folkskamer | people's chamber, commons |
| folkskydd | civil protection |
| folkstemming | referendum |
| folktelling | census |
| for | for |
| foranga | evaporate |
| forbanna | curse, damn |
| forbi | past, by |
| forbida | forbid |
| forbiding | prohibition |
| forbind | bandage |
| forbrenning | combustion |
| fordat | because |
| fordom | prejudice |
| forelder | parent |
| forening | compound (chemical) |
| forevig | forever |
| forfattare | author, writer |
| forklara | explain |
| forlata | abandon, forsake |
| forlating | abandonment |
| forlust | loss |
| form | form, shape |
| formel | formula |
| formell | formal |
| forort | suburb |
| forska | research (v.) |
| forskare | researcher |
| forsking | research (n.) |
| forslag | proposal |
| forsona | reconcile |
| forsoning | reconciliation |
| forstora | destroy, ruin |
| forstoring | destruction |
| forsvar | defence |
| fortid | past (n.) |
| fortsetta | continue |
| fortvivla | despair |
| fortvivling | despair (n.) |
| forventa | expect |
| forventing | expectation |
| forvirra | confuse, bewilder |
| forvirring | confusion |
| fossil | fossil |
| fot | foot |
| fotboll | football |
| foto | photograph |
| foton | photon |
| fotosyntes | photosynthesis |
| fra | from |
| fraga | ask |
| frakt | fracture |
| fram | forward, forth |
| framsteg | progress |
| framtid | future |
| Fransk | French |
| fras | phrase |
| fravaro | absence |
| fred | peace |
| fredag | Friday |
| frekvens | frequency |
| fremling | stranger |
| fremmed | strange, unfamiliar, foreign |
| fri | free |
| frihed | freedom |
| friksjon | friction |
| frisk | fresh, well |
| friskna | recover (get well) |
| fro | seed |
| fru | madam, wife |
| frukost | breakfast |
| frukt | fruit |
| frykt | fear (n.) |
| frykta | fear (v.) |
| frysa | freeze |
| fugel | bird |
| full | full |
| fundamentell | fundamental |
| funksjon | function |
| funksjonshinder | disability |
| fylla | fill |
| fyr | four |
| fyrde | fourth |
| fyrti | forty |
| fysik | physics |
| fysiologi | physiology |
| fysioterapi | physiotherapy |
| fysisk | physical |
| gaa | go |
| gaffel | fork |
| gal | crazy |
| galakse | galaxy |
| galla | bile |
| galleri | gallery |
| gammel | old |
| gang | time, occasion |
| garanti | guarantee |
| gard | yard, farm |
| gardin | curtain |
| gas | gas |
| gata | street |
| gava | gift |
| gebar | gesture |
| gemenskap | community |
| gen | gene |
| generasjon | generation |
| genetik | genetics |
| gennem | through |
| genomsnitt | average |
| gensidig | mutual |
| gensidighed | reciprocity, mutuality |
| geografi | geography |
| geometri | geometry |
| gest | guest |
| get | goat |
| geva | give |
| gift | poison |
| gifta | marry |
| giftig | poisonous |
| giftskap | marriage |
| gips | plaster cast |
| glas | glass |
| glasiar | glacier |
| gleda | gladden, delight |
| glede | joy, gladness |
| glomma | forget |
| gnida | rub |
| gnista | spark |
| gnistra | sparkle, glitter |
| goed | good |
| golv | floor |
| gomma | hide |
| graa | grey |
| graata | weep, cry |
| grad | degree |
| graf | graph |
| gram | gram |
| gran | spruce |
| grapefrukt | grapefruit |
| gras | grass; weed (marijuana, slang) |
| gratis | free of charge |
| grava | dig |
| gravid | pregnant |
| gravitasjon | gravity |
| grens | border |
| grensa | limit (v.) |
| grilla | grill |
| gripa | grip, seize |
| gron | green |
| gronsak | vegetable |
| grubbla | brood, ponder |
| grund | foundation, ground |
| grunda | found, establish |
| grundare | founder |
| grundloj | constitution |
| grupp | group |
| grym | cruel |
| gryn | cereal, grain |
| gryta | pot |
| gud | god, God |
| gudig | divine |
| gul | yellow |
| gurka | cucumber |
| gyldig | valid |
| haar | hair |
| halda | hold |
| hallej | hello |
| hallon | raspberry |
| hals | neck |
| halv | half |
| ham | him |
| hamn | harbour |
| han | he |
| hand | hand |
| handel | trade (n.) |
| handla | trade, shop (v.) |
| handlare | merchant |
| handling | act, action |
| handske | glove |
| hans | his |
| hantera | handle, manage |
| hard | hard |
| hare | hare |
| hastighed | velocity |
| hata | hate |
| hatt | hat |
| hav | ocean, open sea |
| hava | have |
| havre | oat |
| heera | hear |
| hel | whole, entire |
| hela | heal |
| heller | rather, preferably, sooner |
| helning | healing |
| helsa | health |
| helsaministerie | health ministry |
| helsevard | healthcare |
| helvet | hell |
| hem | home |
| hemd | shirt |
| hemlig | secret |
| hemlos | homeless, unhoused |
| hemloshed | homelessness |
| hemskap | belonging |
| hen | they (singular) |
| henne | her (object) |
| hens | their (singular possessive) |
| herda | endure, persevere |
| herlig | great, glorious |
| herre | gentleman, lord |
| hest | horse |
| het | hot |
| hier | here |
| hierarki | hierarchy |
| himmel | sky |
| hinder | obstacle, barrier |
| historie | history |
| historisk | historical |
| hjelp | help (n.) |
| hjelpa | help |
| hjerne | brain |
| hjerneslag | stroke (medical) |
| hjerta | heart |
| hjertinfarkt | heart attack |
| hjertsorg | heartbreak |
| hjul | wheel |
| hoe | how |
| hoft | hip |
| hoger | right (direction) |
| hogtalare | speaker (audio) |
| Hollandsk | Dutch |
| holm | islet |
| hon | she |
| honing | honey |
| hons | her (possessive) |
| hoog | high |
| hopa | hope (v.) |
| hopp | hope (n.) |
| hoppa | jump |
| horlur | headphone |
| hormon | hormone |
| horn | horn |
| host | autumn |
| hosta | cough |
| hotell | hotel |
| hoved | head |
| hovedstad | capital city |
| hovedverk | headache |
| hud | skin |
| hul | hollow |
| humor | humor |
| hund | dog |
| hundra | hundred |
| hunger | hunger |
| hungra | hunger, crave |
| hungrig | hungry |
| hus | house |
| husbond | husband |
| huv | hood, cowl |
| hylla | shelf |
| hypotese | hypothesis |
| hyra | rent |
| hytta | cottage, hut |
| iaften | tonight, this evening |
| idag | today |
| ideal | ideal |
| idealisme | idealism |
| idee | idea |
| identitet | identity |
| ideologi | ideology |
| idiot | idiot |
| igaar | yesterday |
| igen | again |
| igenkenna | recognize |
| ik | I |
| ikon | icon |
| iland | island |
| ilandgrupp | archipelago |
| imaginera | imagine |
| immun | immune |
| imorgen | tomorrow |
| import | import |
| in | in |
| index | index |
| individ | individual |
| industri | industry |
| industriell | industrial |
| infarkt | infarction |
| infeksjon | infection |
| inflamasjon | inflammation |
| inflasjon | inflation |
| informasjon | information |
| ingefer | ginger |
| ingen | none, nobody |
| ingenwaar | nowhere |
| injeksjon | injection |
| inkomst | income |
| innen | within |
| innstellning | setting (config) |
| inre | inner, internal |
| insekt | insect |
| insikt | insight, realization |
| installasjon | installation |
| installera | install |
| instinkt | instinct |
| institusjon | institution |
| instruera | teach |
| instrument | instrument |
| integrasjon | integration |
| integritet | integrity |
| intelligens | intelligence |
| intens | intense |
| intensjon | intention |
| interessant | interesting |
| internett | internet |
| intim | intimate |
| intimitet | intimacy |
| invandring | immigration |
| invasjon | invasion |
| ironi | irony |
| is | ice |
| iskrem | ice cream |
| isolera | isolate |
| isolering | isolation |
| isotop | isotope |
| isskap | refrigerator |
| isteden | instead |
| iver | eagerness, zeal, excitement |
| ja | yes |
| jaar | year |
| jaga | hunt |
| jakk | jacket |
| januar | January |
| jemlik | equal (social) |
| jemlikhed | social equality |
| jer | your (plural) |
| jern | iron |
| jon | ion |
| jord | earth, soil |
| jordber | strawberry |
| jordbruk | agriculture |
| jordskelv | earthquake |
| journalist | journalist |
| ju | you (plural) |
| juli | July |
| juni | June |
| justis | justice |
| kaal | cabbage |
| kabel | cable |
| kafe | coffee |
| kaj | quay |
| kaka | cake |
| kakao | cocoa |
| kald | cold |
| kalkon | turkey |
| kalla | call, name |
| kamer | chamber |
| kamera | camera |
| kamerat | comrade, companion |
| kameratskap | companionship |
| kamp | fight, struggle (n.) |
| kampa | fight, struggle |
| kampanj | campaign |
| kanabis | cannabis |
| kandidat | candidate |
| kanel | cinnamon |
| kano | canoe |
| kanske | maybe, perhaps |
| kaos | chaos |
| kap | cape, headland |
| kapital | capital |
| kapitalisme | capitalism |
| kappa | coat |
| kapten | captain |
| karakter | character |
| karantene | quarantine |
| karbon | carbon |
| karot | carrot |
| kart | map, chart, card |
| kasino | casino |
| kasta | throw |
| katalog | directory, catalogue |
| katalysator | catalyst |
| katastrof | catastrophe |
| katt | cat |
| keks | biscuit |
| kella | source |
| kemi | chemistry |
| kemikalie | chemical (substance) |
| kemisk | chemical |
| kenna | feel |
| kenning | feeling |
| kerl | vessel (anatomical) |
| kersbeer | cherry |
| kilo | kilogram |
| kilometer | kilometre |
| kind | child |
| kindtid | childhood |
| kindvagn | pram, baby carriage |
| kinetisk | kinetic |
| kip | chicken |
| kirk | church |
| kirurg | surgeon |
| kirurgi | surgery (discipline) |
| kista | chest (box) |
| kjol | skirt |
| klaada | itch |
| klaga | complain |
| klar | clear |
| klarhed | clarity |
| klass | class |
| kleda | dress (v.) |
| kleder | clothes |
| klia | itch (v.) |
| klikk | click (n.) |
| klikka | click (v.) |
| klimat | climate |
| klimma | climb |
| klinik | clinic |
| klippa | cliff |
| klistra | paste |
| klok | clock |
| klora | scratch |
| klyva | split |
| knapp | button |
| knie | knee |
| kniv | knife |
| knulla | fuck (vulgar) |
| knusa | crush, shatter, devastate |
| knyta | tie, knot, bind |
| ko | cow |
| kod | code |
| koda | code (v.) |
| koel | cool |
| koela | chill, relax (slang) |
| koffert | suitcase |
| koffertrum | boot, trunk (of a car) |
| kognisjon | cognition |
| kognitiv | cognitive |
| koj | bunk |
| kok | kitchen |
| koka | cook (v.) |
| kokare | cook (n.) |
| kokos | coco |
| kokosnoet | coconut |
| koktail | cocktail |
| kollegie | college |
| kollektiv | collective |
| komers | trade (n.) |
| komet | comet |
| komite | committee |
| komma | come |
| kommensera | begin |
| kommisjon | commission |
| kommun | municipality |
| kommunikasjon | communication |
| kompas | compass |
| kompensera | pay |
| kon | sex, gender |
| kondensera | condense |
| konferens | meeting |
| konflikt | conflict |
| kong | king |
| konklusjon | conclusion |
| konsens | consensus |
| konsensbrekare | consensus-breaker |
| konsert | concert |
| konservativ | conservative |
| konsoll | console |
| konst | art |
| konstant | constant |
| konstruera | construct |
| konstruksjon | construction |
| konsument | consumer |
| kontekst | context |
| konto | account |
| kontrakt | contract |
| kontrast | contrast |
| konvenor | convenor |
| konversasjon | conversation |
| koordinat | coordinate |
| kopa | buy |
| kopie | copy (n.) |
| kopiera | copy (v.) |
| kopp | cup |
| koppar | copper |
| kopstad | town, market town |
| kora | drive |
| korall | coral |
| korelasjon | correlation |
| korg | basket |
| korn | barley, grain |
| korrupsjon | corruption |
| kort | short |
| korv | sausage |
| kost | cost |
| kosta | cost (v.) |
| kott | meat |
| krab | crab |
| kraft | strength, power |
| kraftfelt | force field |
| kraftig | powerful |
| kramma | squeeze, hug |
| kran | tap, faucet |
| kranium | skull |
| krasch | crash (n.) |
| kraska | crash |
| krav | demand, claim |
| krava | demand |
| kredit | credit |
| kreft | cancer |
| krem | cream |
| krig | war |
| kris | crisis |
| krispig | crispy |
| kristall | crystal |
| kritisk | critical |
| krok | hook |
| kromosom | chromosome |
| krona | krona (currency) |
| kronisk | chronic |
| kropp | body |
| krydda | spice |
| krykke | crutch |
| krypa | crawl |
| krypig | cringeworthy (slang) |
| kryptera | encrypt |
| kryptering | encryption |
| kub | cube |
| kudde | pillow, cushion |
| kuk | dick (vulgar) |
| kulle | hill |
| kultur | culture |
| kulturell | cultural |
| kunna | can, be able |
| kunstig | artificial |
| kur | cure |
| kurios | curious |
| kurs | course |
| kusin | cousin |
| kust | coast |
| kval | anguish, torment |
| kvalifikasjon | qualification |
| kvin | woman |
| kvorum | quorum |
| kwadrat | square (shape) |
| kwalme | nausea |
| kwantum | quantum |
| kwark | quark |
| kwart | quarter |
| kwave | nitrogen |
| kwestion | question |
| kwittens | receipt |
| kwizin | cuisine |
| kwot | quotient |
| kyss | kiss (n.) |
| kyssa | kiss |
| laag | low |
| laan | loan |
| laana | borrow, lend |
| laar | thigh |
| laas | lock |
| laboratorie | laboratory |
| lada | box |
| ladda | load, charge |
| laddare | charger |
| laddning | charge (n.) |
| lag | law (of nature) |
| laga | repair, fix |
| lagra | store, save |
| lagring | storage |
| lak | lake |
| lakka | laugh |
| lakking | laughter |
| laks | salmon |
| lam | lamb |
| lampa | lamp |
| land | land, country |
| landsbygd | countryside |
| landskap | landscape |
| lang | long |
| langs | along |
| langsam | slow |
| Lapentieur | Lapentieur (the capital) |
| Laphurdeen | Laphurdeen (the Commonwealth) |
| Laphurdeener | Laphurdeener (citizen) |
| Laphurdi | the Laphurdi language |
| last | cargo |
| lata | let, allow |
| lava | lava |
| led | joint (of the body) |
| leda | lead |
| ledare | leader |
| ledarskap | leadership |
| leder | leather |
| ledminister | lead minister |
| legga | lay |
| leka | play (as a child) |
| lekare | physician |
| lekker | tasty, nice, great (all-purpose approval) |
| leksjon | lesson |
| lemna | leave |
| lemonad | lemonade |
| lengta | long, yearn |
| lengting | longing, yearning |
| lenk | link |
| lente | spring (season) |
| lepp | lip |
| lera | teach |
| lerare | teacher |
| lering | education, teaching |
| lerna | learn |
| lesa | read |
| lesare | reader |
| leta | search |
| lett | light (weight), easy |
| letta | lighten, relieve |
| letting | relief |
| leva | live |
| levande | alive, living |
| lever | liver |
| leverera | deliver |
| liberalisme | liberalism |
| lida | suffer |
| lidare | sufferer |
| liding | suffering |
| liev | love (n.) |
| lieva | love |
| lievare | lover |
| lievling | beloved, darling |
| lievverd | lovable |
| ligga | lie (recline) |
| lik | similar, alike |
| limett | lime |
| linje | line (route, stroke) |
| lins | lens |
| lita | trust, rely on |
| liten | small |
| liter | litre |
| liv | life |
| lobbying | lobbying |
| lofte | promise (n.) |
| logaritme | logarithm |
| logga | log (in) |
| loj | law |
| lojforslag | bill (draft law) |
| lojlig | lawful |
| lok | onion |
| lokal | local |
| lon | wage |
| lopp | run, course |
| lordag | Saturday |
| losa | dissolve, solve |
| losenord | password |
| losning | solution |
| lova | promise, vow |
| luft | air |
| lugn | calm |
| lukta | smell |
| lunga | lung |
| lunsj | lunch |
| lus | louse |
| lyd | sound |
| lyda | sound (v.) |
| lyfta | lift |
| lykka | luck |
| lys | light |
| lyssna | listen |
| lystorn | lighthouse |
| maage | gull |
| maal | goal, objective |
| maaltid | meal |
| maan | moon |
| maat | measure (n.) |
| mage | belly |
| magnet | magnet |
| magnetisk | magnetic |
| magnetisme | magnetism |
| magsap | gastric juice |
| magsekk | stomach (organ) |
| maj | May |
| majones | mayonnaise |
| majoritet | majority |
| majs | corn, maize |
| makt | power |
| mal | time, occurrence |
| mala | paint (v.) |
| malad | sick |
| malare | painter |
| malign | malignant |
| maling | painting |
| man | man |
| manad | month |
| mandag | Monday |
| mange | many |
| mango | mango |
| mapp | folder |
| mariera | marry |
| marknad | market |
| marknadsplas | marketplace |
| markor | cursor |
| mars | March |
| marsk | salt marsh |
| masjin | machine |
| masjinvara | hardware |
| mask | mask |
| massa | mass |
| mast | mast |
| mat | food |
| matematik | mathematics |
| matros | sailor |
| matta | rug, mat |
| med | with |
| medan | while |
| medel | means, agent |
| medesin | doctor |
| media | media |
| median | median |
| medikament | medication |
| medisin | medicine |
| meditasjon | meditation |
| meditera | meditate |
| medlem | member |
| medlemskap | membership |
| medvetande | consciousness |
| medveten | conscious |
| mekanik | mechanics |
| mel | flour |
| melk | milk |
| mellan | between |
| melodi | melody |
| melon | melon |
| membran | membrane |
| men | but |
| mening | opinion, meaning |
| mennisk | human being |
| mental | mental |
| meny | menu |
| mer | more |
| merg | marrow |
| merke | mark |
| merkverdig | remarkable, curious |
| mersi | thanks |
| mesasj | message |
| mest | most |
| meta | measure (v.) |
| metafor | metaphor |
| metaforisk | metaphorical |
| metall | metal |
| meter | metre |
| meting | measurement |
| metod | method |
| mett | full (after eating), sated |
| middag | midday, noon |
| middel | middle |
| midja | waist |
| migrant | migrant |
| migrasjon | migration |
| mij | me (object) |
| mikrob | microbe |
| mikrofon | microphone |
| mikroskop | microscope |
| million | million |
| min | my |
| minder | less |
| mineral | mineral |
| minister | minister |
| ministerie | ministry |
| minna | remember |
| minne | memory (store) |
| minoritet | minority |
| minst | least |
| minut | minute |
| misbruk | abuse, misuse |
| mist | fog |
| mista | ghost, vanish on someone (slang) |
| mjuk | soft |
| mobel | furniture |
| mobil | mobile, movable |
| mod | courage |
| mode | fashion |
| modell | model |
| moder | mother |
| modern | modern |
| modig | brave |
| moeton | mutton |
| mogen | mature, ripe |
| mojlig | possible |
| mojlighed | possibility |
| molekyl | molecule |
| moment | moment, instant |
| moral | morality |
| moralisk | moral |
| mord | murder |
| morda | murder (v.) |
| morgen | morning |
| mork | dark |
| morkhed | darkness |
| moske | mosque |
| moste | must |
| mot | against, toward |
| mote | meeting |
| motivasjon | motivation |
| motivera | motivate |
| motor | motor, engine |
| motorhuv | bonnet, hood (of a car) |
| motto | motto |
| mund | mouth |
| mus | mouse |
| museum | museum |
| musik | music |
| muskel | muscle |
| muskot | nutmeg |
| musla | mussel, shellfish |
| muts | cap |
| mykke | much |
| mynt | coin, mint |
| nabo | neighbour |
| nabolag | neighbourhood |
| nagel | fingernail |
| nara | near |
| narkose | anaesthesia |
| nasjon | nation |
| nasjonal | national |
| natt | night |
| natur | nature |
| naturlig | natural |
| navigasjon | navigation |
| navn | name |
| ned | down |
| nedladda | download |
| nedladdning | download (n.) |
| nej | no |
| nera | nourish |
| nering | nourishment, nutrition |
| nerma | approach, near |
| nerv | nerve |
| nervaro | presence |
| nervos | nervous |
| nese | nose |
| neste | next |
| net | net |
| nett | net (mesh) |
| nettlesare | browser |
| netverk | network |
| neutron | neutron |
| neve | nephew |
| nien | nine |
| niende | ninth |
| nienti | ninety |
| nikt | niece |
| nit | not |
| nitten | nineteen |
| nivaa | level |
| njure | kidney |
| nod | emergency, distress |
| nodtid | state of emergency |
| nodvendig | necessary |
| noet | nut |
| nokel | key |
| nor | nor |
| nord | north |
| nordster | north star, polestar |
| norm | norm |
| normal | normal |
| notifikasjon | notification |
| november | November |
| nu | now |
| nudel | noodle |
| null | zero |
| num | numb |
| nummer | number |
| nutid | present (time) |
| ny | new |
| nyhed | news |
| nykter | sober |
| nykterhed | sobriety |
| nysa | sneeze |
| obduksjon | autopsy |
| observasjon | observation |
| observera | observe |
| oek | also |
| oest | east |
| offentlig | public |
| offisiell | official |
| offline | offline |
| oformell | informal, casual |
| ofta | often |
| og | and |
| ogensidig | unrequited, one-sided |
| ogyldig | invalid, void |
| oje | eye |
| ojeblik | moment, instant |
| ojemlikhed | inequality |
| okraft | incapacity |
| oksygen | oxygen |
| oktober | October |
| olik | different |
| oliv | olive |
| olja | oil |
| om | if, whether, about |
| omlopp | orbit |
| omojlig | impossible |
| on | on |
| onkel | uncle |
| onkologi | oncology |
| online | online |
| onsdag | Wednesday |
| open | open |
| operasjon | operation |
| operera | operate (surgically) |
| opna | open (v.) |
| opp | up |
| oppdaga | discover |
| oppdaging | discovery |
| oppladda | upload |
| oppmerksam | attentive |
| oppmerksamhed | attention |
| optik | optics |
| opulent | rich |
| oranj | orange |
| ord | word |
| ore | ear |
| organ | organ (biological) |
| organdonor | organ donor |
| organisasjon | organisation |
| organism | organism |
| orientasjon | orientation |
| orm | snake |
| os | us |
| oskyldig | innocent |
| ost | cheese |
| ostra | oyster |
| otrolig | astounding, incredible |
| oval | oval |
| over | over, above |
| overalt | everywhere |
| overgang | transition |
| overleva | survive |
| overleving | survival |
| overraska | surprise |
| overrasking | surprise (n.) |
| oversetta | translate |
| oversettare | translator |
| overvaka | surveil |
| overvaking | surveillance |
| ozon | ozone |
| pan | pan |
| papper | paper |
| paprika | bell pepper |
| par | pair |
| paradis | paradise |
| paradoks | paradox |
| parallell | parallel |
| parasit | parasite |
| park | park |
| parkera | park (v.) |
| parlament | parliament |
| parlamentisk | parliamentary |
| parskap | relationship, partnership |
| parti | party (political) |
| partikel | particle |
| pas | passport |
| pasient | patient |
| pasta | pasta |
| patroon | pattern |
| pels | fur |
| peng | money |
| penn | pen |
| peppar | pepper |
| per | pear |
| perfekt | perfect |
| permitera | allow |
| persepsjon | perception |
| persik | peach |
| persilja | parsley |
| persistens | persistence |
| persistent | persistent |
| person | person |
| personlig | personal |
| personlighed | personality |
| perspektiv | perspective |
| piano | piano |
| piksel | pixel |
| pille | pill |
| pinn | stick, twig |
| pipa | pipe |
| pjes | play (theatre) |
| plan | plan |
| planera | plan (v.) |
| planet | planet |
| planta | plant |
| plas | square, place |
| plasma | plasma |
| plattform | platform |
| plikt | duty |
| plomma | plum |
| plotslig | sudden |
| podd | podcast |
| pojk | boy |
| pokker | pox (oath) |
| politi | police |
| politik | politics |
| politisk | political |
| populisme | populism |
| por | pore |
| pork | pork |
| porsjon | portion |
| portrett | portrait |
| post | mail, post |
| potensiell | potential |
| potet | potato |
| pover | poor |
| praktisk | practical |
| preambel | preamble |
| president | president |
| presse | press (media) |
| pris | price |
| privat | private |
| privilegie | privilege |
| problem | problem |
| produkt | product |
| produktiv | productive |
| produsent | producer |
| produsera | produce |
| profesjon | profession |
| profesjonell | professional |
| professor | professor |
| profil | profile |
| program | program |
| programmera | program (v.) |
| programmerare | programmer |
| programvara | software |
| progresiv | progressive |
| proibera | forbid |
| projekt | project |
| propaganda | propaganda |
| proporsjonal | proportionate |
| propp | plug, stopper |
| prosent | percent |
| prosess | process |
| prosessor | processor |
| protein | protein |
| protes | prosthesis |
| protest | protest |
| protestera | protest (v.) |
| proton | proton |
| prov | test |
| prova | try |
| proving | review, testing |
| provins | province |
| psykiater | psychiatrist |
| psykiatri | psychiatry |
| psykolog | psychologist |
| psykologi | psychology |
| psykologisk | psychological |
| publisera | publish |
| publisering | publication |
| puls | pulse |
| pumpa | pumpkin |
| punisjon | punishment |
| punkt | point |
| purjolok | leek |
| purpur | purple |
| pusja | push |
| raa | raw |
| raad | council |
| radera | delete |
| radio | radio |
| rak | straight |
| rakett | rocket |
| rangera | rank (v.) |
| rangering | ranking, ranked choice |
| rapport | report |
| ras | race (of people) |
| rasa | rage, rave |
| raseri | rage, fury |
| ratt | rat |
| reaksjon | reaction |
| realitet | reality |
| redd | afraid |
| redda | save, rescue |
| redding | rescue, salvation |
| reflektera | think |
| reform | reform |
| regel | rule |
| regera | rule, govern |
| region | region |
| register | register |
| regn | rain |
| regna | rain (v.) |
| regnboge | rainbow |
| rehabilitering | rehabilitation |
| reis | journey |
| rekja | shrimp |
| rekna | calculate |
| rekning | bill, invoice |
| rekt | right (entitlement) |
| rektangel | rectangle |
| relativitet | relativity |
| relisjon | religion |
| ren | clean |
| renkontrera | meet |
| rep | rope |
| repetera | repeat |
| repetisjon | repetition |
| representera | represent |
| reprimera | repress |
| reproduksjon | reproduction |
| resa | travel |
| resare | traveller |
| resept | prescription |
| reserv | reserve |
| residens | house |
| respondera | answer |
| respons | answer (n.) |
| restoran | restaurant |
| resultat | result |
| retning | direction |
| rett | correct, right |
| retta | correct (v.) |
| rettika | radish |
| rettvis | just, fair |
| rettvished | justice, fairness |
| rev | fox |
| revisor | auditor |
| revolusjon | revolution |
| ribbe | rib |
| rida | ride |
| rif | reef |
| rijp | ripe |
| rik | rich |
| rikhed | wealth |
| rim | rhyme |
| rima | rhyme (v.) |
| rimlig | reasonable |
| rinna | flow |
| ris | rice |
| risk | risk |
| riskera | risk (v.) |
| ritual | ritual |
| riva | tear |
| roa | row (v.) |
| robot | robot |
| rod | red |
| rok | smoke |
| rolle | role |
| roman | novel |
| romans | romance |
| romantisk | romantic |
| rontgen | x-ray |
| ropa | call, shout |
| roping | call, incitement |
| ror | rudder |
| rora | touch, stir |
| rorelse | motion |
| ros | rose |
| rosa | pink |
| rosmarin | rosemary |
| rost | voice |
| rosta | roast |
| rot | root |
| rue | street |
| rugg | back |
| ruggmerg | spinal cord |
| rulla | roll |
| rullstol | wheelchair |
| rum | room |
| rund | round |
| rundom | around |
| rural | rural |
| rutten | rotten |
| rymd | space (open, outer) |
| rytme | rhythm |
| saarbar | vulnerable |
| saarbarhed | vulnerability |
| safran | saffron |
| saga | story, tale |
| sak | matter, case, affair |
| sakna | miss, lack |
| salat | salad |
| saliv | saliva |
| salt | salt |
| saltig | salty; resentful (slang) |
| samarbeta | cooperate |
| samarbete | cooperation |
| samfund | society |
| samla | gather, assemble |
| samling | assembly, gathering, collection |
| samme | same |
| samtal | conversation |
| samtidig | simultaneous |
| samvelde | commonwealth |
| sand | sand |
| sang | song |
| sannolik | probable |
| sannolikhed | probability |
| sant | true |
| santhed | truth |
| sap | juice |
| sar | wound |
| sara | hurt, wound (v.) |
| satellit | satellite |
| se | see |
| segel | sail (n.) |
| segla | sail (v.) |
| seja | say |
| sekel | century |
| seker | safe, sure |
| sekerhed | security |
| sekerhedskopie | backup |
| sekk | sack, bag |
| seks | six; sex (the act) |
| seksi | sexy |
| sekste | sixth |
| seksten | sixteen |
| seksti | sixty |
| seksuell | sexual |
| sekund | second (time unit) |
| sel | seal |
| selektera | choose |
| self | self |
| selfmord | suicide |
| selfom | although, even though |
| selfs | even |
| selja | sell |
| seljing | sale, selling |
| selleri | celery |
| selvbild | self-image |
| selvkensla | self-esteem |
| sena | tendon |
| senap | mustard |
| senat | senate |
| senator | senator |
| senda | send |
| sending | sending |
| seng | bed |
| sensur | censorship |
| sentral | central |
| sep | soap |
| september | September |
| seremoniell | ceremonial |
| serlig | special, particular |
| server | server |
| servera | serve |
| sesong | season |
| sete | seat (office) |
| setning | sentence |
| setta | set, put |
| sfer | sphere |
| sida | side |
| siden | since; ago |
| sider | cider |
| siens | science |
| siffra | digit |
| signal | signal |
| sik | himself, herself, themself (reflexive) |
| sild | herring |
| silke | silk |
| silver | silver |
| simma | swim |
| singa | sing |
| sinn | mind |
| sinnsjukhed | mental illness |
| sirene | siren |
| sirkel | circle |
| sist | last, final |
| sitron | lemon |
| sitta | sit |
| situasjon | situation |
| sivil | civil |
| sjalu | jealous |
| sjamber | room |
| sjampanje | champagne |
| sjanson | song |
| sjarm | charm, rizz |
| sjarta | charter |
| sjef | cook (n.) |
| sjel | soul |
| sjelig | spiritual |
| sjelvrend | soulmate, kindred spirit |
| sjili | chili |
| sjokolad | chocolate |
| sju | seven |
| sjuk | sick |
| sjukdom | disease |
| sjukhed | illness |
| sjukhus | hospital |
| sjukskotare | nurse |
| sjunde | seventh |
| sjunka | sink |
| sjuti | seventy |
| sjutten | seventeen |
| skaal | bowl |
| skada | harm (v.) |
| skade | injury |
| skalle | skull |
| skam | shame |
| skamma | shame (v.) |
| skammig | embarrassed |
| skanna | scan |
| skanner | scanner |
| skap | cupboard |
| skapa | create |
| skapare | creator |
| skaping | creation |
| skarp | sharp |
| skatt | tax |
| sked | spoon |
| skel | shell |
| skelett | skeleton |
| skelv | tremor |
| skelva | tremble |
| skera | cut |
| skerf | scarf |
| skerm | screen |
| skermbild | screenshot |
| skift | shift (n.) |
| skifta | shift, switch |
| skilja | separate, part |
| skiljing | separation, parting |
| skilsmisse | divorce |
| skinka | ham |
| skip | ship |
| skit | shit (mild oath) |
| skiva | disk |
| skoe | shoe |
| skog | forest |
| skol | school |
| skola | shall (future auxiliary) |
| skota | tend, care for |
| skotare | carer |
| skott | shot |
| skrift | writing, petition |
| skrika | scream |
| skriva | write |
| skrivare | printer |
| skuld | debt |
| skydd | protection |
| skydda | protect |
| skyldig | guilty |
| slag | blow, stroke |
| slaga | hit |
| slekt | kin, family line |
| slem | mucus |
| slett | smooth |
| slo | dull, blunt |
| smak | taste |
| smaka | taste (v.) |
| smal | narrow |
| smart | smart |
| smarttelefon | smartphone |
| smed | smith |
| smelta | melt |
| smerta | pain |
| smerthed | soreness |
| smertig | painful |
| smertmedel | painkiller |
| smila | smile |
| smitta | contagion |
| smittsam | contagious |
| smor | butter |
| smuts | dirt |
| smutsig | dirty |
| snabb | fast |
| snabbhed | speed |
| snart | soon |
| sne | snow |
| snyfta | sob |
| so | so, thus |
| soda | soda |
| sofa | sofa, couch |
| sok | sock |
| soka | search, seek |
| sokning | search (n.) |
| sol | sun |
| soldat | soldier |
| solidaritet | solidarity |
| solnedgang | sunset |
| soloppgang | sunrise |
| solsystem | solar system |
| somhoe | somehow |
| somme | some |
| sommer | summer |
| somtid | sometime |
| somting | something, anything |
| somwaar | somewhere, anywhere |
| somwie | someone, anyone |
| somwies | someone's, anyone's |
| son | son |
| sopp | fungus |
| soppa | soup |
| sorg | sorrow, grief |
| sorga | mourn, grieve |
| sos | sauce |
| sosial | social |
| sosialisme | socialism |
| sosiolog | sociologist |
| sosiologi | sociology |
| sot | sweet |
| sova | sleep |
| sovig | sleepy |
| sovrum | bedroom |
| sparka | kick |
| spegel | mirror |
| spektrum | spectrum |
| spel | game |
| spela | play (a game, music) |
| spelhus | casino |
| spenat | spinach |
| spin | spider |
| spoka | haunt |
| spoke | ghost, specter |
| sport | sport |
| spotta | spit |
| sprak | language |
| spreka | speak |
| sprekare | speaker |
| spreking | speech, speaking |
| springa | run |
| sprit | liquor, spirits |
| spruta | syringe |
| spuus | spouse |
| staa | stand |
| stad | city |
| stadshus | city hall |
| stand | status, standing |
| stanna | stay, remain |
| stark | strong |
| starkhed | intensity |
| start | start, beginning |
| starta | start (v.) |
| stasjon | station |
| stat | state |
| statistik | statistics |
| statshoved | head of state |
| statstjenst | civil service |
| status | status |
| steg | step |
| steka | fry |
| stem | vote (n.) |
| stemma | vote |
| stemmare | voter, elector |
| stemming | voting |
| sten | stone |
| stenga | close (v.) |
| ster | star |
| sterbild | constellation |
| steril | sterile |
| sterlys | starlight |
| stert | tail |
| sterva | die |
| sti | path |
| stiga | rise |
| stikka | knit |
| still | quiet, still |
| stillhed | silence |
| stimulera | stimulate |
| stinga | stab |
| stol | chair |
| stolt | proud |
| stoppa | stop |
| stor | big |
| storm | storm |
| storsegla | flex, show off (slang) |
| stotta | support, prop up |
| stov | dust |
| stovel | boot |
| straala | radiate |
| straaling | radiation |
| straff | punishment |
| straffa | punish |
| strand | beach, shore |
| strejk | strike (labour) |
| strejka | strike (v.) |
| stress | stress |
| strev | effort, striving |
| streva | strive, toil |
| strofe | stanza |
| strom | current, stream |
| stromma | stream (v.) |
| stromming | streaming |
| struktur | structure |
| student | student |
| studera | study |
| styra | steer |
| styring | governance, government |
| substans | substance |
| suga | suck |
| suggerera | suggest, propose |
| sukker | sugar |
| summa | sum |
| sund | healthy |
| sundag | Sunday |
| sur | sour |
| surfa | browse |
| surfplatta | tablet |
| suveren | sovereign |
| suverenitet | sovereignty |
| svaar | difficult |
| svaarhed | difficulty |
| svag | weak |
| svamp | mushroom |
| svan | swan |
| svar | answer (n.) |
| svara | answer |
| svart | black |
| svella | swell |
| Svensk | Swedish |
| svett | sweat |
| svimmel | dizzy |
| svin | pig |
| svulst | swelling |
| sya | sew |
| syd | south |
| sykel | bicycle |
| sykla | cycle (v.) |
| sylt | jam |
| symbol | symbol |
| symmetri | symmetry |
| symptom | symptom |
| synagoge | synagogue |
| syndrom | syndrome |
| synonym | synonym |
| syra | acid |
| system | system |
| syster | sister |
| ta | take |
| taar | tear (weeping) |
| tak | roof |
| tal | speech |
| tallrik | plate |
| talman | speaker (presiding officer) |
| tand | tooth |
| tandlekare | dentist |
| tandverk | toothache |
| tanke | thought |
| tante | aunt |
| tapa | lose |
| tarm | gut, guts, intestine |
| tast | key (button) |
| tastbord | keyboard |
| te | to |
| teater | theatre |
| teder | tender, gentle |
| tee | tea |
| teken | sign, symbol (written) |
| tekst | text |
| telefon | telephone |
| teleskop | telescope |
| televisjon | television |
| tella | count |
| telling | counting |
| tema | theme |
| tempel | temple |
| temperatur | temperature |
| tenka | think |
| teorem | theorem |
| teori | theory |
| terapeut | therapist |
| terapi | therapy |
| termin | term (of office) |
| terminera | end (v.) |
| termisk | thermal |
| termodynamik | thermodynamics |
| termometer | thermometer |
| territorie | territory |
| terug | back (returning) |
| tesamme | together |
| tid | time |
| tidning | newspaper |
| tien | ten |
| tiende | tenth |
| tij | tide |
| tilata | allow |
| tilbaka | back, backward |
| tilhora | belong |
| tillit | trust, confidence |
| tils | until |
| timjan | thyme |
| ting | thing |
| tisdag | Tuesday |
| titel | title |
| tjenst | service |
| tjok | thick |
| toalett | toilet |
| tolka | interpret |
| tolkning | interpretation |
| tolv | twelve |
| tom | empty |
| tomat | tomato |
| tomhed | emptiness |
| tomma | empty (v.) |
| ton | tone |
| torden | thunder |
| torka | dry, wipe |
| torn | tower |
| torr | dry |
| torsdag | Thursday |
| torsk | cod |
| torst | thirst |
| torstig | thirsty |
| tortur | torture |
| traad | thread, wire |
| traadlos | wireless |
| tradisjon | tradition |
| trafik | traffic |
| tragedi | tragedy |
| tragisk | tragic |
| trakig | boring |
| tram | tram |
| transformasjon | transformation |
| transformera | transform |
| transparens | transparency |
| transparent | transparent |
| transplantasjon | transplant (n.) |
| transplantera | transplant (v.) |
| transport | transport |
| trappa | stairs |
| trauma | trauma |
| travalje | work (n.) |
| tre | tree |
| treffa | meet |
| tren | train |
| tretten | thirteen |
| tri | three |
| triangel | triangle |
| tridde | third |
| trist | sad |
| triti | thirty |
| tro | belief, faith |
| troa | believe |
| troest | comfort, consolation |
| troesta | comfort, console |
| tromma | drum |
| trots | despite, in spite of |
| trott | tired |
| trua | threaten |
| trussel | threat |
| trykk | pressure |
| tulp | tulip |
| tumor | tumour |
| tunel | tunnel |
| tung | heavy |
| tunga | tongue |
| tunn | thin |
| tur | trip, tour |
| tusen | thousand |
| tutt | tit (colloquial) |
| tuul | tool |
| tvers | across, crosswise |
| tversover | across |
| tvivla | doubt |
| twe | two |
| tweti | twenty |
| ugn | oven |
| ull | wool |
| under | under, during, according to |
| undernera | undernourish, malnourish |
| undertrykka | oppress, suppress |
| undvika | avoid |
| ung | young |
| ungdom | youth |
| uniform | uniform |
| unik | unique |
| univers | universe |
| universell | universal |
| universitet | university |
| unjon | union |
| uppdatera | update (v.) |
| uppdatering | update (n.) |
| urbanisera | urbanise |
| urbanisering | urbanisation |
| urgent | urgent, emergent |
| urin | urine |
| ursekta | excuse, apologize |
| ursprung | origin |
| ut | out |
| uten | without |
| utforska | explore |
| utforsking | exploration |
| utgava | edition |
| utilisera | use |
| utland | foreign lands, abroad |
| utmana | challenge |
| utmaning | challenge (n.) |
| utmatta | exhaust, wear out |
| utmatting | exhaustion |
| utnemna | appoint |
| utnemning | appointment |
| utslag | rash |
| uttrykk | expression |
| uttrykka | express |
| utvandring | emigration |
| uur | hour |
| vaag | wave |
| vaar | our |
| vaat | wet |
| vagn | wagon, carriage |
| vakna | wake, wake up |
| vaksin | vaccine |
| vaksinera | vaccinate |
| vakuum | vacuum |
| val | whale |
| valg | election |
| vana | habit |
| vandra | walk, wander |
| vanilj | vanilla |
| vanlig | ordinary, usual |
| vara | goods, ware |
| vard | host |
| variabel | variable |
| varm | warm |
| varmhed | warmth |
| varsla | warn |
| varslare | whistleblower |
| varsling | warning |
| vaska | wash |
| vate | hydrogen |
| vatter | water |
| vatterfall | waterfall |
| vattermelon | watermelon |
| ved | at, by |
| veder | weather |
| vedve | widow |
| veg | road, way |
| vegg | wall |
| vek | week |
| veksa | grow |
| veksla | exchange, change |
| veksling | amendment, change |
| vekst | growth |
| vekt | weight |
| velja | choose |
| veljing | choice, choosing |
| velkom | welcome |
| velsigna | bless |
| velsigning | blessing |
| velvard | welfare |
| ven | vein |
| venda | turn |
| vendera | sell |
| venig | few |
| venstra | left |
| venta | wait |
| vera | be |
| verd | worth, worthy |
| verda | value (v.) |
| verde | value |
| verdig | dignified, worthy |
| verdighed | dignity |
| verifiera | verify |
| verifikasjon | verification |
| verk | ache |
| verka | ache, throb |
| vers | verse |
| versjon | version |
| vesel | ship |
| veta | know |
| vete | wheat |
| veto | veto |
| vetska | liquid |
| vetskap | knowledge |
| vev | tissue |
| vi | we |
| video | video |
| vik | cove, inlet |
| viktig | important |
| vilja | want, will |
| vin | wine |
| vind | wind |
| vinge | wing |
| vinkel | angle |
| vinna | win |
| vinst | profit |
| vinter | winter |
| viral | viral |
| virtuell | virtual |
| virus | virus |
| vis | wise |
| visa | show |
| visdom | wisdom |
| visjon | vision |
| viska | whisper |
| vit | white |
| vitlok | garlic |
| vitne | witness |
| vold | violence |
| volym | volume |
| votera | vote |
| vrend | friend |
| vrendskap | friendship |
| vuksen | adult |
| vulkan | volcano |
| waar | where |
| warfor | why |
| wat | what |
| webb | web |
| websida | website |
| wen | when |
| wensa | wish |
| wensing | wish (n.), desire |
| werf | shipyard |
| werk | work (n.) |
| werka | work |
| werkare | worker |
| werkarunjon | trade union |
| werld | world |
| west | west |
| wie | who |
| wifi | wifi |
| wolk | cloud |
| wolktjenst | cloud service |
| wonder | wonder, miracle |
| wonderbar | wonderful |
| wurm | worm |
| yoghurt | yoghurt |
| yta | surface, area |
| ytre | outer, external |
| zee | sea |
| zeeman | seaman |
